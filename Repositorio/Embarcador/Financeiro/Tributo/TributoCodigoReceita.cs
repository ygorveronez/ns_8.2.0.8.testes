...
{
    public class TributoCodigoReceita : RepositorioBase<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoCodigoReceita>
    {
        public TributoCodigoReceita(UnitOfWork unitOfWork) : base(unitOfWork) { }

        public Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoCodigoReceita BuscarPorCodigo(int codigo)
        {
            var query = this.SessionNHiBernate.Query<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoCodigoReceita>();
            var result = from obj in query where obj.Codigo == codigo select obj;
            return result.FirstOrDefault();
        }

        public List<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoCodigoReceita> Consultar(int codigoEmpresa, string codigoTributo, string descricao, Dominio.ObjetosDeValor.Embarcador.Enumeradores.SituacaoAtivoPesquisa status, string propOrdenacao, string dirOrdenacao, int inicioRegistros, int maximoRegistros)
        {
            var query = this.SessionNHiBernate.Query<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoCodigoReceita>();
