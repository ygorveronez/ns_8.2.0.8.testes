...
{
    public class TributoTipoImposto : RepositorioBase<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoImposto>
    {
        public TributoTipoImposto(UnitOfWork unitOfWork) : base(unitOfWork) { }

        public Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoImposto BuscarPorCodigo(int codigo)
        {
            var query = this.SessionNHiBernate.Query<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoImposto>();
            var result = from obj in query where obj.Codigo == codigo select obj;
            return result.FirstOrDefault();
        }

        public List<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoImposto> Consultar(int codigoEmpresa, string codigoTributo, string descricao, Dominio.ObjetosDeValor.Embarcador.Enumeradores.SituacaoAtivoPesquisa status, string propOrdenacao, string dirOrdenacao, int inicioRegistros, int maximoRegistros)
        {
            var query = this.SessionNHiBernate.Query<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoImposto>();
