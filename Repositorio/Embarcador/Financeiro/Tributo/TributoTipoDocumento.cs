...
{
    public class TributoTipoDocumento : RepositorioBase<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoDocumento>
    {
        public TributoTipoDocumento(UnitOfWork unitOfWork) : base(unitOfWork) { }

        public Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoDocumento BuscarPorCodigo(int codigo)
        {
            var query = this.SessionNHiBernate.Query<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoDocumento>();
            var result = from obj in query where obj.Codigo == codigo select obj;
            return result.FirstOrDefault();
        }

        public List<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoDocumento> Consultar(int codigoEmpresa, string codigoTributo, string descricao, Dominio.ObjetosDeValor.Embarcador.Enumeradores.SituacaoAtivoPesquisa status, string propOrdenacao, string dirOrdenacao, int inicioRegistros, int maximoRegistros)
        {
            var query = this.SessionNHiBernate.Query<Dominio.Entidades.Embarcador.Financeiro.Tributo.TributoTipoDocumento>();
