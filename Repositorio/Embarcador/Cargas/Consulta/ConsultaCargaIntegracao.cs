...
{
    sealed class ConsultaCargaIntegracao : Consulta.Consulta<Dominio.ObjetosDeValor.Embarcador.Carga.FiltroPesquisaRelatorioCargaIntegracao>
    {
        #region Construtores

        public ConsultaCargaIntegracao() : base(tabela: "T_CARGA_CARGA_INTEGRACAO as CargaIntegracao") { }
        ...
        protected override void SetarSelect(string propriedade, int codigoDinamico, StringBuilder select, StringBuilder joins, StringBuilder groupBy, bool somenteContarNumeroRegistros, Dominio.ObjetosDeValor.Embarcador.Carga.FiltroPesquisaRelatorioCargaIntegracao filtrosPesquisa)
        {
            switch (propriedade)
            {
                case "Codigo":
                    ...
                    {
                        select.Append("CargaIntegracao.CAI_CODIGO Codigo, ");
                        groupBy.Append("CargaIntegracao.CAI_CODIGO, ");
                    }
                    break;
                ...
                case "Tentativas":
                    ...
                    {
                        select.Append("CargaIntegracao.INT_NUMERO_TENTATIVAS Tentativas, ");
                        groupBy.Append("CargaIntegracao.INT_NUMERO_TENTATIVAS, ");
                    }
                    break;
                ...
                case "DataIntegracao":
                    ...
                    {
                        select.Append("CargaIntegracao.INT_DATA_INTEGRACAO DataIntegracao, ");
                ...
                case "DescricaoSituacao":
                    ...
                    {
                        select.Append("CargaIntegracao.INT_SITUACAO_INTEGRACAO Situacao, ");
                ...
                case "Mensagem":
                    ...
                    {
                        select.Append("CargaIntegracao.INT_PROBLEMA_INTEGRACAO Mensagem, ");
                ...
                case "Protocolo":
                    ...
                    {
                        select.Append("CargaIntegracao.CCA_PROTOCOLO Protocolo, ");
                        groupBy.Append("CargaIntegracao.CCA_PROTOCOLO, ");
                    }
                    break;
        ...
        protected override void SetarWhere(Dominio.ObjetosDeValor.Embarcador.Carga.FiltroPesquisaRelatorioCargaIntegracao filtrosPesquisa, StringBuilder where, StringBuilder joins, StringBuilder groupBy, List<Embarcador.Consulta.ParametroSQL> parametros = null)
        {
            string pattern = "yyyy-MM-dd";

            joins.Insert(0, @" join T_CARGA Carga on Carga.CAR_CODIGO = CargaIntegracao.CAR_CODIGO 
                               join T_TIPO_INTEGRACAO TipoIntegracao on TipoIntegracao.TPI_CODIGO = CargaIntegracao.TPI_CODIGO");
            ...
            if (filtrosPesquisa.DataInicioIntegracao != DateTime.MinValue)
                where.Append($" AND CAST(CargaIntegracao.INT_DATA_INTEGRACAO AS DATE) >= '{ filtrosPesquisa.DataInicioIntegracao.ToString(pattern) }'");

            if (filtrosPesquisa.DataFinalIntegracao != DateTime.MinValue)
                where.Append($" AND CAST(CargaIntegracao.INT_DATA_INTEGRACAO AS DATE) <= '{ filtrosPesquisa.DataFinalIntegracao.ToString(pattern) }'");

            if (filtrosPesquisa.CodigoTipoOperacao > 0)
                where.Append($" and Carga.TOP_CODIGO = {filtrosPesquisa.CodigoTipoOperacao}");

            if (filtrosPesquisa.Situacao.HasValue)
                where.Append($" and CargaIntegracao.INT_SITUACAO_INTEGRACAO = {filtrosPesquisa.Situacao.Value.ToString("d")}");
            ...
            if (filtrosPesquisa.CodigoTipoIntegracao > 0)
                where.Append($" and TipoIntegracao.TPI_TIPO = {filtrosPesquisa.CodigoTipoIntegracao}");
            ...
            if (filtrosPesquisa.CodigosFiliais.Any(codigo => codigo == -1))
            {
             ...
             if (filtrosPesquisa.SituacaoCarga != SituacaoCarga.Todas)
                where.Append($@" and Carga.CAR_SITUACAO = {(int)filtrosPesquisa.SituacaoCarga}");
