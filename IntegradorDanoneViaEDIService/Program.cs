    ...
    static class Program
    {
        ...
        public static void Start()
        {
            ...
            try
            {
                ...
                string CNPJsNovoPadrao = appSettings["AppSettings:CNPJsNovoPadrao"];
                bool validarPorCNPJNovoPadrao = bool.Parse(appSettings["AppSettings:ValidarPorCNPJNovoPadrao"]);
                string caminhoBatReiniciar = appSettings["AppSettings:CaminhoBatReiniciar"];
                int tamanhoStack = int.Parse(appSettings["AppSettings:TamanhoStack"]);
                string filialRetornoIntegracao = appSettings["AppSettings:filialRetornoIntegracao"];
                string dataInicioIntegracao = appSettings["AppSettings:DataInicioIntegracoes"];
                string dataFimIntegracao = appSettings["AppSettings:DataFimIntegracoes"];
                bool extrairXMLsPorTomador = !string.IsNullOrWhiteSpace(appSettings["AppSettings:ExtrairXMLsPorTomador"]) ? bool.Parse(appSettings["AppSettings:ExtrairXMLsPorTomador"]) : false;                

                if (extrairXMLsPorTomador)
                {
                    Servicos.EDI.StartUpExtrairXML startUpExtrairXML = new Servicos.EDI.StartUpExtrairXML(unitOfWork);
                    startUpExtrairXML.Iniciar(dataInicioIntegracao, dataFimIntegracao, tamanhoStack);
                }
                if (!string.IsNullOrWhiteSpace(filialRetornoIntegracao))
                {
                    int.TryParse(filialRetornoIntegracao, out int codigoFilial);
                    Servicos.EDI.StartupRetornoMagalog startUpRetornoMagalog = new Servicos.EDI.StartupRetornoMagalog(unitOfWork);
                    startUpRetornoMagalog.Iniciar(codigoFilial, dataInicioIntegracao, dataFimIntegracao, tamanhoStack);
                }
                else
                {
                    string cliente = appSettings["AppSettings:Cliente"];
                    if (!string.IsNullOrWhiteSpace(cliente) && cliente == "Americanas")
                    {
                        Servicos.EDI.StartupEDIAmericanas startupEDIAmericanas = new Servicos.EDI.StartupEDIAmericanas(unitOfWork);
                        startupEDIAmericanas.Iniciar(caminho, tipoArmazenamento, enderecoFTP, usuarioFTP, senhaFTP, caminhoRaizFTP, ftpPassivo, portaFTP, utilizaSFTP, StringConexaoAdmin, caminhoBatReiniciar, tamanhoStack);
                    }
                    else if (!string.IsNullOrWhiteSpace(cliente) && cliente == "Walmart")
                    {
                        string caminhoRaizRoteirizacao = appSettings["AppSettings:CaminhoRaizRoteirizacao"];
                        Servicos.EDI.StartupEDIWalmart startupEDIWalmart = new Servicos.EDI.StartupEDIWalmart(unitOfWork);
                        startupEDIWalmart.Iniciar(caminho, tipoArmazenamento, enderecoFTP, usuarioFTP, senhaFTP, caminhoRaizFTP, caminhoRaizRoteirizacao, ftpPassivo, portaFTP, utilizaSFTP, StringConexaoAdmin, caminhoBatReiniciar, tamanhoStack);
                    }
                    else if (!string.IsNullOrWhiteSpace(cliente) && cliente == "MagazineLuiza")
                    {
                        Servicos.EDI.StartupEDIMagazineLuiza startupEDIMagazineLuiza = new Servicos.EDI.StartupEDIMagazineLuiza(unitOfWork);
                        startupEDIMagazineLuiza.Iniciar(caminho, tipoArmazenamento, enderecoFTP, usuarioFTP, senhaFTP, caminhoRaizFTP, ftpPassivo, portaFTP, utilizaSFTP, StringConexaoAdmin, caminhoBatReiniciar, tamanhoStack);
                    }
                    else if (!string.IsNullOrWhiteSpace(cliente) && cliente == "MartinBrower")
                    {
                        Servicos.EDI.StartupEDIMartinBrower startupEDIMartinBrower = new Servicos.EDI.StartupEDIMartinBrower(unitOfWork);
                        startupEDIMartinBrower.Iniciar(caminho, tipoArmazenamento, enderecoFTP, usuarioFTP, senhaFTP, caminhoRaizFTP, ftpPassivo, portaFTP, utilizaSFTP, StringConexaoAdmin, caminhoBatReiniciar, tamanhoStack);
                    }
                    else
                    {
                        Servicos.EDI.StartupEDIDanone startupEDIDanone = new Servicos.EDI.StartupEDIDanone(unitOfWork);
                        startupEDIDanone.Iniciar(caminho, tipoArmazenamento, enderecoFTP, usuarioFTP, senhaFTP, caminhoRaizFTP, ftpPassivo, portaFTP, utilizaSFTP, StringConexaoAdmin, caminhoBatReiniciar, CNPJsNovoPadrao, validarPorCNPJNovoPadrao, tamanhoStack, prefixosFTP);
                    }
                }
            }
            finally
        ...
        private static void MigrarAppSettings(string diretorio)
        {
            ...
            if (!Utilidades.IO.FileStorageService.LocalStorage.Exists(jsonFilePath))
            {
                ...
                var allowedKeys = new[] { "Host", "Cliente", "CNPJsNovoPadrao", "ValidarPorCNPJNovoPadrao", "CaminhoBatReiniciar", "TamanhoStack", "filialRetornoIntegracao", "DataInicioIntegracoes", "DataFimIntegracoes", "ExtrairXMLsPorTomador", "CaminhoRaizRoteirizacao" };
