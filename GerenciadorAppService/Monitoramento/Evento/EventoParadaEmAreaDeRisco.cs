...
{
    public class EventoParadaEmAreaDeRisco : IMonitoramento
    {
        ...
        private Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta tipoAlerta = Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.ParadaEmAreaDeRisco;
        ...
        private void InicializarParametro()
        {
            ...
            parametro = repMonitoramentoEvento.BuscarParametorPorTipo(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoMonitoramentoEvento.ParadaEmAreaDeRisco);
        ...
        private void Inicializar()
        {
            ...
            ultimoAlertaVeiculo = repAlerta.BuscarUltimoAlertaVeiculo(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.ParadaEmAreaDeRisco);

            locais = repLocais.BuscarPorTipoDeLocal(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoLocal.AreaDeRisco);
        ...
        private void InserirAlerta(Dominio.Entidades.Embarcador.Logistica.Posicao posicao, Dominio.Entidades.Embarcador.Cargas.Carga carga)
        {
            ...
            catch (Exception ex)
            {
                ...
                Servicos.Log.TratarErro("Erro ao gravar EventoTempoParado: " + ex);
                throw;
            }
        }
        ...
        if (posicoes.Count == 0)
            ...
            foreach (Dominio.Entidades.Embarcador.Logistica.Monitoramento cargaMonitorada in cargasMonitoradas)
            {
                ...
                DateTime dataParada = cargaMonitorada?.PosicaoParadaEmAreaDeRisco?.DataVeiculo ?? DateTime.MinValue;
                foreach (var posicaoVeiculo in posicoesVeiculo)
                {
                    ...
                    var temRegistro = (cargaMonitorada?.PosicaoParadaEmAreaDeRisco != null);

                    if (!temRegistro)
                    {
                        cargaMonitorada.PosicaoParadaEmAreaDeRisco = posicaoVeiculo;
                    ...
                    if (temRegistro)
                    {
                        ...
                        if ((cargaMonitorada.PosicaoParadaEmAreaDeRisco != null) && (cargaMonitorada.PosicaoParadaEmAreaDeRisco.DataVeiculo < posicaoVeiculo.DataVeiculo))
                            distancia = Servicos.Embarcador.Logistica.Distancia.CalcularDistanciaMetros(posicaoVeiculo.Latitude, posicaoVeiculo.Longitude, cargaMonitorada.PosicaoParadaEmAreaDeRisco.Latitude, cargaMonitorada.PosicaoParadaEmAreaDeRisco.Longitude);

                        var atualizarVeiculoParado = true;
                        if (distancia < 0.1)
                        {
                            atualizarVeiculoParado = false;
                            //verifica tempo da parada
                            var parametroTempoMaximoParado = parametro.Tempo;
                            var dataparado = cargaMonitorada.PosicaoParadaEmAreaDeRisco.Data.AddMinutes(parametroTempoMaximoParado);
                            validarVeiculoParado = posicaoVeiculo.Data > dataparado;

                            //Verificar tempo do alerta
                            var data = dataparado.AddMinutes(parametro.Tempo);
                            if (ultimoAlertaVeiculo != null)
                                data = ultimoAlertaVeiculo.Data.AddMinutes(parametro.Tempo);
