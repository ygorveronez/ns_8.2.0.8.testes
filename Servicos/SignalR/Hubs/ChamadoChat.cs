...
namespace Servicos.SignalR.Hubs
{
	...
	//vamos usar os mesmos modelos do chat com motorista no controle entrega
	public class ChamadoChat : SignalRBase<ChamadoChat>
	{
		public ChamadoChat()
		{
		}
		...
		public void MensagemRecebida(Dominio.MSMQ.Notification notification)
		{
			...
			if (connectionId.Count > 0)
				SendToConnectionIds(connectionId, "chamadoMensagemRecebida", JsonConvert.SerializeObject(notification.Content));
		...
		{
			SendToAll("chamadoMensagemEnviada", JsonConvert.SerializeObject(notification.Content));
