...
using System.Linq;
using AdminMultisoftware.Dominio.ObjetosDeValor.Auditoria;
...
{
    public class ChamadoOcorrencia : RelatorioBase<Dominio.ObjetosDeValor.Embarcador.Chamado.FiltroPesquisaRelatorioChamado, Dominio.Relatorios.Embarcador.DataSource.Chamados.ChamadoOcorrencia.ChamadoOcorrencia>
    {
        #region Atributos

        private readonly Repositorio.Embarcador.Chamados.Chamado _repChamadoOcorrencia;

        #endregion

        #region Construtores

        public ChamadoOcorrencia(Repositorio.UnitOfWork unitOfWork, AdminMultisoftware.Dominio.Enumeradores.TipoServicoMultisoftware tipoServicoMultisoftware, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente clienteMultisoftware) : base(unitOfWork, tipoServicoMultisoftware, clienteMultisoftware)
        {
            _repChamadoOcorrencia = new Repositorio.Embarcador.Chamados.Chamado(_unitOfWork);
        }

        #endregion        
        // TODO: ToList metodo override
        protected override List<Dominio.Relatorios.Embarcador.DataSource.Chamados.ChamadoOcorrencia.ChamadoOcorrencia> ConsultarRegistros(FiltroPesquisaRelatorioChamado filtrosPesquisa, List<PropriedadeAgrupamento> propriedadesAgrupamento, Dominio.ObjetosDeValor.Embarcador.Consulta.ParametroConsulta parametrosConsulta)
        {
            return _repChamadoOcorrencia.ConsultarChamadoOcorrencia(filtrosPesquisa, propriedadesAgrupamento, parametrosConsulta.PropriedadeAgrupar, parametrosConsulta.DirecaoAgrupar, parametrosConsulta.PropriedadeOrdenar, parametrosConsulta.DirecaoOrdenar, parametrosConsulta.InicioRegistros, parametrosConsulta.LimiteRegistros).ToList();
        }

        protected override int ContarRegistros(FiltroPesquisaRelatorioChamado filtrosPesquisa, List<PropriedadeAgrupamento> propriedadesAgrupamento)
        {
            return _repChamadoOcorrencia.ContarConsultaChamadoOcorrencia(filtrosPesquisa, propriedadesAgrupamento, "", "", "", "");
        }

        protected override string ObterCaminhoPaginaRelatorio()
        {
            return "Relatorios/Chamados/ChamadoOcorrencia";
        }

        protected override List<Parametro> ObterParametros(FiltroPesquisaRelatorioChamado filtrosPesquisa, Dominio.ObjetosDeValor.Embarcador.Consulta.ParametroConsulta parametrosConsulta)
        {
        ...
        {
            if (propriedadeOrdenarOuAgrupar.Contains("DescricaoSituacao"))
                return propriedadeOrdenarOuAgrupar.Replace("Descricao", "");

            if (propriedadeOrdenarOuAgrupar.Contains("TempoTotal"))
                return propriedadeOrdenarOuAgrupar.Replace("DataCriacao", "");

            if (propriedadeOrdenarOuAgrupar.Contains("StatusAtrasoCarga"))
                return propriedadeOrdenarOuAgrupar.Replace("DiasAtraso", "");

            if (propriedadeOrdenarOuAgrupar.Contains("DescricaoAosCuidadosDo"))
                return propriedadeOrdenarOuAgrupar.Replace("Descricao", "");
