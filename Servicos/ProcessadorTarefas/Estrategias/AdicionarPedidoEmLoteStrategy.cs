...
public class AdicionarPedidoEmLoteStrategy : IStrategyQuebraRequest, IStrategyRetornoIntegracao
{
    ...
    public AdicionarPedidoEmLoteStrategy(
    ...
    {
        List<Dominio.ObjetosDeValor.WebService.Carga.CargaIntegracao> cargasIntegracao =
            contexto.RequestDoc.Dados.FromBsonDocument<List<Dominio.ObjetosDeValor.WebService.Carga.CargaIntegracao>>();

        return PreencherSubtarefas(cargasIntegracao, contexto.TarefaId);
    }
    ...
    public async Task RetornoIntegracaoTarefa(ContextoEtapa contexto, TarefaIntegracao tarefaIntegracao, CancellationToken cancellationToken)
    {
        switch (tarefaIntegracao.TipoIntegracao)
        {
            case TipoIntegracao.GrupoSC:
                await new Embarcador.Integracao.GrupoSC.IntegracaoGrupoSC(_unitOfWork, _repositorioSubtarefa, _repositorioTarefaIntegracao)
                    .IntegrarRetornoConfirmacaoPedidosAsync(contexto, tarefaIntegracao, cancellationToken);
                break;
            ...
            case TipoIntegracao.ArcelorMittal:
                await new Embarcador.Integracao.ArcelorMittal.IntegracaoArcelorMittal(_unitOfWork, _repositorioSubtarefa, _repositorioTarefaIntegracao)
                    .IntegrarRetornoAdicionarPedidoEmLoteAsync(contexto, tarefaIntegracao, cancellationToken);
                break;
        ...
        string tarefaId)
    {
        ...
        foreach (Dominio.ObjetosDeValor.WebService.Carga.CargaIntegracao cargaIntegracao in cargasIntegracao)
        {
            retorno.Add(new RequestSubtarefa
            {
                TarefaId = tarefaId,
                Dados = cargaIntegracao.ToBsonDocument(),
                Ordem = ordem++
            });
        }
