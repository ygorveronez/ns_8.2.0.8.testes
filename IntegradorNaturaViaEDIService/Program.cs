    ...
    static class Program
    {
        ...
        public static void Start()
        {
            ...
            try
            {
                ...
                int empresa = int.Parse(appSettings["AppSettings:CodigoEmpresaMultisoftware"]);
                int minutos = int.Parse(appSettings["AppSettings:MinutosParaConsultaNatura"]);
                string tipoArmazenamento = appSettings["AppSettings:TipoArmazemento"];
                string emails = appSettings["AppSettings:Emails"];                
                bool gerarNotFisPorNota = bool.Parse(appSettings["AppSettings:GerarNotFisPorNota"]);

                List<string> filiais = new List<string>();
                string[] filiaisSplit = appSettings["AppSettings:FiliaisNatura"].Split(',');
        ...
        private static void MigrarAppSettings(string diretorio)
        {
            ...
            if (!Utilidades.IO.FileStorageService.LocalStorage.Exists(jsonFilePath))
            {
                ...
                var allowedKeys = new[] { "Host", "CodigoEmpresaMultisoftware", "MinutosParaConsultaNatura", "TipoArmazemento", "GerarNotFisPorNota", "FiliaisNatura" };
