    ...
    class SessionHelper
    {
...
#if DEBUG
                    string caminhoScript = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "schema.sql");
        ...
        private static MemoryStream ObterSream()
        {
            ...
            try
            {
                ...
                string pathBin = pathApp.Contains(@"\bin\") ? pathApp : System.IO.Path.Combine(pathApp, "bin");
                string pathCache = System.IO.Path.Combine(pathBin, "cacheNHibernate.cache");

                if (VefificarDataDominioAlterado(pathBin))
                {
                    if (System.IO.File.Exists(pathCache))
                        System.IO.File.Delete(pathCache);
                }
        ...
        {
            string pathContoleCache = System.IO.Path.Combine(path, "cacheNHibernate.txt");
            string fileDominio = System.IO.Path.Combine(path, "Dominio.dll");

            DateTime dataDominio = File.GetLastWriteTime(fileDominio);

            System.IO.File.WriteAllText(pathContoleCache, dataDominio.ToString());
        }
        private static bool VefificarDataDominioAlterado(string path)
        {
            string fileContoleCache = System.IO.Path.Combine(path, "cacheNHibernate.txt");
            string fileDominio = System.IO.Path.Combine(path, "Dominio.dll");

            DateTime dataDominio = File.GetLastWriteTime(fileDominio);

            string dataTexto = string.Empty;
            if (File.Exists(fileContoleCache))
                dataTexto = System.IO.File.ReadAllText(fileContoleCache);
