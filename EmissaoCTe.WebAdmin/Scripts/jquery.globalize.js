        ...
        */
        ...
        customNumberFormat = function (value, format, formatInfo) {
            ...
            function numberOfSeparators(startPosition, endPosition, formatInfo) {
                ...
                while ((i += formatInfo.groupSizes[groupIndex]) < endPosition) {
                    if (i >= startPosition)
                        ret++;
                    if (formatInfo.groupSizes.length > groupIndex + 1)
                        groupIndex++;
                }
            ...
            if (patterns.length === 3)
                ...
                zeroPattern = patterns[0];

            // drill down to correct format
            ...
            // separate the fluff (textualSubpatterns) from the good stuff (subpatterns)
            // The formatting string can start with either, so keep a reference of what it
            // starts bit (firstSubpatternContainer)
            var len = pattern.length,
			firstSubpatternContainer,
			prevSubpatternContainer,
			subpatterns = [],
			textualSubpatterns = [],
			subpatternContainer,
			subpattern = '',
            ...
            for (i = 0; i < len; i++) {
                ...
                if (!isEscaped && pattern.charAt(i) === ',') {
                    ...
                    if (!firstSubpatternContainer)
                        firstSubpatternContainer = prevSubpatternContainer = subpatternContainer;

                    if (subpatternContainer === prevSubpatternContainer)
                        subpattern += pattern.charAt(i);
                    else {
                        prevSubpatternContainer.push(subpattern);
                        subpattern = pattern.charAt(i);
                    }
            ...
            if (!subpatterns.length)
                return textualSubpatterns.length ? textualSubpatterns[0] : '';
            ...
            if (intDecPatterns.length > 1) {
                decimalsLengthRange = [intDecPatterns[1].lastIndexOf('0') + 1];
                decimalsLengthRange.push(Math.max(decimalsLengthRange[0],
				intDecPatterns[1].lastIndexOf('#') + 1));
            } else
            ...
            if (~(intMinLength = intDecPatterns[0].indexOf('0')))
                ...
                intMinLength = rounded > -1 && rounded < 1 ? 0 : 1;
            ...
            if (~decimalsSeparatorIndex && intDecPatterns.length > 1 && intDecPatterns[1] !== '') {
                ...
                else if (roundedDecimals.length < decimalsLengthRange[0])
                    decimalPlaces = decimalsLengthRange[0];
                else
            ...
            for (i = subpatterns.length - 1; i > 0; i--) {
                ...
                if (i <= decimalsSeparatorIndex || ! ~decimalsSeparatorIndex) { // int part
                    ...
                    if (hasGroupsSeparator)
                        sliceLength += numberOfSeparators(curIntLength, curIntLength += sliceLength, formatInfo);
            ...
            while (curSubpatterns.length) {
                ...
                if (curSubpatterns === textualSubpatterns)
                    ...
                    curSubpatterns = textualSubpatterns;
            }
        ...
        formatNumber = function (value, format, culture) {
            ...
            if (/^(?:D|N|C|P)\d*$/i.test(format))
                precision = format.length > 1 ? parseInt(format.slice(1), 10) : -1;
            else
                return customNumberFormat(value, format, nf);
