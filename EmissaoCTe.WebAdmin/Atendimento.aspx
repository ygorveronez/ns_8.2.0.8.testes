<%@ Page Title="Atendimento" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Atendimento.aspx.cs" Inherits="EmissaoCTe.WebAdmin.Atendimento" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script defer="defer" type="text/javascript">
        ...
        $(document).ready(function () {
            CarregarConsultaDeAtendimentos("default-search", "default-search", RetornoConsultaAtendimento, true)

            CarregarConsultaDeEmpresas("btnEmpresa", "btnEmpresa", "A", RetornoConsultaTransportador, true);
            CarregarConsultaDeTipoAtendimento("btnTipo", "btnTipo", RetornoConsultaTipo, true);
        ...
        function BuscarTipoAtendimentoPadrao() {
            executarRest("/Atendimento/TipoAtendimentoPadrao?callback=?", {}, function (r) {
                if (r.Sucesso) {
        ...
        function RetornoConsultaAtendimento(atendimento) {
            LimparCampos();
            executarRest("/Atendimento/ObterDetalhes?callback=?", atendimento, function (r) {
                if (r.Sucesso) {
                    var dados = r.Objeto;
                    ...
                    $("#selSistema").val(dados.Sistema);
                    $("#selSituacao").val(dados.Situacao);
                    $("#selSatisfacao").val(dados.Satisfacao);
                    $("#selTipoContato").val(dados.TipoContato);
                    $("#txtContato").val(dados.Contato);
                    $("#chkNecessitouAuxilio").attr("checked", dados.NecessitouAuxilio);
        ...
        function LimparCampos() {
            ...
            $("#txtContato").val("");
            $("#chkNecessitouAuxilio").attr("checked", false);
        ...
        function Salvar() {
            if (ValidarCampos()) {
                var dados = {
                    ...
                    NecessitouAuxilio: $("#chkNecessitouAuxilio")[0].checked,
                };

                executarRest("/Atendimento/Salvar?callback=?", dados, function (r) {
                    if (r.Sucesso) {
                        if (uploader.files.length > 0) {
                            uploader.settings.url = 'Atendimento/Anexos?callback=?&Codigo=' + r.Objeto.Codigo;
        ...
        function Excluir() {
            jConfirm("Tem certeza que deseja excluir", "Confirmar exclusão", function (r) {
                if (r) {
                    executarRest("/Atendimento/Excluir?callback=?", Impressora, function (r) {
                        if (r.Sucesso) {
        ...
        function DownloadArquivo(id) {
            $("#ifrDownload").attr("src", "Atendimento/DownloadAnexo?Codigo=" + id);
        }
        ...
        function DeletarArquivo(id) {
            executarRest("/Atendimento/AnexoExcluir?callback=?", { Codigo: id }, function (r) {
                if (r.Sucesso) {
                    ExibirMensagemSucesso("Excluído com sucesso.", "Sucesso");
                } else {
                    ExibirMensagemErro(r.Erro, "Atenção");
                        ...
                        <div class="fieldzao">
                            <div class="field fieldois">
                                <div class="checkbox" style="margin-top: 22px;">
                                    <input type="checkbox" id="chkNecessitouAuxilio" />
                                    <label>Necessitou Auxílio</label>
                                </div>
                            </div>
                        </div>
