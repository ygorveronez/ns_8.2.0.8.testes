<%@ Page Title="CT-es Integrados" Language="C#" MasterPageFile="Site.Master" AutoEventWireup="true" CodeBehind="CTesIntegrados.aspx.cs" Inherits="EmissaoCTe.WebAdmin.CTesIntegrados" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function Carregar() {
            CriarGridView("/IntegracaoCTeRecebimento/Consultar?callback=?", { inicioRegistros: 0, Status: $("#selStatus").val(), NumeroCTe: $("#txtNumeroFiltro").val(), DataEmissao: $("#txtDataEmissaoFiltro").val(), Empresa: $("#txtEmpresaFiltro").val(), Usuario: $("#txtUsuarioFiltro").val() }, "tbl_ctes_table", "tbl_ctes", "tbl_paginacao_ctes", [{ Descricao: "DACTE", Evento: DownloadDACTE }, { Descricao: "Alocar", Evento: Alocar }, { Descricao: "Aprovar", Evento: Aprovar }, { Descricao: "Reprovar", Evento: Reprovar }, { Descricao: "Acessar", Evento: AcessarSistema }, { Descricao: "E-mail", Evento: EnviarEmail }], [0, 1, 2]);
        }
        ...
        function EnviarEmail(doc) {
            ...
            executarRest("/ConhecimentoDeTransporteEletronico/RegerarDACTE?callback=?", solicitacao, function (r) {
                if (r.Sucesso) {
                    jAlert("Envio de e-mail solicitado com com sucesso.", "Atenção!");
                } else {
                    jAlert(r.Erro, "Atenção!");
                }
            });
        ...
        function DownloadDACTE(doc) {
            $("#ifrDownload").attr("src", "ConhecimentoDeTransporteEletronico/DownloadDacte?CodigoCTe=" + doc.data.CodigoCTe + "&CodigoEmpresa=" + doc.data.CodigoEmpresa);
        }

        function DownloadXML(doc) {
            $("#ifrDownload").attr("src", "ConhecimentoDeTransporteEletronico/DownloadXML?CodigoCTe=" + doc.data.CodigoCTe + "&CodigoEmpresa=" + doc.data.CodigoEmpresa);
        }
        ...
        function Alocar(doc) {
            ...
            executarRest("/IntegracaoCTeRecebimento/Alocar?callback=?", solicitacao, function (r) {
                if (r.Sucesso) {
                    Carregar();
                } else {
                    Carregar();
        ...
        function Aprovar(doc) {
            var solicitacao = {
                Codigo: doc.data.Codigo
            }
            executarRest("/IntegracaoCTeRecebimento/Aprovar?callback=?", solicitacao, function (r) {
                if (r.Sucesso) {
                    Carregar();
                } else {
                    Carregar();
        ...
        function Reprovar(doc) {
            var solicitacao = {
                Codigo: doc.data.Codigo
            }
            executarRest("/IntegracaoCTeRecebimento/Reprovar?callback=?", solicitacao, function (r) {
                if (r.Sucesso) {
                    Carregar();
                } else {
                    Carregar();
                ...
                <div class="form">
                    <div class="fields" style="margin-top: 15px;">
                        ...
                        <div class="field fieldum">
                            ...
                            <div class="input">
                                <select id="selStatus" class="select">
                                    ...
                                    <option value="1">Alocados</option>
                        ...
                        <div class="field fielddois">
                            ...
                            <div class="input">
                                <input type="text" id="txtUsuarioFiltro" />
                            </div>
                        </div>
