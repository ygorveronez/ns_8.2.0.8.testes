...
{
    public class CONEMBController : ApiController
    {
        #region Variáveis Globais

        private Dominio.ObjetosDeValor.PaginaUsuario Permissao()
        {
            return (from obj in this.BuscarPaginasUsuario() where obj.Pagina.Formulario.ToLower().Equals("conemb.aspx") select obj).FirstOrDefault();
        }
        ...
        public ActionResult Consultar()
        {
            ...
            try
            {
                ...
                List<Dominio.ObjetosDeValor.ConsultaCTe> ctes = repCTe.ConsultarPorDuplicata(this.EmpresaUsuario.Codigo, dataInicial, dataFinal, 0, 0, placa, string.Empty, cpfCnpjRemetente, string.Empty, new string[] { "A" }, Dominio.Enumeradores.TipoCTE.Todos, this.EmpresaUsuario.TipoAmbiente, this.Usuario.Series.Where(o => o.Tipo == Dominio.Enumeradores.TipoSerie.CTe).Select(o => o.Codigo).ToArray(), 0, string.Empty, string.Empty, inicioRegistros, 50, 0, true);
                int countCTes = repCTe.ContarConsultaPorDuplicata(this.EmpresaUsuario.Codigo, dataInicial, dataFinal, 0, 0, placa, string.Empty, cpfCnpjRemetente, string.Empty, new string[] { "A" }, Dominio.Enumeradores.TipoCTE.Todos, this.EmpresaUsuario.TipoAmbiente, this.Usuario.Series.Where(o => o.Tipo == Dominio.Enumeradores.TipoSerie.CTe).Select(o => o.Codigo).ToArray(), 0, string.Empty, string.Empty, 0, true);
        ...
        public ActionResult SelecionarTodos()
        {
            ...
            try
            {
                ...
                List<Dominio.ObjetosDeValor.ConsultaCTe> ctes = repCTe.ConsultarPorDuplicata(this.EmpresaUsuario.Codigo, dataInicial, dataFinal, 0, 0, placa, string.Empty, cpfCnpjRemetente, string.Empty, new string[] { "A" }, Dominio.Enumeradores.TipoCTE.Todos, this.EmpresaUsuario.TipoAmbiente, this.Usuario.Series.Where(o => o.Tipo == Dominio.Enumeradores.TipoSerie.CTe).Select(o => o.Codigo).ToArray(), 0, string.Empty, string.Empty, 0, countCTes, 0);
        ...
        public ActionResult PreparaGerarao()
        {
            ...
            string nomeSessao = "CONEMB" + DateTime.Now.ToString("HHmmss");
        ...
        public ActionResult Gerar()
        {
            ...
            try
            {
                ...
                Dominio.Entidades.LayoutEDI layout = repLayout.BuscarPorCodigoETipo(codigoLayout, Dominio.Enumeradores.TipoLayoutEDI.CONEMB);

                if (layout == null)
                    return Json<bool>(false, false, "Layout do arquivo não encontrado.");

                System.IO.MemoryStream arquivo = this.GerarCONEMB(cpfCnpjRemetente, dataInicial, dataFinal, codigoVeiculo, layout, codigosCTes, unitOfWork);
                Servicos.GeracaoEDI svcEDI = new Servicos.GeracaoEDI(unitOfWork);
                string nomeArquivo = string.Empty;
                if (!string.IsNullOrWhiteSpace(layout.Nomenclatura))
                    nomeArquivo = svcEDI.ObterNomenclaturaLayoutEDI(layout.Nomenclatura, this.EmpresaUsuario, null, string.Empty, DateTime.Now);

                if (string.IsNullOrWhiteSpace(nomeArquivo))
                    nomeArquivo = string.Concat("CONEMB_", dataInicial.ToString("ddMMyy"), "-", dataFinal.ToString("ddMMyy"));
            ...
            {
                Servicos.Log.TratarErro(ex);
                return Json<bool>(false, false, "Ocorreu uma falha ao gerar o arquivo CONEMB.");
            }
            finally
            {
                unitOfWork.Dispose();
        ...
        private System.IO.MemoryStream GerarCONEMB(string cpfCnpjRemetente, DateTime dataInicial, DateTime dataFinal, int codigoVeiculo, Dominio.Entidades.LayoutEDI layout, List<int> codigosCTes, Repositorio.UnitOfWork unitOfWork)
        {
            Servicos.GeracaoEDI svcEDI = null;

            if (string.IsNullOrWhiteSpace(cpfCnpjRemetente))
            {
                svcEDI = new Servicos.GeracaoEDI(unitOfWork, layout, this.EmpresaUsuario, dataInicial, dataFinal, codigoVeiculo, false, codigosCTes, 0, null, null);
                return svcEDI.GerarLote();
            }
            else
            {
                svcEDI = new Servicos.GeracaoEDI(unitOfWork, layout, this.EmpresaUsuario, cpfCnpjRemetente, string.Empty, dataInicial, dataFinal, codigoVeiculo, false, codigosCTes, 0, null, null);
