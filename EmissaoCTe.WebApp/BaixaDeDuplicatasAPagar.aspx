<%@ Page Title="Baixa de Duplicatas a Pagar" Language="C#" MasterPageFile="Site.Master" AutoEventWireup="true" CodeBehind="BaixaDeDuplicatasAPagar.aspx.cs" Inherits="EmissaoCTe.WebApp.BaixaDeDuplicatasAPagar" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            CarregarConsultaDeDocumentosDeEntrada("btnBuscarDocumentoEntrada", "btnBuscarDocumentoEntrada", "1", RetornoConsultaDocumentoEntrada, true, false);

            $("#txtCliente").keydown(function (e) {
                if (e.which != 9 && e.which != 16) {
                    if (e.which == 8 || e.which == 46) {
                        $(this).val("");
                        $("body").data("codigoCliente", null);
                    } else {
                        e.preventDefault();
                    }
                }
            ...
            $("#txtDocumentoEntrada").keydown(function (e) {
                if (e.which != 9 && e.which != 16) {
                    if (e.which == 8 || e.which == 46) {
                        $(this).val("");
                        $("body").data("codigoDocumentoEntrada", null);
                    } else {
                        e.preventDefault();
                    }
                }
        ...
        function RetornoConsultaCliente(cliente) {
            ...
            $("body").data("codigoCliente", cliente.CPFCNPJ);
        }

        function RetornoConsultaDocumentoEntrada(documento) {
            $("#txtDocumentoEntrada").val(documento.Numero + " (" + documento.Fornecedor + ")");
            $("body").data("codigoDocumentoEntrada", documento.Codigo);
        }

        function LimparCampos() {
            $("body").data("codigoCliente", null);
            $("body").data("codigoDocumentoEntrada", null);
            $("body").data("duplicatas", null);
            $("#txtDocumentoEntrada").val("");
            $("#txtCliente").val("");
            $("#txtDataInicial").val("");
            $("#txtDataFinal").val("");
            $("#txtObservacao").val("");
            $("#selStatus").val($("#selStatus option:first").val());
            $("body").data("status", $("#selStatus").val());
        ...
        function AtualizarGridDuplicatas() {
            var duplicatas = $("body").data("duplicatas") == null ? new Array() : $("body").data("duplicatas");
            if ($("body").data("status") != $("#selStatus").val() && duplicatas.length > 0) {
        ...
        function CarregarInformacoesDuplicatas() {
            ...
            CriarGridView("/BaixaDuplicatasAPagar/Consultar?callback=?", dados, "tbl_duplicatas_table", "tbl_duplicatas", "tbl_paginacao_duplicatas", colunas, [0], null);
        }
        ...
        function ObterFiltros() {
            var filtros = {
                ...
                CodigoDocumentoEntrada: $("body").data("codigoDocumentoEntrada"),
                CpfCnpjCliente: $("body").data("codigoCliente")
            };
        ...
        function AdicionarDuplicata(duplicata) {
            ...
            $("body").data("duplicatas", duplicatas);
        ...
        function AdicionarTodasDuplicatas() {
            if ($("#selStatus").val() == "0") {
                executarRest("/BaixaDuplicatasAPagar/Consultar?callback=?", ObterFiltros(), function (r) {
                    if (r.Sucesso) {
        ...
        function ReverterDuplicata(duplicata) {
            jConfirm("Deseja realmente estornar a duplicata <b>" + duplicata.data.Numero + "</b>?", "Atenção", function (r) {
                if (r) {
                    executarRest("/BaixaDuplicatasAPagar/ReverterDuplicata?callback=?", { Codigo: duplicata.data.Codigo }, function (r) {
                        if (r.Sucesso) {
        ...
        function Salvar() {
            ...
            if (duplicatas.length > 0) {
                ...
                executarRest("/BaixaDuplicatasAPagar/BaixarDuplicatas?callback=?", dados, function (r) {
                    if (r.Sucesso) {
                        ExibirMensagemSucesso("Dados salvos com sucesso!", "Sucesso");
                        LimparCampos();
                    } else {
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    <div class="page-header">
        <h2>Baixa de Duplicatas a Pagar
        </h2>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12 col-md-8 col-lg-9">
            <div class="input-group">
                <span class="input-group-addon">Doc. Entrada:
                </span>
                <input type="text" id="txtDocumentoEntrada" class="form-control" />
                <span class="input-group-btn">
                    <button type="button" id="btnBuscarDocumentoEntrada" class="btn btn-primary">Buscar</button>
                </span>
            </div>
        </div>
    </div>
