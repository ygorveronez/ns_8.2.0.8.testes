...
var Regras = {
    ...
    required_if: function (valCampo, params, campo, linha) {
        ...
        if (!valida)
            return campo.nome + " é obrigatório quando quando " + linha[param].nome + " está preenchido com " + paramValor + ".";
    ...
    size: function (val, params, campo) {
        ...
        if (!valida && __required_helper(val))
            return campo.nome + " deve ter " + params[0] + " caractere(s).";
    ...
    numbers: function (val, params, campo) {
        ...
        if (!valida && __required_helper(val))
            return campo.nome + " deve ser apenas números.";
    ...
    digits: function (val, params, campo) {
        ...
        if (!valida && __required_helper(val))
            return campo.nome + " deve ser apenas letras.";
    ...
    cpfcnpj: function (val, params, campo) {
        ...
        if (!valida && __required_helper(val))
            return campo.nome + " informado é inválido.";
    ...
    min_length: function (val, params, campo) {
        var valida = !(val.length < params[0]);
        if (!valida)
            return campo.nome + " deve ter no mínimo " + params[0] + " caractere(s).";
    ...
    max_length: function (val, params, campo) {
        var valida = !(val.length > params[0]);
        if (!valida)
            return campo.nome + " deve ter no máximo " + params[0] + " caractere(s).";
    ...
    tipo_dono: function (val, params, campo) {
        ...
        if (!valida)
            return campo.nome + " deve ser P (Próprio) ou T (Terceiro).";
    ...
    tipo_veiculo: function (val, params, campo) {
        ...
        if (!valida)
            return campo.nome + " deve ser 0 (Tração) ou 1 (Reboque).";
    ...
    tipo_rodado: function (val, params, campo) {
        ...
        if (!valida)
            return campo.nome + " deve ser 00 (Não Aplicado); 01 (Truck); 02 (Toco); 03 (Cavalo); 04 (Van); 05 (Utilitários); 06 (Outros).";
    ...
    tipo_carroceria: function (val, params, campo) {
        ...
        if (!valida)
            return campo.nome + " deve ser 00 (Não Aplicado); 01 (Aberta); 02 (Fechada Baú); 03 (Granel); 04 (Porta Container); 05 (Sider).";
    ...
    tipo_proprietario: function (val, params, campo) {
        ...
        if (!valida)
            return campo.nome + " deve ser 0 (TAC Agregado); 1 (TAC Independente); 2 (Outros).";
