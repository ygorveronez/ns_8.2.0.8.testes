    ...
    public partial class Logon : System.Web.UI.Page
    {
        ...
        protected void btnLogar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                this._Usuario = repositorioUsuario.BuscarPorLogin(this.txtUsuario.Text, Dominio.Enumeradores.TipoAcesso.Emissao);
                ...
                {
                    this._Usuario = repositorioUsuario.BuscarPorLoginESenha(this.txtUsuario.Text, this.txtSenha.Text, Dominio.Enumeradores.TipoAcesso.Emissao);
                    ...
                    if (versao.VersaoEmissaoCTE != version)
                    {
                        versao.CodigoCliente = this._Usuario.Empresa.Codigo;
                        versao.VersaoEmissaoCTE = version;
                    ...
                    if (this._Usuario.Status.ToUpper().Equals("A") && this._Usuario.Empresa.Status.Equals("A"))
                    {
                        this._Usuario.Session = Guid.NewGuid().ToString();
                        ...
                        Session["IdUsuario"] = this._Usuario.Codigo;
                        Session["GuidUser"] = this._Usuario.Session;
                        ...
                        Response.Cookies.Add(new HttpCookie("NomeEmpresa", Server.UrlEncode(string.Concat(this._Usuario.Empresa.RazaoSocial.Substring(0, this._Usuario.Empresa.RazaoSocial.Length > 50 ? 50 : this._Usuario.Empresa.RazaoSocial.Length), this._Usuario.Empresa.RazaoSocial.Length > 50 ? "..." : ""))));

                        if (Response.Cookies.Get("TipoAmbienteEmpresa") != null)
                            Response.Cookies.Remove("TipoAmbienteEmpresa");
                        Response.Cookies.Add(new HttpCookie("TipoAmbienteEmpresa", Server.UrlEncode(this._Usuario.Empresa.TipoAmbiente == Dominio.Enumeradores.TipoAmbiente.Producao ? "PRODUÇÃO" : "HOMOLOGAÇÃO")));

                        if (Response.Cookies.Get("GuidUser") != null)
                            Response.Cookies.Remove("GuidUser");
                        Response.Cookies.Add(new HttpCookie("GuidUser", this._Usuario.Session));

                        this.SetarPermissoesUsuario(unitOfWork);

                        FormsAuthentication.SetAuthCookie(this._Usuario.Nome, true);

                        if (new Class.LogonService().VerificarStatusFinanceiro(this._Usuario, unitOfWork))
                            Page.Response.Redirect("AvisoEmpresaBloqueioFinanceiro.aspx", false);

                        else if (this._Usuario.Tipo == "M") //Caso seja um motorista
                        {
                            Repositorio.PaginaUsuario repPaginaUsuario = new Repositorio.PaginaUsuario(unitOfWork);

                            Dominio.Entidades.PaginaUsuario pagina = repPaginaUsuario.BuscarPorCaminhoPaginaEUsuario("EntregaMotorista.aspx", this._Usuario.Codigo);

                            if (pagina != null && pagina.PermissaoDeAcesso == "A")
                                Page.Response.Redirect("EntregaMotorista.aspx", false);
                            else
                                FormsAuthentication.RedirectFromLoginPage(this._Usuario.Nome, true);
                        }
                        else if (this._Usuario.AlterarSenhaAcesso)
                        {
                            Repositorio.PaginaUsuario repPaginaUsuario = new Repositorio.PaginaUsuario(unitOfWork);

                            Dominio.Entidades.PaginaUsuario pagina = repPaginaUsuario.BuscarPorCaminhoPaginaEUsuario("AlterarSenha.aspx", this._Usuario.Codigo);
                            ...
                            if (pagina != null) //&& pagina.PermissaoDeAcesso == "A"
                            {
                                ...
                                Page.Response.Redirect("AlterarSenha.aspx", false);
                            }
                        }
                        else if (Servicos.Usuario.ObrigatorioTermos(this._Usuario, unitOfWork))
                        {

                            Page.Response.Redirect("TermosDeUso.aspx", false);
                        }
                        else
                        {
                            FormsAuthentication.RedirectFromLoginPage(this._Usuario.Nome, true);
                        ...
                        //SingleSessionPreparation.CreateAndStoreSessionToken(this._Usuario);
                    }
                    else
                    {
                        this.divMsgErroLogin.Style["display"] = "";
