...
var PesquisaQuantidade = function () {
    ...
    this.CentroCarregamento = PropertyEntity({ text: "Centro de Carregamento:", issue: 320, type: types.multiplesEntities, codEntity: ko.observable(0), idBtnSearch: guid(), visible: ko.observable(true) });
    ...
    this.Situacao = PropertyEntity({ val: ko.observable(EnumTipoQuantidadeCarga.AgConfirmacaoTransportador), options: EnumTipoQuantidadeCarga.obterOpcoes(), def: EnumTipoQuantidadeCarga.AgConfirmacaoTransportador, text: "Situação:", visible: ko.observable(true) });
    this.Filial = PropertyEntity({ text: "Filial:", issue: 70, type: types.entity, type: types.multiplesEntities, codEntity: ko.observable(0), idBtnSearch: guid(), visible: ko.observable(true) });
    ...
    this.TipoOperacao = PropertyEntity({ text: "Tipo Operação:", type: types.entity, codEntity: ko.observable(0), idBtnSearch: guid(), visible: ko.observable(true) });
...
var CRUDFiltrosRelatorio = function () {
    ...
    this.BuscaAvancada = PropertyEntity({
        eventClick: function (e) {
            if (_pesquisaQuantidade.Visible.visibleFade()) {
...
function LoadQuantidadeCarga() {
    ...
    //_gridQuantidade = new GridView(_CRUDFiltrosRelatorio.Preview.idGrid, "Relatorios/Quantidade/Pesquisa", _pesquisaQuantidade);
    // Substituído por pela linha abaixo que passa os parâmetros incluindo o callback para montar o svg do monitoramento

    _gridQuantidade = new GridView(_CRUDFiltrosRelatorio.Preview.idGrid, "Relatorios/Quantidade/Pesquisa", _pesquisaQuantidade, undefined, undefined,
    ...
    _relatorioQuantidade = new RelatorioGlobal("Relatorios/Quantidade/BuscarDadosRelatorio", _gridQuantidade, function () {
        _relatorioQuantidade.loadRelatorio(function () {
            ...
            new BuscarCentrosCarregamento(_pesquisaQuantidade.CentroCarregamento);
            ...
            new BuscarTiposOperacao(_pesquisaQuantidade.TipoOperacao);
...
function GerarRelatorioPDFClick(e, sender) {
    _relatorioQuantidade.gerarRelatorio("Relatorios/Quantidade/GerarRelatorio", EnumTipoArquivoRelatorio.PDF);
}

function GerarRelatorioExcelClick(e, sender) {
    _relatorioQuantidade.gerarRelatorio("Relatorios/Quantidade/GerarRelatorio", EnumTipoArquivoRelatorio.XLS);
}
