...
namespace SGT.WebAdmin.Areas.Relatorios.Controllers.Frotas
{
    ...
    public class DespesaOrdemServicoProdutoController : BaseController
    {
		#region Construtores

		public DespesaOrdemServicoProdutoController(Conexao conexao) : base(conexao) { }

		#endregion

        Dominio.Relatorios.Embarcador.Enumeradores.CodigoControleRelatorios CodigoControleRelatorio = Dominio.Relatorios.Embarcador.Enumeradores.CodigoControleRelatorios.R303_DespesaOrdemServicoProduto;
        ...
        public async Task<IActionResult> Pesquisa(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Dominio.ObjetosDeValor.Embarcador.Frota.FiltroRelatorioDespesaOrdemServicoProduto filtrosPesquisa = ObterFiltrosPesquisa();
                Dominio.ObjetosDeValor.Embarcador.Consulta.ParametroConsulta parametrosConsulta = grid.ObterParametrosConsulta();
                List<PropriedadeAgrupamento> agrupamentos = mdlRelatorio.VerificarAgrupamentosParaConsulta(grid.header, parametrosConsulta, string.Empty);
                Servicos.Embarcador.Relatorios.Frotas.DespesaOrdemServicoProduto servicoRelatorioDespesaOrdemServicoProduto = new Servicos.Embarcador.Relatorios.Frotas.DespesaOrdemServicoProduto(unitOfWork, TipoServicoMultisoftware, Cliente);

                servicoRelatorioDespesaOrdemServicoProduto.ExecutarPesquisa(out List<Dominio.Relatorios.Embarcador.DataSource.Frota.DespesaOrdemServicoProduto> listaDespesaOrdemServicoProduto, out int totalRegistros, filtrosPesquisa, agrupamentos, parametrosConsulta);

                grid.setarQuantidadeTotal(totalRegistros);
                grid.AdicionaRows(listaDespesaOrdemServicoProduto);
        ...
        private Dominio.ObjetosDeValor.Embarcador.Frota.FiltroRelatorioDespesaOrdemServicoProduto ObterFiltrosPesquisa()
        {
            return new Dominio.ObjetosDeValor.Embarcador.Frota.FiltroRelatorioDespesaOrdemServicoProduto()
            {
        ...
        private Models.Grid.Grid GridPadrao()
        {
            ...
            grid.Prop("Produto").Nome("Produtos").Tamanho(TamanhoColunasDescritivos).Align(Models.Grid.Align.left).Visibilidade(true);
            ...
            grid.Prop("Numero").Nome("Ordem de Serviço").Tamanho(TamanhoColunasPequeno).Align(Models.Grid.Align.right).OrdAgr(true, true).Visibilidade(false);
