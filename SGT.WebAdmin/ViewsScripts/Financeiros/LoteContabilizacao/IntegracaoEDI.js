...
function LoadIntegracaoEDI(loteContabilizacao, idKnockoutIntegracaoEDI) {
    _integracaoEDI = new IntegracaoEDI();
    _integracaoEDI.LoteContabilizacao.val(loteContabilizacao.Codigo.val());
...
function ConfigurarPesquisaIntegracaoEDI() {
    ...
    _gridIntegracaoEDI = new GridView(_integracaoEDI.Pesquisar.idGrid, "LoteContabilizacaoIntegracaoEDI/Pesquisa", _integracaoEDI, menuOpcoes);

    _gridIntegracaoEDI.CarregarGrid();
}

function DownloadIntegracaoEDI(data) {
    executarDownload("LoteContabilizacaoIntegracaoEDI/Download", { Codigo: data.Codigo });
}

function ReenviarIntegracaoEDI(data) {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar o arquivo EDI?", function () {
        executarReST("LoteContabilizacaoIntegracaoEDI/Reenviar", { Codigo: data.Codigo }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
...
function ReenviarTodosIntegracaoEDI() {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar todas as integrações de EDI?", function () {
        executarReST("LoteContabilizacaoIntegracaoEDI/ReenviarTodos", { LoteContabilizacao: _integracaoEDI.LoteContabilizacao.val(), Situacao: _integracaoEDI.Situacao.val() }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
                    _gridIntegracaoEDI.CarregarGrid();
...
function ObterTotaisIntegracaoEDI() {
    executarReST("LoteContabilizacaoIntegracaoEDI/ObterTotais", { LoteContabilizacao: _integracaoEDI.LoteContabilizacao.val() }, function (r) {
        if (r.Success) {
