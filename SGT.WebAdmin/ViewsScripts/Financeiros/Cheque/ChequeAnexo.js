...
function loadAnexo() {
    _anexo = new Anexo();
    KoBindings(_anexo, "knockoutChequeAnexo");
...
function adicionarAnexoClick() {
    ...
    if (_cheque.Codigo.val() > 0)
        enviarAnexos(_cheque.Codigo.val(), [anexo]);
    else {
...
function downloadAnexoClick(registroSelecionado) {
    executarDownload("ChequeAnexo/DownloadAnexo", { Codigo: registroSelecionado.Codigo });
}
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("ChequeAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function enviarAnexos(codigo, anexos) {
    ...
    if (formData) {
        enviarArquivo("ChequeAnexo/AnexarArquivos?callback=?", { Codigo: codigo }, formData, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function isPermitirGerenciarAnexos() {
    var status = _cheque.Status.val();

    return (status !== EnumStatusCheque.Cancelado) && (status !== EnumStatusCheque.Compensado);
}
