...
function LoadChequeBaixa() {
    ...
    new BuscarCheque(_negociacaoBaixa.ChequeBaixa, adicionarCheques, _gridChequeBaixa);
...
function excluirChequeClick(chequeSelecionado) {
    executarReST("BaixaTituloPagar/ExcluirChequePorCodigo", { Codigo: chequeSelecionado.Codigo }, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data)
...
function adicionarCheques(chequesSelecionadosPesquisa) {
    ...
    executarReST("BaixaTituloPagar/AdicionarDadosCheque", dados, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data)
...
function carregarDadosChequeBaixa() {
    executarReST("BaixaTituloPagar/CarregarDadosCheque", { Codigo: _baixaTituloPagar.Codigo.val() }, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data)
...
function isPermitirGerenciarCheques() {
    return (_baixaTituloPagar.Etapa.val() === EnumEtapasBaixaTituloPagar.Iniciada || _baixaTituloPagar.Etapa.val() === EnumEtapasBaixaTituloPagar.EmNegociacao);
}
