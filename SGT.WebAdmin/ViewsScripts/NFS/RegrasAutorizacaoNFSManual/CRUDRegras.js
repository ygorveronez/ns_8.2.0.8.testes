...
function atualizarClick(e, sender) {
    SalvarRegras("RegrasAutorizacaoNFSManual/Atualizar", "Atualizado com sucesso.", e, sender);
}

function excluirClick(e, sender) {
    exibirConfirmacao("Confirmação", "Realmente deseja excluir as regras?", function () {
        ExcluirPorCodigo(_regraNFSManual, "RegrasAutorizacaoNFSManual/ExcluirPorCodigo", function (arg) {
            if (arg.Success) {
                if (arg.Data) {
...
function adicionarClick(e, sender) {
    SalvarRegras("RegrasAutorizacaoNFSManual/Adicionar", "Cadastrado com sucesso.", e, sender);
}
...
function ValidacaoGeral() {
    ...
    //-- Valida aprovadores
    if (_regraNFSManual.GridAprovadores.val().length < _regraNFSManual.NumeroAprovadores.val())
        return ["O número de aprovadores selecionados deve ser maior ou igual a " + _regraNFSManual.NumeroAprovadores.val()];
...
function SalvarRegras(url, mensagemSucesso, e, sender) {
    ...
    if (msg_regras.length > 0)
        ...
        Salvar(_regraNFSManual, url, function (arg) {
            if (arg.Success) {
                if (arg.Data) {
