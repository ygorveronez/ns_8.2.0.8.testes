...
var FilialEmissao = function () {
    ...
    this.FilialEmissao = PropertyEntity({ text: "Filial de Emissão", type: types.entity, required: ko.observable(true), codEntity: ko.observable(0), idBtnSearch: guid() });

    // Controle de regra
    this.Regras = PropertyEntity({ text: "Filial de Emissão", type: types.map, getType: typesKnockout.dynamic, def: new Array(), required: ko.observable(false), val: ko.observable(new Array()), idGrid: guid() });
    ...
    // Controle de uso
    this.UsarRegraPorFilialEmissao = PropertyEntity({ getType: typesKnockout.bool, text: "Ativar Regra de autorização por filial de emissão", val: ko.observable(false), def: false, visible: ko.observable(true) });
    this.UsarRegraPorFilialEmissao.val.subscribe(function (novoValor) {
        SincronzarRegras();
        UsarRegraPorFilialEmissao(novoValor);
    });

    this.Adicionar = PropertyEntity({ eventClick: adicionarRegraFilialEmissaoClick, type: types.event, text: Localization.Resources.Gerais.Geral.Adicionar, visible: ko.observable(true) });
    this.Atualizar = PropertyEntity({ eventClick: atualizarRegraFilialEmissaoClick, type: types.event, text:Localization.Resources.Gerais.Geral.Atualizar, visible: ko.observable(false) });
    this.Excluir = PropertyEntity({ eventClick: excluirRegraFilialEmissaoClick, type: types.event, text: Localization.Resources.Gerais.Geral.Excluir, visible: ko.observable(false) });
    this.Cancelar = PropertyEntity({ eventClick: cancelarRegraFilialEmissaoClick, type: types.event, text: Localization.Resources.Gerais.Geral.Cancelar, visible: ko.observable(false) });
}
...
function loadFilialEmissao() {
    ...
    //-- Grid Regras
    _gridRegrasFilialEmissao = new GridReordering(_configRegras.infoTable, _filialEmissao.Regras.idGrid, GeraHeadTable("Filial de Emissão"));
