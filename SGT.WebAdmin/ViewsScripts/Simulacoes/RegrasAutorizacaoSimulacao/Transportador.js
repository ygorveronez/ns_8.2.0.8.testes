...
var Transportador = function () {
    ...
    // Controle de regra
    this.Regras = PropertyEntity({ text: "Transportador", type: types.map, getType: typesKnockout.dynamic, def: new Array(), required: ko.observable(false), val: ko.observable(new Array()), idGrid: guid() });
    ...
    // Controle de uso
    this.UsarRegraPorTransportador = PropertyEntity({ getType: typesKnockout.bool, text: "Ativar Regra de autorização por transportador", val: ko.observable(false), def: false, visible: ko.observable(true) });
    this.UsarRegraPorTransportador.val.subscribe(function (novoValor) {
        SincronzarRegras();
        UsarRegraPorTransportador(novoValor);
    });

    this.Adicionar = PropertyEntity({ eventClick: adicionarRegraTransportadorClick, type: types.event, text: Localization.Resources.Gerais.Geral.Adicionar, visible: ko.observable(true) });
    this.Atualizar = PropertyEntity({ eventClick: atualizarRegraTransportadorClick, type: types.event, text: Localization.Resources.Gerais.Geral.Atualizar, visible: ko.observable(false) });
    this.Excluir = PropertyEntity({ eventClick: excluirRegraTransportadorClick, type: types.event, text: Localization.Resources.Gerais.Geral.Excluir, visible: ko.observable(false) });
    this.Cancelar = PropertyEntity({ eventClick: cancelarRegraTransportadorClick, type: types.event, text: Localization.Resources.Gerais.Geral.Cancelar, visible: ko.observable(false) });
}
...
function loadTransportador() {
    ...
    //-- Grid Regras
    _gridRegrasTransportador = new GridReordering(_configRegras.infoTable, _transportador.Regras.idGrid, GeraHeadTable("Transportador"));
