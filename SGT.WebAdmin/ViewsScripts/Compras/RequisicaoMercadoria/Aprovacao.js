...
function reprocessarRegrasClick(e, sender) {
    executarReST("RequisicaoMercadoria/ReprocessarRegras", { Codigo: _requisicaoMercadoria.Codigo.val() }, function (arg) {
        if (arg.Success) {
            if (arg.Data) {
                if (arg.Data) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Requisição está aguardando aprovação.");
                    BuscarRequisicaoPorCodigo(_requisicaoMercadoria.Codigo.val());
                }
                else
                    exibirMensagem(tipoMensagem.aviso, "Sem Regra", "Nenhuma regra para aprovar a Requisição.");
            } else {
...
function detalharAutorizacaoClick(dataRow) {
    ...
    BuscarPorCodigo(_detalheAutorizacao, "RequisicaoMercadoria/DetalhesAutorizacao", function (arg) {
        if (arg.Success) {
            if (arg.Data != null) {
...
function GridAprovadores() {
    ...
    _gridAutorizacoes = new GridView(_aprovacao.Regras.idGrid, "RequisicaoMercadoria/PesquisaAutorizacoes", _aprovacao, menuOpcoes);
...
function ListarAprovacoes(data) {
    ...
    if (data.Situacao == EnumSituacaoRequisicaoMercadoria.SemRegra) {
