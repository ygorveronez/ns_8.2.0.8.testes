...
function voltarEtapaClick() {
    ...
    executarReST("CancelamentoPagamento/VoltarEtapa", dados, function (arg) {
        if (arg.Success) {
            if (arg.Data != false) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Pagamento pendente de aprovação.");
                editarPagamento({ Codigo: _cancelamentoPagamento.Codigo.val() });
            } else {
...
function integrarClick(e, sender) {
    exibirConfirmacao("Confirmação", "Você realmente deseja integrar o lote?", function () {
        ...
        executarReST("CancelamentoPagamento/IntegrarPagamento", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data != false) {
...
function DownloadIntegracaoEDI(data) {
    executarDownload("CancelamentoPagamentoIntegracaoEDI/Download", { Codigo: data.Codigo });
}

function ReenviarIntegracaoEDI(data) {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar o arquivo EDI?", function () {
        executarReST("CancelamentoPagamentoIntegracaoEDI/Reenviar", { Codigo: data.Codigo }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
...
function CarregaIntegracao() {
    _integracao.Pagamento.val(_cancelamentoPagamento.Codigo.val());
    ObterTotaisIntegracaoEDI();
...
function ObterTotaisIntegracaoEDI() {
    executarReST("CancelamentoPagamentoIntegracaoEDI/ObterTotais", { Pagamento: _integracao.Pagamento.val() }, function (r) {
        if (r.Success) {
...
function ReenviarTodosIntegracao() {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar todas as integrações de EDI?", function () {
        executarReST("CancelamentoPagamentoIntegracaoEDI/ReenviarTodos", { Pagamento: _integracao.Pagamento.val(), Situacao: _integracao.Situacao.val() }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
                    _gridIntegracaoEDI.CarregarGrid();
...
function ConfigurarPesquisaIntegracaoEDI() {
    ...
    _gridIntegracao = new GridView(_integracao.Pesquisar.idGrid, "CancelamentoPagamentoIntegracaoEDI/Pesquisa", _integracao, menuOpcoes);
    _gridIntegracao.CarregarGrid();
}
