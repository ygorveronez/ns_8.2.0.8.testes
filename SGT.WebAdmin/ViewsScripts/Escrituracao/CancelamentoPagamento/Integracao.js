...
var IntegracaoCarga = function () {
    this.Cancelamento = PropertyEntity({ val: ko.observable(0), def: 0, getType: typesKnockout.int });
...
function DownloadIntegracaoCarga(data) {
    executarDownload("CancelamentoPagamentoIntegracao/Download", { Codigo: data.Codigo });
}

function ReenviarIntegracaoCarga(data) {
    executarReST("CancelamentoPagamentoIntegracao/Reenviar", { Codigo: data.Codigo }, function (r) {
        if (r.Success) {
...
function DownloadArquivosHistoricoIntegracaoPagamento(historicoConsulta) {
    executarDownload("CancelamentoPagamentoIntegracao/DownloadArquivosHistoricoIntegracao", { Codigo: historicoConsulta.Codigo, Integracao: _historicoIntegracaoCarga.Integracao.val() });
}


//*******MÉTODOS*******
function CarregaIntegracaoCarga() {
    _integracaoCarga.Cancelamento.val(_cancelamentoPagamento.Codigo.val());
    ObterTotaisIntegracaoCarga();
    ConfigurarPesquisaIntegracaoCarga();
}

function ObterTotaisIntegracaoCarga() {
    executarReST("CancelamentoPagamentoIntegracao/ObterTotais", { Cancelamento: _integracaoCarga.Cancelamento.val() }, function (r) {
        if (r.Success) {
...
function ReenviarTodosIntegracaoCarga() {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar todas as integrações?", function () {
        executarReST("CancelamentoPagamentoIntegracao/ReenviarTodos", { Cancelamento: _integracaoCarga.Cancelamento.val(), Situacao: _integracaoCarga.Situacao.val() }, function (r) {
            if (r.Success) {
                exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
                _gridIntegracaoCarga.CarregarGrid();
            } else {
...
function CarregarHistoricoIntegracaoCarga() {
    ...
    _gridHistoricoIntegracaoCarga = new GridView(_historicoIntegracaoCarga.Grid.id, "CancelamentoPagamentoIntegracao/PesquisaHistorico", _historicoIntegracaoCarga, menuOpcoes);
}
...
function ConfigurarPesquisaIntegracaoCarga() {
    ...
    _gridIntegracaoCarga = new GridView(_integracaoCarga.Pesquisar.idGrid, "CancelamentoPagamentoIntegracao/Pesquisa", _integracaoCarga, menuOpcoes);
    _gridIntegracaoCarga.CarregarGrid();
}