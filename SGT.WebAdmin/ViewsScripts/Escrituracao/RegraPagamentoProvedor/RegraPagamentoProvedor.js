...
var _configuracaoRegras = {
    ...
    Alcadas: [
        {
            descricao: "Diferença Valor",
            prop: "DiferencaValor",
            tipoPropriedadeAlcada: EnumTipoPropriedadeAlcada.Decimal,
            visivel: true
        },
        {
            descricao: "Diferença Valor Menor",
            prop: "DiferencaValorMenor",
            tipoPropriedadeAlcada: EnumTipoPropriedadeAlcada.Decimal,
            visivel: true
        },
        {
            descricao: "Diferença Valor Maior",
            prop: "DiferencaValorMaior",
            tipoPropriedadeAlcada: EnumTipoPropriedadeAlcada.Decimal,
            visivel: true
        }
    ]
...
var _nomeControllerRegrasAutorizacao = "RegraPagamentoProvedor";
...
var RegrasAutorizacao = function () {
    ...
    this.ValidarTodosCamposAuditoriaDocumentoProvedor = PropertyEntity({ val: ko.observable(false), getType: typesKnockout.bool, def: false, text: "Validar todos os campos da auditoria do documento do provedor", visible: ko.observable(true) });
    this.BloquearPagamentoMultiplosCTe = PropertyEntity({ val: ko.observable(false), getType: typesKnockout.bool, def: false, text: "Bloquear pagamentos multiplos CTe's", visible: ko.observable(true) });
    ...
    _configuracaoRegras.Alcadas.forEach(function (alcada) {
        ...
        switch (alcada.tipoPropriedadeAlcada) {
            ...
            case EnumTipoPropriedadeAlcada.Enum:
                this[alcada.prop] = PropertyEntity({ text: "*" + alcada.descricao + ":", type: types.map, options: alcada.opcoes });
                break;
...
function loadRegrasAutorizacao() {
    ...
    KoBindings(_regrasAutorizacao, "knockoutCadastroRegraPagamentoProvedor");
    ...
    _configuracaoRegras.Alcadas.forEach(function (alcada) {
        ...
        _regrasAutorizacao["Alcadas" + alcada.prop].val.subscribe(function () {
            renderizarGridRegras(alcada);
            atualizaListaAlcadas(alcada.prop);
        });
    });

    _pesquisaRegraAutorizacao = new PesquisaRegrasAutorizacao();
    KoBindings(_pesquisaRegraAutorizacao, "knockoutPesquisaRegraPagamentoProvedor", false, _pesquisaRegraAutorizacao.Pesquisar.id);

    HeaderAuditoria("RegraAutorizacaoDocumento", _regrasAutorizacao);

    loadGridAprovadores();

    BuscarFuncionario(_regrasAutorizacao.AdicionarAprovador, AdicionarAprovadorRetorno, _gridAprovadores);
    BuscarFuncionario(_pesquisaRegraAutorizacao.Aprovador);
...
function adicionarAlcadaClick(prop) {
    ...
    if (listaRegrasAdicionar.length == 1) {
        if (isRegraDuplicada(listaRegras, listaRegrasAdicionar[0], prop))
        ...
        listaRegrasAdicionar.forEach(function (regraAdicionar) {
            if (!isRegraDuplicada(listaRegras, regraAdicionar, prop))
...
function atualizarAlcadaClick(prop) {
    ...
    if (listaRegrasAtualizar.length == 1) {
        ...
        if (isRegraDuplicada(listaRegras, regra, prop))
            return exibirMensagem(tipoMensagem.atencao, "Regra Duplicada", "Já existe uma regra idêntica a essa.");
        ...
        listaRegrasAtualizar.forEach(function (regraAtualizar) {
            if (!isRegraDuplicada(listaRegras, regraAtualizar, prop))
...
function obterObjetoRegraSalvar() {
    var dados = {
        ...
        ValidarTodosCamposAuditoriaDocumentoProvedor: _regrasAutorizacao.ValidarTodosCamposAuditoriaDocumentoProvedor.val(),
        BloquearPagamentoMultiplosCTe: _regrasAutorizacao.BloquearPagamentoMultiplosCTe.val(),
...
function renderizarGridRegras(alcada) {
    var html = "";
    var kout = _regrasAutorizacao["Alcadas" + alcada.prop];
    var listaRegras = obterRegrasOrdenadas(kout);

    $.each(listaRegras, function (i, regra) {
        html += '<tr data-position="' + regra.Ordem + '" data-codigo="' + regra.Codigo + '" id="sort_regra_' + alcada.prop + '_' + regra.Ordem + '"><td>' + regra.Ordem + '</td>';
        ...
        switch (alcada.tipoPropriedadeAlcada) {
            ...
            case EnumTipoPropriedadeAlcada.Enum:
                var descricaoEnumerador = "";
                
                for (var i = 0; i < alcada.opcoes.length; i++) {
                    if (alcada.opcoes[i].value == regra.Valor) {
                        descricaoEnumerador = alcada.opcoes[i].text;
                        break;
                    }
                }

                html += '<td>' + descricaoEnumerador + '</td>';
                break;
        ...
        html += '<td class="text-align-center"><a href="javascript:;" onclick="editarRegraAlcadaClick(\'' + alcada.prop + '\', \'' + regra.Codigo + '\')">Editar</a></td></tr>';
    });

    alcada.grid.RecarregarGrid(html);
}
...
function isRegraDuplicada(listaRegras, regra, prop) {
    var config = obterConfiguracaoRegraPorPropriedade(prop);
    var validarEntidadeDuplicada = (config.tipoPropriedadeAlcada === EnumTipoPropriedadeAlcada.Entidade) || (config.tipoPropriedadeAlcada === EnumTipoPropriedadeAlcada.MultiplasEntidades);
    ...
    for (var i in listaRegras) {
        if (
            ...
            (
                (validarEntidadeDuplicada && listaRegras[i].Entidade.Codigo == regra.Entidade.Codigo) ||
                (!validarEntidadeDuplicada && listaRegras[i].Valor == regra.Valor)
            )
        )
            return true;
    }
...
function obterObjetoAlcada(prop) {
    ...
    switch (config.tipoPropriedadeAlcada) {
        ...
        case EnumTipoPropriedadeAlcada.Enum:
            regra.Valor = _regrasAutorizacao[prop].val();
            break;
...
function validarCamposAlcada(prop) {
    ...
    if ((config.tipoPropriedadeAlcada === EnumTipoPropriedadeAlcada.Entidade) || (config.tipoPropriedadeAlcada === EnumTipoPropriedadeAlcada.MultiplasEntidades))
        valido = ValidarCampoObrigatorioEntity(regra);
    else
        valido = ValidarCampoObrigatorioMap(regra);
