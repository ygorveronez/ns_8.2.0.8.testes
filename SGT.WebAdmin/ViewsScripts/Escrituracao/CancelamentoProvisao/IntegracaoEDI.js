...
function voltarEtapaClick() {
    var dados = {
        Codigo: _cancelamentoProvisao.Codigo.val()
    }
    executarReST("CancelamentoProvisao/VoltarEtapa", dados, function (arg) {
        if (arg.Success) {
            if (arg.Data != false) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Provisão pendente de aprovação.");
                editarCancelamentoProvisao({ Codigo: _cancelamentoProvisao.Codigo.val() });
            } else {
...
function integrarClick(e, sender) {
    exibirConfirmacao("Confirmação", "Você realmente deseja integrar o lote?", function () {
        ...
        executarReST("CancelamentoProvisao/IntegrarCancelamentoProvisao", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data != false) {
...
function DownloadIntegracaoEDI(data) {
    executarDownload("CancelamentoProvisaoIntegracaoEDI/Download", { Codigo: data.Codigo });
}

function ReenviarIntegracaoEDI(data) {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar o arquivo EDI?", function () {
        executarReST("CancelamentoProvisaoIntegracaoEDI/Reenviar", { Codigo: data.Codigo }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
...
function CarregaIntegracao() {
    _integracao.CancelamentoProvisao.val(_cancelamentoProvisao.Codigo.val());
    ObterTotaisIntegracaoEDI();
...
function ObterTotaisIntegracaoEDI() {
    executarReST("CancelamentoProvisaoIntegracaoEDI/ObterTotais", { CancelamentoProvisao: _integracao.CancelamentoProvisao.val() }, function (r) {
        if (r.Success) {
...
function ReenviarTodosIntegracao() {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar todas as integrações de EDI?", function () {
        executarReST("CancelamentoProvisaoIntegracaoEDI/ReenviarTodos", { CancelamentoProvisao: _integracao.CancelamentoProvisao.val(), Situacao: _integracao.Situacao.val() }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
                    _gridIntegracaoEDI.CarregarGrid();
...
function ConfigurarPesquisaIntegracaoEDI() {
    ...
    _gridIntegracao = new GridView(_integracao.Pesquisar.idGrid, "CancelamentoProvisaoIntegracaoEDI/Pesquisa", _integracao, menuOpcoes);
    _gridIntegracao.CarregarGrid();
}
