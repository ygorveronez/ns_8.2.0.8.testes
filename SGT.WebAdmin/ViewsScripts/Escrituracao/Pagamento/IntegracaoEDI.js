...
function loadIntegracao() {
    ...
    if (VerificaSePossuiPermissaoEspecial(EnumPermissaoPersonalizada.Carga_ReenviarIntegracoes, _PermissoesPersonalizadasPagamento))
...
function voltarEtapaClick() {
    ...
    executarReST("Pagamento/VoltarEtapa", dados, function (arg) {
        if (arg.Success) {
            if (arg.Data != false) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Pagamento pendente de aprovação.");
                editarPagamento({ Codigo: _pagamento.Codigo.val() });
            } else {
...
function integrarClick(e, sender) {
    exibirConfirmacao("Confirmação", "Você realmente deseja integrar o lote?", function () {
        ...
        executarReST("Pagamento/IntegrarPagamento", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data != false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Pagamento integrado com sucesso.");
                    editarPagamento({ Codigo: _pagamento.Codigo.val() });
                } else {
                    exibirMensagem(tipoMensagem.atencao, "Atenção", arg.Msg);
                }
...
function DownloadIntegracaoEDI(data) {
    executarDownload("PagamentoIntegracaoEDI/Download", { Codigo: data.Codigo });
}

function ReenviarIntegracaoEDI(data) {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar o arquivo EDI?", function () {
        executarReST("PagamentoIntegracaoEDI/Reenviar", { Codigo: data.Codigo }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
...
function CarregaIntegracao() {
    _integracao.Pagamento.val(_pagamento.Codigo.val());
    ObterTotaisIntegracaoEDI();
...
function ObterTotaisIntegracaoEDI() {
    executarReST("PagamentoIntegracaoEDI/ObterTotais", { Pagamento: _integracao.Pagamento.val() }, function (r) {
        if (r.Success) {
...
function ReenviarTodosIntegracao() {
    exibirConfirmacao("Atenção!", "Deseja realmente reenviar todas as integrações de EDI?", function () {
        executarReST("PagamentoIntegracaoEDI/ReenviarTodos", { Pagamento: _integracao.Pagamento.val(), Situacao: _integracao.Situacao.val() }, function (r) {
            if (r.Success) {
                if (r.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
                    _gridIntegracaoEDI.CarregarGrid();
...
function ConfigurarPesquisaIntegracaoEDI() {
    ...
    _gridIntegracao = new GridView(_integracao.Pesquisar.idGrid, "PagamentoIntegracaoEDI/Pesquisa", _integracao, menuOpcoes);
    _gridIntegracao.CarregarGrid(function () {
        if (_gridIntegracao.NumeroRegistros() > 0) {
            $("#liIntegracaoEDI").show();
            $("#liIntegracaoEDI a").tab('show');
        }
        else
...
function VisibilidadeOpcaoDownloadIntegracaoEDI(data) {
    return VerificaSePossuiPermissaoEspecial(EnumPermissaoPersonalizada.Carga_DownloadArquivoIntegracoes, _PermissoesPersonalizadasPagamento);
}

function VisibilidadeOpcaoReenviarIntegracaoEDI(data) {
    if (data.Tipo == EnumTipoIntegracao.NaoPossuiIntegracao || !VerificaSePossuiPermissaoEspecial(EnumPermissaoPersonalizada.Carga_ReenviarIntegracoes, _PermissoesPersonalizadasPagamento))
        return false;
