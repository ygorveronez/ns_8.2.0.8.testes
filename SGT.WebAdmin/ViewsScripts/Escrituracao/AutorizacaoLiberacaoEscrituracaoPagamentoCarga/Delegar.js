...
function delegarClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar a carga?", function () {
        var dados = {
            Codigo: _carga.Codigo.val(),
            UsuarioDelegado: _delegar.UsuarioDelegado.codEntity(),
        };

        executarReST("AutorizacaoLiberacaoEscrituracaoPagamentoCarga/Delegar", dados, function (retorno) {
            if (retorno.Success) {
...
function delegarSelecionadosClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar todas as cargas selecionadas?", function () {
        ...
        executarReST("AutorizacaoLiberacaoEscrituracaoPagamentoCarga/DelegarMultiplas", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    atualizarGridCargas();
