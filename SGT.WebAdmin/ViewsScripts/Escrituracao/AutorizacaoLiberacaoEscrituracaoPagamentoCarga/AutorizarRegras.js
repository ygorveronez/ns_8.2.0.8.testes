/// <reference path="AutorizacaoLiberacaoEscrituracaoPagamentoCarga.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarAlteracaoFreteCargaClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
    ...
    this.Reprocessar = PropertyEntity({ type: types.event, val: ko.observable(false), visible: ko.observable(false), eventClick: reprocessarCargaClick, text: "Reprocessar" });
}
...
function loadGridRegras() {
    var opcaoAprovar = {
        ...
        metodo: AprovarAlteracaoFreteCarga
    };
    ...
    var opcaoRejeitar = {
        ...
        metodo: rejeitarAlteracaoFreteCarga
    };
    ...
    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoLiberacaoEscrituracaoPagamentoCarga/RegrasAprovacao", knoutCarga, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoLiberacaoEscrituracaoPagamentoCarga/AprovarMultiplasRegras", { Codigo: _carga.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarAlteracaoFreteCargaClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a liberação de escrituração e pagamento da carga é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a liberação de escrituração e pagamento da carga?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoLiberacaoEscrituracaoPagamentoCarga/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function reprocessarCargaClick() {
    executarReST("AutorizacaoLiberacaoEscrituracaoPagamentoCarga/ReprocessarCarga", { Codigo: _carga.Codigo.val() }, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                if (retorno.Data.RegraReprocessada) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "A carga foi reprocessada com sucesso.");
...
function AprovarAlteracaoFreteCarga(registroSelecionado) {
    executarReST("AutorizacaoLiberacaoEscrituracaoPagamentoCarga/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarCarga() {
    BuscarPorCodigo(_carga, "AutorizacaoLiberacaoEscrituracaoPagamentoCarga/BuscarPorCodigo", function (retorno) {
        if (retorno.Success)
...
function rejeitarAlteracaoFreteCarga(registroSelecionado) {
