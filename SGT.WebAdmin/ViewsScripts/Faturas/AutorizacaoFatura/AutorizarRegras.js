/// <reference path="AutorizacaoFatura.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarFaturaClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function loadGridRegras() {
    ...
    var knoutFatura = {
        Codigo: _fatura.Codigo,
        Usuario: _fatura.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoFatura/RegrasAprovacao", knoutFatura, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoFatura/AprovarMultiplasRegras", { Codigo: _Fatura.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarFaturaClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a Fatura é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a Fatura?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoFatura/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarFatura(registroSelecionado) {
    executarReST("AutorizacaoFatura/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function rejeitarFatura(registroSelecionado) {
