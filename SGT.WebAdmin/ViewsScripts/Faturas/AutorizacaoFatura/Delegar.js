...
function delegarClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar a Fatura?", function () {
        var dados = {
            Codigo: _fatura.Codigo.val(),
            UsuarioDelegado: _delegar.UsuarioDelegado.codEntity(),
        };

        executarReST("AutorizacaoFatura/Delegar", dados, function (retorno) {
            if (retorno.Success) {
...
function delegarSelecionadosClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar todas as Faturas selecionadas?", function () {
        ...
        executarReST("AutorizacaFatura/DelegarMultiplas", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    atualizarGridFaturas();
