...
function delegarClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar a tabela de frete?", function () {
        var dados = {
            Codigo: _tabelaFrete.Codigo.val(),
            UsuarioDelegado: _delegar.UsuarioDelegado.codEntity(),
        };

        executarReST("AutorizacaoTabelaFrete/Delegar", dados, function (retorno) {
            if (retorno.Success) {
...
function delegarSelecionadosClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar todas as tabelas de frete selecionadas?", function () {
        ...
        executarReST("AutorizacaoTabelaFrete/DelegarMultiplas", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    atualizarGridTabelaFrete();
