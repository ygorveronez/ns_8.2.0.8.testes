...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarTabelaFreteClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function loadGridRegras() {
    ...
    var knoutTabelaFrete = {
        ...
        Usuario: _tabelaFrete.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoTabelaFrete/RegrasAprovacao", knoutTabelaFrete, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoTabelaFrete/AprovarMultiplasRegras", { Codigo: _tabelaFrete.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarTabelaFreteClick(dataRow) {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a tabela de frete é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a tabela de frete?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoTabelaFrete/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarTabelaFrete(registroSelecionado) {
    executarReST("AutorizacaoTabelaFrete/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function rejeitarTabelaFrete(registroSelecionado) {
