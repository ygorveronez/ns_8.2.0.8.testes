/// <reference path="ConsultaReajusteTabelaFrete.js" />
...
function loadRegras() {
    ...
    new BuscarMotivoRejeicaoAjusteFrete(_autorizacao.MotivoRejeicao);
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        var dados = {
            Codigo: _ajusteTabela.Codigo.val()
        };

        executarReST("ConsultaReajusteTabelaFrete/AprovarMultiplasRegras", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data !== false) {
...
function rejeitarAvariaClick(dataRow) {
    ...
    //    return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar o ajuste é necessário informar ao mínimo 20 caracteres no motivo.");



    if (_autorizacao.MotivoRejeicao.codEntity() == 0)
        return exibirMensagem(tipoMensagem.aviso, "Motivo", "O campo motivo é obrigatório.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar o ajuste?", function () {
        var dados = {
            Codigo: _regraAvaria.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
            MotivoRejeicao: _autorizacao.MotivoRejeicao.codEntity()
        };

        executarReST("ConsultaReajusteTabelaFrete/Rejeitar", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    AtualizarGridRegras();
...
function RejeitarAvaria(dataRow) {
    ...
    _autorizacao.MotivoRejeicao.visible(true);
    _autorizacao.Rejeitar.visible(true);
}

function AprovarAvaria(dataRow) {
    executarReST("ConsultaReajusteTabelaFrete/Aprovar", dataRow, function (arg) {
        if (arg.Success) {
            if (arg.Data !== false) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function limparRejeicao() {
    ...
    _autorizacao.MotivoRejeicao.visible(false);
...
function GridRegras() {
    ...
    // Grid
    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "ConsultaReajusteTabelaFrete/RegrasAprovacao", _ajusteTabela, menuOpcoes);
}