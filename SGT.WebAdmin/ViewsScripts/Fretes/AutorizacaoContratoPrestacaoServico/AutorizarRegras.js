...
/// <reference path="AutorizacaoContratoPrestacaoServico.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarContratoPrestacaoServicoClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function loadGridRegras() {
    var opcaoAprovar = {
        ...
        metodo: aprovarContratoPrestacaoServico
    };
    ...
    var opcaoRejeitar = {
        ...
        metodo: rejeitarContratoPrestacaoServico
    };
    ...
    var knoutContratoPrestacaoServico = {
        Codigo: _contratoPrestacaoServico.Codigo,
        Usuario: _contratoPrestacaoServico.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoContratoPrestacaoServico/RegrasAprovacao", knoutContratoPrestacaoServico, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoContratoPrestacaoServico/AprovarMultiplasRegras", { Codigo: _contratoPrestacaoServico.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarContratoPrestacaoServicoClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar o contrato de prestação de serviço é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar o contrato de prestação de serviço?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoContratoPrestacaoServico/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function aprovarContratoPrestacaoServico(registroSelecionado) {
    executarReST("AutorizacaoContratoPrestacaoServico/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarContratoPrestacaoServico() {
...
function rejeitarContratoPrestacaoServico(registroSelecionado) {
