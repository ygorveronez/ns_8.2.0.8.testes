...
function atualizarClick(e, sender) {
    SalvarRegras("RegraControleReajusteFretePlanilha/Atualizar", "Atualizado com sucesso.", e, sender);
}

function excluirClick(e, sender) {
    exibirConfirmacao("Confirmação", "Realmente deseja excluir as regras?", function () {
        ExcluirPorCodigo(_regraControleReajusteFretePlanilha, "RegraControleReajusteFretePlanilha/ExcluirPorCodigo", function (arg) {
            if (arg.Success) {
                if (arg.Data) {
...
function adicionarClick(e, sender) {
    SalvarRegras("RegraControleReajusteFretePlanilha/Adicionar", "Cadastrado com sucesso.", e, sender);
}
...
function ValidacaoGeral() {
    ...
    //-- Valida aprovadores
    if (_gridAprovadores.BuscarRegistros().length < _regraControleReajusteFretePlanilha.NumeroAprovadores.val())
        return ["O número de aprovadores selecionados deve ser maior ou igual a " + _regraControleReajusteFretePlanilha.NumeroAprovadores.val()];


    //-- Valida as regras
    if (_tipoOperacao.Alcadas.required() && _tipoOperacao.Alcadas.val().length == 0)
...
function SalvarRegras(url, mensagemSucesso, e, sender) {
    ...
    if (msg_regras.length > 0)
        ...
        Salvar(_regraControleReajusteFretePlanilha, url, function (arg) {
            if (arg.Success) {
                if (arg.Data) {
