...
var TipoOperacao = function () {
    ...
    // Controle de regra
    this.Alcadas = PropertyEntity({ text: "Tipo de Operação", type: types.map, getType: typesKnockout.dynamic, def: new Array(), required: ko.observable(false), val: ko.observable(new Array()), idGrid: guid() });
...
function UsarRegraPorTipoOperacao(usarRegra) {
    ...
    _tipoOperacao.Alcadas.required(usarRegra);
}

function loadTipoOperacao() {
    _tipoOperacao = new TipoOperacao();
    KoBindings(_tipoOperacao, "knockoutAlcadaTipoOperacao");

    //-- Busca
    new BuscarTiposOperacao(_tipoOperacao.TipoOperacao);

    //-- Grid Regras
    _gridRegrasTipoOperacao = new GridReordering(_configRegras.infoTable, _tipoOperacao.Alcadas.idGrid, GeraHeadTable("TipoOperacao"));
    _gridRegrasTipoOperacao.CarregarGrid();
    $("#" + _tipoOperacao.Alcadas.idGrid).on('sortstop', function () {
        LinhasReordenadasRegraControleReajusteFretePlanilha(_tipoOperacao);
    });
}

function editarRegraTipoOperacaoClick(codigo) {
    // Buscar todas regras
    var listaRegras = _tipoOperacao.Alcadas.val();
    var regra = null;
...
function adicionarRegraTipoOperacaoClick() {
    ...
    _tipoOperacao.Alcadas.val(listaRegras);
