...
/// <reference path="SimuladorFretePedido.js" />
/// <reference path="SimuladorFretes.js" />
/// <reference path="SimuladorFreteTransporte.js" />
...
function ValidarSeMudouOrdemManualmente() {
    return reordenarPosicoesEntregasSimuladorFrete();
}
...
function renderizarGoogleMapsSimuladorFrete(reprocessar) {
    ...
    var ultimoPonto = 0;
    if (_roteirizadorSimuladorFrete.TipoUltimoPontoRoteirizacao.val() == EnumTipoUltimoPontoRoteirizacao.AteOrigem || _roteirizadorSimuladorFrete.TipoUltimoPontoRoteirizacao.val() == EnumTipoUltimoPontoRoteirizacao.Retornando)
...
function loadGoogleMapa() {
    ...
    _mapRoteirizador = new google.maps.Map(document.getElementById(_roteirizadorSimuladorFrete.Map.id), {
        zoom: 4,
        center: latlng,
        scaleControl: true,
...
function gerarRoteirizacaoGoogleMaps() {
    ...
    if (_roteirizadorSimuladorFrete.TipoUltimoPontoRoteirizacao.val() == EnumTipoUltimoPontoRoteirizacao.AteOrigem) 
        latLngDestino = { lat: parseFloat(_Pontos[0].coordenadas.latitude), lng: parseFloat(_Pontos[0].coordenadas.longitude) };
...
function setarDirections(latLngOrigem, latLngDestino, waypts, indexUltimoPonto, otimizar, preecherRetorno, limite, responseArray) {
    ...
    _directionsServiceRoteirizador.route({
        ...
        if (status === google.maps.DirectionsStatus.OK) {
            ...
            if (limite < waypts.length) {
                ...
                if (!otimizar) {
                    ...
                    for (var j = 0; j < responseArray.length; j++) {
                        ...
                        directionsDisplayRoteirizador.setPanel(document.getElementById(_roteirizadorSimuladorFrete.Narativa.id));
            ...
            exibirMensagem(tipoMensagem.aviso, "Erro ao roterizar", msg);
            finalizarControleManualRequisicao();
        }
    });
}
...
function MensagemDirectionsStatus(status) {
    switch (status) {
        case google.maps.DirectionsStatus.NOT_FOUND: return "O servidor (API) não conseguiu resolver algum dos pontos informados na rota.";
        case google.maps.DirectionsStatus.ZERO_RESULTS: return "Não foi encontrada nenhuma rota entre a origem e o destino. Verifique a os pontos que compõe o SimuladorFrete.";
        case google.maps.DirectionsStatus.MAX_WAYPOINTS_EXCEEDED: return "Quantidade de pontos para roteirização foi excedida.";
        case google.maps.DirectionsStatus.MAX_ROUTE_LENGTH_EXCEEDED: return "A rota é muito longa ou complexa e não pode ser processada pelo servidor (API).";
        case google.maps.DirectionsStatus.INVALID_REQUEST: return "A rota é inválida para processamento.";
        case google.maps.DirectionsStatus.OVER_QUERY_LIMIT: return "A página ultrapassou o limite de requisições dentro do perído permitido.";
        case google.maps.DirectionsStatus.REQUEST_DENIED: return "A página está sem permissão para usar o serviço.";
        default: return "O servidor retornou uma situação inesperada.";
    }
}
...
function computeTotalDistance(responseArray, res) {
    ...
    _roteirizadorSimuladorFrete.Distancia.val(Globalize.format(total / 1000, "n2") + " km");
}
