...
function loadAnexo() {
    _anexo = new Anexo();
    KoBindings(_anexo, "knockoutLicitacaoAnexo");
...
function adicionarAnexoClick() {
    ...
    if (_licitacao.Codigo.val() > 0)
        enviarAnexos(_licitacao.Codigo.val(), [anexo]);
    else {
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("LicitacaoAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function enviarAnexos(codigo, anexos) {
    ...
    if (formData) {
        enviarArquivo("LicitacaoAnexo/AnexarArquivos?callback=?", { Codigo: codigo }, formData, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
