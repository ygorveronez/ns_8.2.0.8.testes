...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarTaxaDescargaClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
    ...
    this.Reprocessar = PropertyEntity({ type: types.event, val: ko.observable(false), visible: ko.observable(false), eventClick: reprocessarTaxaDescargaClick, text: "Reprocessar" });
}
...
function loadGridRegras() {
    ...
    var knoutTaxaDescarga = {
        Codigo: _taxaDescarga.Codigo,
        Usuario: _taxaDescarga.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoTaxaDescarga/RegrasAprovacao", knoutTaxaDescarga, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoTaxaDescarga/AprovarMultiplasRegras", { Codigo: _taxaDescarga.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarTaxaDescargaClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a Taxa de Descarga é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a Taxa de Descarga?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoTaxaDescarga/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function reprocessarTaxaDescargaClick() {
    executarReST("AutorizacaoTaxaDescarga/ReprocessarTaxaDescarga", { Codigo: _taxaDescarga.Codigo.val() }, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                if (retorno.Data.RegraReprocessada) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "A taxa descarga foi reprocessada com sucesso.");
...
function AprovarTaxaDescarga(registroSelecionado) {
    executarReST("AutorizacaoTaxaDescarga/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarGridRegras(callback) {
    _gridRegras.CarregarGrid(function (retorno) {
        ...
        retorno.data.forEach(function (autorizacao) {
            ...
            if (autorizacao.PodeAprovar && (_taxaDescarga.Situacao.val() == EnumSituacaoAjusteConfiguracaoDescargaCliente.AgAprovacao))
...
function atualizarTaxaDescarga() {
    BuscarPorCodigo(_taxaDescarga, "AutorizacaoTaxaDescarga/BuscarPorCodigo", function (retorno) {
        if (retorno.Success)
            controlarExibicaoAbaDelegar(retorno.Data.Situacao === EnumSituacaoAjusteConfiguracaoDescargaCliente.AgAprovacao);
        else
            exibirMensagem(tipoMensagem.falha, "falha", retorno.Msg);
    }, null);
}

function rejeitarTaxaDescarga(registroSelecionado) {
