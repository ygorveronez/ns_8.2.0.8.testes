/// <reference path="ControlePallet.js" />
...
function loadAnexos() {
    _anexo = new Anexo();
    KoBindings(_anexo, "knockoutAnexos");
...
function adicionarAnexoClick() {
    ...
    if (_anexo.CodigoMovimentacao.val() > 0)
        enviarAnexos(_anexo.CodigoMovimentacao.val(), [anexo]);
...
function downloadAnexoClick(registroSelecionado) {
    ...
    executarDownload("ControlePalletAnexo/DownloadAnexo", dados);
}
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("ControlePalletAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function enviarAnexos(codigo, anexos) {
    ...
    enviarArquivo("ControlePalletAnexo/AnexarArquivos", { Codigo: codigo }, formData, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data)
