...
function atualizarClick(e, sender) {
    SalvarRegras("RegrasAnaliseChamados/Atualizar", "Atualizado com sucesso.", e, sender);
}

function excluirClick(e, sender) {
    exibirConfirmacao("Confirmação", "Realmente deseja excluir as regras?", function () {
        ExcluirPorCodigo(_regraChamado, "RegrasAnaliseChamados/ExcluirPorCodigo", function (arg) {
            if (arg.Success) {
                if (arg.Data) {
...
function adicionarClick(e, sender) {
    SalvarRegras("RegrasAnaliseChamados/Adicionar", "Cadastrado com sucesso.", e, sender);
}
...
function ValidacaoGeral() {
    ...
    //-- Valida aprovadores
    if (_regraChamado.GridAprovadores.val().length == 0)
        return ["Nenhum aprovador selecionado."];


    //-- Valida as regras
    // MotivoChamado
    if (_motivoChamado.Regras.required() && _motivoChamado.Regras.val().length == 0)
        msg_regras.push(msg_padrao.replace("#nomeregra#", "Motivo da Chamado"));
...
function SalvarRegras(url, mensagemSucesso, e, sender) {
    ...
    if (msg_regras.length > 0)
        ...
        Salvar(_regraChamado, url, function (arg) {
            if (arg.Success) {
                if (arg.Data) {
