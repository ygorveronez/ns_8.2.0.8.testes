...
var Transportador = function () {
    ...
    // Controle de uso
    this.RegraPorTransportador = PropertyEntity({ getType: typesKnockout.bool, text: "Ativar Regra de autorização por Transportador:", val: ko.observable(false), def: false, visible: ko.observable(true) });
    this.RegraPorTransportador.val.subscribe(function (novoValor) {
        SincronzarRegras();
        RegraPorTransportador(novoValor);
    });
...
function loadRegraTransportador() {
    ...
    $("#" + _transportador.Regras.idGrid).on('sortstop', function () {
        LinhasReordenadas(_transportador);
    });
}
