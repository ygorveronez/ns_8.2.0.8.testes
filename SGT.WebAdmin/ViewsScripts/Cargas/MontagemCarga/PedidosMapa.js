...
var PedidoMapa = function () {
    this.Localidade = PropertyEntity({ type: types.entity, codEntity: ko.observable(0), text: Localization.Resources.Cargas.MontagemCarga.Localidade, idBtnSearch: guid(), enable: ko.observable(true) });
    this.Rota = PropertyEntity({ type: types.entity, codEntity: ko.observable(0), text: Localization.Resources.Cargas.MontagemCarga.Rota, idBtnSearch: guid(), enable: ko.observable(true) });
    this.TotalPedidos = PropertyEntity({ val: ko.observable(""), def: "", text: Localization.Resources.Cargas.MontagemCarga.TotalPedidos });
    this.Buscar = PropertyEntity({ eventClick: buscarCargasRotaClick, type: types.event, text: Localization.Resources.Cargas.MontagemCarga.AdicionarPedidos, visible: ko.observable(true), enable: ko.observable(true) });
    this.RemoverPedidos = PropertyEntity({ eventClick: removerPedidosRotaClick, type: types.event, text: Localization.Resources.Cargas.MontagemCarga.RemoverPedidos, visible: ko.observable(true), enable: ko.observable(true) });
}
...
function removerPedidosRotaClick() {
    exibirConfirmacao(Localization.Resources.Gerais.Geral.Confirmacao, Localization.Resources.Cargas.MontagemCarga.RealmenteDesejaRemoverTodosOsPedidosQueEstaoNaRota, function () {
        ...
        if (codigoRota > 0 || codigoLocalidade > 0) {
            ...
            if (!remove) 
                exibirMensagem(tipoMensagem.aviso, Localization.Resources.Gerais.Geral.Aviso, Localization.Resources.Cargas.MontagemCarga.NaoFoiEncontradaNenhumaEntregaParaEssaConfiguracao);
        } else {
            exibirMensagem(tipoMensagem.atencao, Localization.Resources.Cargas.MontagemCarga.Obrigatorio, Localization.Resources.Cargas.MontagemCarga.ObrigatorioInformarRotaOuLocalidade);
        }
...
function buscarCargasRotaClick() {
    ...
    if (codigoRota > 0 || codigoLocalidade > 0) {
        ...
        for (var i = 0; i < _arrayMarker.length; i++) {
            ...
            for (var p = 0; p < marker.pedidos.length; p++) {
                var pedido = marker.pedidos[p];
                if ((codigoRota > 0 && pedido.RotaFrete != null && pedido.RotaFrete.Codigo == codigoRota) || (codigoLocalidade > 0 && ObterLocalidadePedido(pedido, marker.distribuidor) == codigoLocalidade)) {
...
function AtualizarPontosFaltantes(index) {
    ...
    if (_pedidosSemPontos != undefined && index < _pedidosSemPontos.length) {
...
function AtualizarPontosObtidos() {
    if (_PedidosAtualizarPontos.length > 0) {
        ...
        executarReST("Pessoa/AtualizarLatLngClientes", dados, function (arg) {
            if (!arg.Success) {
                exibirMensagem(tipoMensagem.falha, "Falha", arg.falha);
            }
