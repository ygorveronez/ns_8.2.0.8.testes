...
var CadastroSistemaIntegracao = function () {
    this.SistemaIntegracao = PropertyEntity({ text: Localization.Resources.Cargas.TipoCarga.SistemaDeIntegracao.getRequiredFieldDescription(), val: ko.observable(""), def: "", options: ko.observable([]), required: true });
...
function buscaIntegracoes() {
    return new Promise(function (resolve) {
        executarReST("TipoCarga/BuscarIntegracoes", {}, function (retorno) {
...
function adicionarConfiguracaoIntegracaoClick() {
    if (ValidarCamposObrigatorios(_cadastroSistemaIntegracao)) {
        if (isConfiguracaoSistemaIntegracaoExistente())
            exibirMensagem(tipoMensagem.aviso, Localization.Resources.Cargas.TipoCarga.SistemaDeIntegracaoJaExistente, Localization.Resources.Cargas.TipoCarga.SistemaIntegracaoJaEstaCadastrado.format(obterDescricaoSistemaIntegracao()));
        else {
...
function preencherIntegracaoSalvar(tipoCarga) {
    tipoCarga["Integracoes"] = obterConfiguracaoIntegracaoSalvar();
    tipoCarga["CodigosIntegracao"] = JSON.stringify(_gridCodigosIntegracaoTipoCarga.BuscarRegistros());
}
