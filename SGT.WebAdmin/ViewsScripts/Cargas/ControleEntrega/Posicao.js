...
var Posicao = function () {
    ...
    this.UltimaAtualizacao = PropertyEntity({ text: Localization.Resources.Cargas.ControleEntrega.UltimaAtualizacao.getFieldDescription(), val: ko.observable(""), def: "" });
    this.Endereco = PropertyEntity({ text: Localization.Resources.Cargas.ControleEntrega.Endereco.getFieldDescription(), val: ko.observable(""), def: "" });
}
...
function exibirPosicaoControleEntrega(etapa) {
    LimparCampos(_posicao);
    executarReST("PosicaoAtual/BuscarPorCarga", { Carga: etapa.Carga }, function (arg) {
        if (arg.Success) {
            if (arg.Data !== false) {
                ...
                if (arg.Data.Latitude != 0 && arg.Data.Longitude != 0) {
                    ...
                    setarCoordenadasPosicao(position);
                    setMarker(position);
                } else {
                    var posPadrao = posicionamentoPadrao();
                    setarCoordenadasPosicao(posPadrao);
                    _posicao.UltimaAtualizacao.val(Localization.Resources.Cargas.ControleEntrega.UltimaAtualizacao);
                }
...
function setarCoordenadasPosicao(position) {
    if (_map != null) {
