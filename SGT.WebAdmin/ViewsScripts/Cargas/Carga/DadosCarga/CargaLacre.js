...
var _visibilidadeTabLacre = false;
...
function LoadCargaDadosEmissaoCargaLacre(carga) {
    _cargaDadosEmissaoLacre = new CargaDadosEmissaoLacre();
    $.get("Content/Static/Carga/CargaLacre.html?dyn=" + guid(), function (data) {
        let idDivCargaLacre = carga.EtapaInicioTMS.idGrid;

        data = data.replaceAll("#IDDadosTransporte", idDivCargaLacre);

        if (!_visibilidadeTabLacre)
            return;

        $("#liTabCargaDadosTransporteLacre_" + idDivCargaLacre).removeClass("d-none");
        $("#tabCargaDadosTransporteLacre_" + idDivCargaLacre).html(data);

        _cargaDadosEmissaoLacre = new CargaDadosEmissaoLacre();
        KoBindings(_cargaDadosEmissaoLacre, "knockoutDadosTransporteLacre_" + idDivCargaLacre);
