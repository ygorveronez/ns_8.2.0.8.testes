...
function LerQRCodeCamera() {
    ...
    $('#livestream_scanner').on('shown.bs.modal', function (e) {
        ...
        navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } }).then(function (stream) {
            ...
            loadingMessage.innerText = Localization.Resources.Cargas.CargaControleExpedicao.ErroUtilizacaoCameraSistema
        });

        function tick() {
            if (!cameraQRCodeAtiva) return;
            loadingMessage.innerText = Localization.Resources.Cargas.CargaControleExpedicao.CarregandoVideo;
            if (video.readyState === video.HAVE_ENOUGH_DATA) {
                ...
                if (code) { // Ao detectar
                    ...
                    _conferenciaSeparacao.CodigoBarras.val(code.data);
                    return;
                } else {
