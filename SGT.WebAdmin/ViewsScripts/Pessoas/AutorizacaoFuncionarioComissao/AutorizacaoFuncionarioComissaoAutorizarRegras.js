...
/// <reference path="AutorizacaoFuncionarioComissao.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarFuncionarioComissaoClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        var dados = {
            Codigo: _funcionarioComissao.Codigo.val()
        };

        executarReST("AutorizacaoFuncionarioComissao/AprovarMultiplasRegras", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data !== false) {
...
function rejeitarFuncionarioComissaoClick(dataRow) {
    ...
    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a requisição?", function () {
        ...
        executarReST("AutorizacaoFuncionarioComissao/Rejeitar", dados, function (arg) {
            if (arg.Success) {
                if (arg.Data !== false) {
...
function RejeitarFuncionarioComissao(dataRow) {
    _regra.Codigo.val(dataRow.Codigo);
    _autorizacao.Motivo.visible(true);
    _autorizacao.Rejeitar.visible(true);
}

function AprovarFuncionarioComissao(dataRow) {
    executarReST("AutorizacaoFuncionarioComissao/Aprovar", dataRow, function (arg) {
        if (arg.Success) {
            if (arg.Data !== false) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                AtualizarGridRegras();
...
function GridRegras() {
    ...
    var aprovar = {
        ...
        metodo: AprovarFuncionarioComissao
    };
    var rejeitar = {
        descricao: "Rejeitar",
        id: guid(),
        ...
        metodo: RejeitarFuncionarioComissao
    };
    ...
    var ko_funcionarioComissao = {
        Codigo: _funcionarioComissao.Codigo,
        Usuario: _funcionarioComissao.Usuario,
    };

    // Grid
    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoFuncionarioComissao/RegrasAprovacao", ko_funcionarioComissao, menuOpcoes);
}