...
function criarHTMLModulos(modulos) {
    ...
    $.each(modulos, function (i, modulo) {
        html += "<li>";
        html += "<span><i class='fal " + modulo.Icone + "'></i> " + modulo.Descricao + "</span> &nbsp; <text style='position:absolute; margin-top:7px;'><input type='checkbox' onclick='ckbModuloLiberadoClick(" + modulo.CodigoModulo + ");' id='ckbModuloLiberado_" + modulo.CodigoModulo + "' name='checkbox-inline'>&nbsp; <label for='ckbModuloLiberado_" + modulo.CodigoModulo + "' > " + Localization.Resources.Pessoas.Usuario.LiberarAcessoTotalAoMenu + " " + modulo.Descricao + "</label></text>"
...
function criarHTMLFormularios(modulo) {
    ...
    html += "<thead>";
    html += "<tr>";
    html += "<td style='width:8%' title='" + Localization.Resources.Pessoas.Usuario.LiberarAcesso + "'><input type='checkbox' style='margin-top:0px' onclick='ckbTodosFormulariosClick(" + modulo.CodigoModulo + ");' id='ckb_liberarTodos_" + modulo.CodigoModulo + "' />&nbsp;<label for='ckb_liberarTodos_" + modulo.CodigoModulo + "'>" + Localization.Resources.Pessoas.Usuario.LiberarAcesso + "</label></td>";
    html += "<td style='width:8%' title='" + Localization.Resources.Pessoas.Usuario.SomenteLeitura + "'><input type='checkbox' style='margin-top:0px'  onclick='ckbSomenteLeituraTodosFormulariosClick(" + modulo.CodigoModulo + ");' id='ckb_SomenteLeituraTodos_" + modulo.CodigoModulo + "' />&nbsp;<label for='ckb_SomenteLeituraTodos_" + modulo.CodigoModulo + "'>" + Localization.Resources.Pessoas.Usuario.SomenteLeitura + " </label></td>";
    html += "<td><label>" + Localization.Resources.Pessoas.Usuario.Formulario + "</label></td>";
    html += "</tr>";
    html += "</thead>";
    html += "<tbody>";
    $.each(modulo.Formularios, function (i, formulario) {
        ...
        if (formulario.PermissoesPersonalizadas.length > 0) {
            html += "<tr id='trPermissaoPersonalizada_" + formulario.CodigoFormulario + "'  class='trPermissaoPersonalizada' style='display:none'><td colspan='3'><h4 style='padding:5px;'> " + formulario.Descricao + " " + Localization.Resources.Pessoas.Usuario.PermissoesEspeciais + " </h4><table id='' class='table table-bordered table-hover table-condensed table-striped'>";
...
function buscarPermissoesFormularios() {
    _usuario.FormulariosUsuario.list = new Array();
    _usuario.ModulosUsuario.list = new Array();
...
function PreecherPermissoes(modulos) {
    $.each(modulos, function (i, modulo) {
        if ($("#ckbModuloLiberado_" + modulo.CodigoModulo).prop("checked")) {
            ...
            _usuario.ModulosUsuario.list.push(moduloMap);
        }
        ...
        $.each(modulo.Formularios, function (j, formulario) {
            if ($("#ckbLiberar_" + formulario.CodigoFormulario).prop("checked")) {
                ...
                _usuario.FormulariosUsuario.list.push(formularioMap);
            }
        });
...
function setarPermissoesModulosFormularios() {
    $.each(_usuario.ModulosUsuario.list, function (i, modulo) {
    ...
    $.each(_usuario.FormulariosUsuario.list, function (i, formulario) {
