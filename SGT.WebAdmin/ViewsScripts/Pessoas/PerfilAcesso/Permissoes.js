...
var FormularioUsuarioMap = function () {
    ...
    this.PermissoesPersonalizadasNSelecionadas = PropertyEntity({ type: types.listEntity, list: new Array(), required: false, codEntity: ko.observable(0) });
    this.Descricao = PropertyEntity({ type: types.map, val: "", def: "" });
}
...
var PermissaoPersonalizadaMap = function () {
    ...
    this.Descricao = PropertyEntity({ type: types.map, val: "", def: "", getType: typesKnockout.string });
}
...
function buscarPaginas() {
    executarReST("Pagina/BuscarModulos", null, function (arg) {
        if (arg.Success) {
...
function criarHTMLModulos(modulos) {
    ...
    $.each(modulos, function (i, modulo) {
        html += "<li>";
        html += "<span><i class='fa fa-lg " + modulo.Icone + "'></i> " + modulo.Descricao + "</span> &nbsp; <text style='position:absolute; margin-top:7px;'><input type='checkbox' onclick='ckbModuloLiberadoClick(" + modulo.CodigoModulo + ");' id='ckbModuloLiberado_" + modulo.CodigoModulo + "' name='checkbox-inline'>&nbsp; <label for='ckbModuloLiberado_" + modulo.CodigoModulo + "' > Liberar acesso total ao menu " + modulo.Descricao + "</label></text>"
...
function buscarPermissoesFormularios() {
    ...
    _perfil.FormulariosNSelecionados.list = new Array();
    _perfil.ModulosNSelecionados.list = new Array();
...
function PreecherPermissoes(modulos) {
    $.each(modulos, function (i, modulo) {
        if ($("#ckbModuloLiberado_" + modulo.CodigoModulo).prop("checked")) {
            ...
            moduloMap.Descricao.val = modulo.Descricao;
            _perfil.ModulosPerfil.list.push(moduloMap);
        } else {
            var moduloNSelecionadoMap = new ModuloUsuarioMap();
            moduloNSelecionadoMap.CodigoModulo.val = modulo.CodigoModulo;
            moduloNSelecionadoMap.Descricao.val = modulo.Descricao;
            _perfil.ModulosNSelecionados.list.push(moduloNSelecionadoMap);
        }
        ...
        $.each(modulo.Formularios, function (j, formulario) {
            if ($("#ckbLiberar_" + formulario.CodigoFormulario).prop("checked")) {
                ...
                formularioMap.Descricao.val = formulario.Descricao;
                $.each(formulario.PermissoesPersonalizadas, function (p, permissaoPersonalizada) {
                    if ($("#ckbLiberarPermissao_" + formulario.CodigoFormulario + "_" + permissaoPersonalizada.CodigoPermissao).prop("checked")) {
                        ...
                        permissaoPersonalizadaMap.Descricao.val = permissaoPersonalizada.Descricao;
                        formularioMap.PermissoesPersonalizadas.list.push(permissaoPersonalizadaMap);
                    }
                    else {
                        var permissaoPersonalizadaNSelecionadaMap = new PermissaoPersonalizadaMap();
                        permissaoPersonalizadaNSelecionadaMap.CodigoPermissaoPersonalizada.val = permissaoPersonalizada.CodigoPermissao;
                        permissaoPersonalizadaNSelecionadaMap.Descricao.val = permissaoPersonalizada.Descricao;
                        formularioMap.PermissoesPersonalizadasNSelecionadas.list.push(permissaoPersonalizadaNSelecionadaMap);
                    }
                });
                _perfil.FormulariosPerfil.list.push(formularioMap);
            } else {
                var formularioNSelecionadoMap = new FormularioUsuarioMap();
                formularioNSelecionadoMap.CodigoFormulario.val = formulario.CodigoFormulario;
                formularioNSelecionadoMap.SomenteLeitura.val = $("#ckbSomenteLeitura_" + formulario.CodigoFormulario).prop("checked");
                formularioNSelecionadoMap.PermissoesPersonalizadas.list = new Array();
                formularioNSelecionadoMap.Descricao.val = formulario.Descricao;
                _perfil.FormulariosNSelecionados.list.push(formularioNSelecionadoMap);
            }
        });
