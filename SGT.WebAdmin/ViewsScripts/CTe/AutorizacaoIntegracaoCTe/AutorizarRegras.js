/// <reference path="AutorizacaoIntegracaoCTe.js" />
...
function loadGridRegras() {
    ...
    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoIntegracaoCTe/RegrasAprovacao", knoutCarga, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoIntegracaoCTe/AprovarMultiplasRegras", { Codigo: _carga.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarAlteracaoFreteCargaClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a integração dos CT-es é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a integração dos CT-es da carga?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoIntegracaoCTe/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarAlteracaoFreteCarga(registroSelecionado) {
    executarReST("AutorizacaoIntegracaoCTe/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarCarga() {
    BuscarPorCodigo(_carga, "AutorizacaoIntegracaoCTe/BuscarPorCodigo", function (retorno) {
        if (retorno.Success)
