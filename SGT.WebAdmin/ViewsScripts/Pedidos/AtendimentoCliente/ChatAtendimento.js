...
var _abrirChamadoPaginaPedidos;
var _dataChat;
var _pedidoSelecionadoTelaPedidos;
var _ListaMensagem = new Array();

var ChatAtendimentoPedido = function () {

    this.Pedido = PropertyEntity({ val: ko.observable("") });
    this.DetalhesNumeroCte = PropertyEntity({ val: ko.observable("") });
    this.DetalhesCliente = PropertyEntity({ val: ko.observable("") });
    this.DetalhesCNPJCliente = PropertyEntity({ val: ko.observable("") });
    this.DetalhesDestino = PropertyEntity({ val: ko.observable("") });
    this.DetalhesEmissao = PropertyEntity({ val: ko.observable("") });
    this.DetalhesEntrega = PropertyEntity({ val: ko.observable("") });
    this.DetalhesContato = PropertyEntity({ val: ko.observable("") });
...
function adicionarHistorico(mensagem) {
    ...
    html += '<div class="col" style="' + (proprio ? "float:right;" : "float:left;") + ' margin-bottom: 5px; max-width:70%; min-width:340px; padding-right:0px">';
    html += '<div class="well well-sm" style="padding: 7px 12px; border-radius: 5px; background-color:' + (proprio ? "#475999" : "#FFFFF") + ';color:' + (proprio ? "#FFF" : "#666") + ';border-color:' + (proprio ? "#FFF" : "#666") + '">';
...
function abrirChamado(data) {
    limparModalChat();
    $("#" + data.Codigo + " img").removeClass("blink");

    if (_cardativo != data.Codigo) {

        if (data.PossuiChatAtivo) {
            _atendimentoClientePedido.classPedidos.cssClass("col col-xl-6");
            $("#knoutChatAtendimentoPedido").show();
        } else {
            Global.abrirModal("divSolicitarNovoAtendimentoChat");
        }

        $("#divPaginationAtendimentoPedidoCliente").hide();

        $("#" + _cardativo).removeClass("cardAtivo");
        _cardativo = data.Codigo;
        $("#" + data.Codigo).addClass("cardAtivo");
    } else {
        $("#" + _cardativo).removeClass("cardAtivo");
        _atendimentoClientePedido.classPedidos.cssClass("col col-xl-12");
        $("#knoutChatAtendimentoPedido").hide();
        $("#divPaginationAtendimentoPedidoCliente").show();
        _cardativo = 0;

    }

    _chatAtendimentoPedido.Pedido.val(data.Codigo);
    _chatAtendimentoPedido.DetalhesNumeroCte.val(data.NumeroPedido);
    _chatAtendimentoPedido.DetalhesCliente.val(data.Destinatario);
    _chatAtendimentoPedido.DetalhesEmissao.val(data.DataCriacao);
    _chatAtendimentoPedido.DetalhesEntrega.val(data.DataEntrega);
    _chatAtendimentoPedido.DetalhesDestino.val(data.Destino);
    _chatAtendimentoPedido.DetalhesContato.val(data.Contato);
    _chatAtendimentoPedido.DetalhesCNPJCliente.val(data.CNPJCliente);

    obterHistoricoChat();
}


function abrirChamadoPaginaPedidos(pedido) {
    _pedidoSelecionadoTelaPedidos = pedido;
}
...
function loadChatAtendimentoCliente() {
    ...
    if (_pedidoSelecionadoTelaPedidos > 0) {

        var data = { Codigo: _pedidoSelecionadoTelaPedidos }
        ...
        executarReST("AtendimentoPedidoCliente/Pesquisa", data, function (arg) {
            if (arg.Success) {
                if (arg.Data) {
                    if (arg.Data.Pedidos.length > 0)
                        abrirChamado(arg.Data.Pedidos[0]);
                }
                else
