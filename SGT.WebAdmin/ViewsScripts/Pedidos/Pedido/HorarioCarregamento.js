...
var InfoCarregamentoMontagem = function () {
    ...
    this.EncaixarHorario = PropertyEntity({ val: ko.observable(false), def: false, text: Localization.Resources.Pedidos.Pedido.EncaixarHorario, eventClick: encaixarHorarioClick, getType: typesKnockout.bool, visible: ko.observable(false) });
    this.DataCarregamento = PropertyEntity({ text: Localization.Resources.Pedidos.Pedido.DataCarregamento.getRequiredFieldDescription(), required: true, getType: typesKnockout.dateTime, visible: ko.observable(false) });
    this.DataCarregamentoDisponibilidade = PropertyEntity({ text: Localization.Resources.Pedidos.Pedido.Data.getFieldDescription(), getType: typesKnockout.date, val: ko.observable(Global.DataAtual()), diminuirData: diminuirDataClick, aumentarData: aumentarDataClick, idGrid: guid() });
...
function alterarDataCarregamentoMontagemCarga() {
    _infoCarregamentoMontagem = new InfoCarregamentoMontagem();
    _infoCarregamentoMontagem.Filial.val(_pedido.Filial.codEntity());
    _infoCarregamentoMontagem.TipoOperacao.val(_pedido.TipoOperacao.codEntity());
    //_infoCarregamentoMontagem.ModeloVeicularCarga.val(_pedido.ModeloVeicularCarga.codEntity());

    if (_infoCarregamentoMontagem.Filial.val() == 0)
        return exibirMensagem(tipoMensagem.aviso, Localization.Resources.Pedidos.Pedido.Filial, Localization.Resources.Pedidos.Pedido.SelecioneUmaFilialAntesDeEscolherDataCarregamento);

    if (_CONFIGURACAO_TMS.PermiteSelecionarHorarioEncaixe === true)
        _infoCarregamentoMontagem.EncaixarHorario.visible(true);

    KoBindings(_infoCarregamentoMontagem, "knoutHorarioCarregamento");

    CarregarGridHorarios(_infoCarregamentoMontagem);
    _infoCarregamentoMontagem.DataCarregamentoDisponibilidade.val.subscribe(function () {
        AtualizarHorarios(_infoCarregamentoMontagem);
    });
...
function confirmarAlteracaoCarregamentoClick(e) {
    var data = e.DataCarregamento.val();
    _pedido.DataCarregamento.val(data);
    _pedido.EncaixarHorario.val(e.EncaixarHorario.val());
