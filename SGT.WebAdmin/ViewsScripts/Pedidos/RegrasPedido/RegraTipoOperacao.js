...
var TipoOperacao = function () {
    ...
    // Controle de uso
    this.RegraPorTipoOperacao = PropertyEntity({ getType: typesKnockout.bool, text: "Ativar Regra de autorização por tipo de operação:", val: ko.observable(false), def: false, visible: ko.observable(true) });
    this.RegraPorTipoOperacao.val.subscribe(function (novoValor) {
        SincronzarRegras();
        RegraPorTipoOperacao(novoValor);
    });
...
//*******EVENTOS*******
function RegraPorTipoOperacao(usarRegra) {
...
function loadTipoOperacao() {
    ...
    $("#" + _tipoOperacao.Regras.idGrid).on('sortstop', function () {
        LinhasReordenadasPagamentoMotorista(_tipoOperacao);
    });
}
