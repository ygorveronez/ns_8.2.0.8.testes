...
function delegarClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar a configuração de regra para ICMS?", function () {
        var dados = {
            Codigo: _regraICMS.Codigo.val(),
            UsuarioDelegado: _delegar.UsuarioDelegado.codEntity(),
        };

        executarReST("AutorizacaoAlteracaoRegraICMS/Delegar", dados, function (retorno) {
            if (retorno.Success) {
...
function delegarSelecionadosClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar todas as configurações de regra para ICMS selecionadas?", function () {
        ...
        executarReST("AutorizacaoAlteracaoRegraICMS/DelegarMultiplas", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    atualizarGridRegrasICMS();
