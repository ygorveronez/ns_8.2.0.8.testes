/// <reference path="AutorizacaoAlteracaoRegraICMS.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarRegraICMSClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
    ...
    this.Reprocessar = PropertyEntity({ type: types.event, val: ko.observable(false), visible: ko.observable(false), eventClick: reprocessarRegraICMSClick, text: "Reprocessar" });
}
...
function loadGridRegras() {
    ...
    var knoutRegraICMS = {
        Codigo: _regraICMS.Codigo,
        Usuario: _regraICMS.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoAlteracaoRegraICMS/RegrasAprovacao", knoutRegraICMS, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoAlteracaoRegraICMS/AprovarMultiplasRegras", { Codigo: _regraICMS.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarRegraICMSClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a configuração de regra para ICMS é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a configuração de regra para ICMS?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoAlteracaoRegraICMS/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function reprocessarRegraICMSClick() {
    executarReST("AutorizacaoAlteracaoRegraICMS/ReprocessarRegraICMS", { Codigo: _regraICMS.Codigo.val() }, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                if (retorno.Data.RegraReprocessada) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "A configuração de regra para ICMS foi reprocessada com sucesso.");
...
function AprovarRegraICMS(registroSelecionado) {
    executarReST("AutorizacaoAlteracaoRegraICMS/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function rejeitarRegraICMS(registroSelecionado) {
