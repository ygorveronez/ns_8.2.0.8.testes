...
var Anexos = function () {
    this.GuaritaCheckList = PropertyEntity({ val: ko.observable(0), def: 0, getType: typesKnockout.int });

    //-- Adicionar arquivo
    this.Codigo = PropertyEntity({ val: ko.observable(0), def: 0, getType: typesKnockout.int });
    this.Descricao = PropertyEntity({ text: "Descrição:", type: types.map, getType: typesKnockout.string, maxlength: 150 });    
    this.Arquivo = PropertyEntity({ type: types.file, val: ko.observable(""), text: "Anexo:", enable: ko.observable(true), codEntity: ko.observable(0), visible: ko.observable(true) });
    ...
    this.config = PropertyEntity({
        ...
        url: {
            excluir: "GuaritaCheckListAnexo/ExcluirAnexo",
            download: "GuaritaCheckListAnexo/DownloadAnexo",
            anexar: "GuaritaCheckListAnexo/AnexarArquivos?callback=?"
        }
    });
}
...
function GetCodigoTela() {
    return _guaritaCheckList.Codigo.val();
}

// Retorna o nome do parametro da entidade principal
function GetNomeParametro() {
    return "GuaritaCheckList";
}

// Verifica se pode ou nao anexar
function PodeGerenciarAnexos() {
    var codigo = _guaritaCheckList.Codigo.val();

    return codigo > 0 && _guaritaCheckList.Situacao.val() == EnumSituacaoGuaritaCheckList.Aberto;
}
