...
function adicionarAnexoClick() {
    ...
    if (_RFIConvite.Codigo.val() > 0)
        enviarAnexos(_RFIConvite.Codigo.val(), [anexo]);
...
function downloadAnexoClick(registroSelecionado) {
    ...
    executarDownload("RFIConviteAnexo/DownloadAnexo", dados);
}
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("RFIConviteAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function enviarAnexos(codigo, anexos) {
    ...
    enviarArquivo("RFIConviteAnexo/AnexarArquivos", { Codigo: codigo }, formData, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data)
