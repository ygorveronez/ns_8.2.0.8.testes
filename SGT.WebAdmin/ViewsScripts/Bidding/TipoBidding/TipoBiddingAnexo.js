...
function adicionarAnexoClick() {
    ...
    if (_tipoBidding.Codigo.val() > 0)
        enviarAnexos(_tipoBidding.Codigo.val(), [anexo]);
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("TipoBiddingAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function enviarAnexos(codigo, anexos) {
    ...
    enviarArquivo("TipoBiddingAnexo/AnexarArquivos", { Codigo: codigo }, formData, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data)
