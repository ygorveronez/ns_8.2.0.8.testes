...
function loadAprovacaoTransferenciaPallet() {
    _aprovacao = new Aprovacao();
    KoBindings(_aprovacao, "knockoutAprovacaoTransferenciaPallet");
...
function loadGridAutorizacoes() {
    ...
    _gridAutorizacoes = new GridView(_aprovacao.Regras.idGrid, "Transferencia/PesquisaAutorizacoes", _aprovacao, menuOpcoes);
...
function detalharAutorizacaoClick(registroSelecionado) {
    ...
    BuscarPorCodigo(_detalheAutorizacao, "Transferencia/DetalhesAutorizacao", function (retorno) {
        if (retorno.Success) {
...
function preencherAprovacao(dadosAprovacao, codigoTransferencia, situacaoTransferencia) {
    if (situacaoTransferencia === EnumSituacaoTransferenciaPallet.SemRegraAprovacao)
        _aprovacao.PossuiRegras.val(false);
    else {
        _aprovacao.PossuiRegras.val(true);

        if (dadosAprovacao) {
            _aprovacao.Codigo.val(codigoTransferencia);
