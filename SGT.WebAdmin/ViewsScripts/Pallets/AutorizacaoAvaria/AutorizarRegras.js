/// <reference path="AutorizacaoAvaria.js" />
...
function loadGridRegras() {
    ...
    var knoutAvaria = {
        Codigo: _avaria.Codigo,
        Usuario: _avaria.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoAvariaPallet/RegrasAprovacao", knoutAvaria, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoAvariaPallet/AprovarMultiplasRegras", { Codigo: _avaria.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarAvariaClick(dataRow) {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a avaria de pallets é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a avaria de pallets?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoAvariaPallet/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarAvaria(registroSelecionado) {
    executarReST("AutorizacaoAvariaPallet/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarAvaria() {
    BuscarPorCodigo(_avaria, "AutorizacaoAvariaPallet/BuscarPorCodigo", function (retorno) {
        if (retorno.Success)
...
function rejeitarAvaria(registroSelecionado) {
