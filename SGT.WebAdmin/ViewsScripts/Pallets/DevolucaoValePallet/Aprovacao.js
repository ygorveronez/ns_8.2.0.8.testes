...
function loadAprovacaoDevolucaoValePallet() {
    _aprovacao = new Aprovacao();
    KoBindings(_aprovacao, "knockoutAprovacaoDevolucaoValePallet");
...
function loadGridAutorizacoes() {
    ...
    _gridAutorizacoes = new GridView(_aprovacao.Regras.idGrid, "DevolucaoValePallet/PesquisaAutorizacoes", _aprovacao, menuOpcoes);
...
function detalharAutorizacaoClick(registroSelecionado) {
    ...
    BuscarPorCodigo(_detalheAutorizacao, "DevolucaoValePallet/DetalhesAutorizacao", function (retorno) {
        if (retorno.Success) {
...
function preencherAprovacao(dadosAprovacao, codigoDevolucaoValePallet, situacaoDevolucaoValePallet) {
    if (situacaoDevolucaoValePallet === EnumSituacaoDevolucaoValePallet.SemRegraAprovacao)
        _aprovacao.PossuiRegras.val(false);
    else {
        _aprovacao.PossuiRegras.val(true);

        if (dadosAprovacao) {
            _aprovacao.Codigo.val(codigoDevolucaoValePallet);
