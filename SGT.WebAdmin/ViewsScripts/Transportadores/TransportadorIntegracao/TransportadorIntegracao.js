...
var _pesquisaTransportadorIntegracao;
var _gridTransportadorIntegracao;
...
function loadTransportadorIntegracao() {

    _pesquisaTransportadorIntegracao = new PesquisaTransportador();
    KoBindings(_pesquisaTransportadorIntegracao, "knockoutPesquisaTransportador", false, _pesquisaTransportadorIntegracao.Pesquisar.id);

    new BuscarTransportadores(_pesquisaTransportadorIntegracao.Transportador);

    BuscarIntegracoesTransportador();
}
...
function reenviarMultiplasIntegracoesClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja reenviar todas as integrações que estão com falha de integração?", function () {
        executarReST("TransportadorIntegracao/ReenviarMultiplasIntegracoes", null, function (r) {
            if (r.Success) {
                if (r.Data) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
                    _gridTransportadorIntegracao.CarregarGrid();
                }
                else
...
function BuscarIntegracoesTransportador() {
    ...
    var configExportacao = {
        url: "TransportadorIntegracao/ExportarPesquisa",
        titulo: "Integrações Transportador"
    };

    _gridTransportadorIntegracao = new GridViewExportacao(_pesquisaTransportadorIntegracao.Pesquisar.idGrid, "TransportadorIntegracao/Pesquisa", _pesquisaTransportadorIntegracao, menuOpcoes, configExportacao, null, 10);
    RecarregarGrid();
}

function ReenviarIntegracao(data) {
    executarReST("TransportadorIntegracao/Reenviar", { Codigo: data.Codigo }, function (r) {
        if (r.Success) {
            exibirMensagem(tipoMensagem.ok, "Sucesso!", "Reenvio solicitado com sucesso.");
            _gridTransportadorIntegracao.CarregarGrid();
        } else {
...
function BuscarHistoricoIntegracao(integracao) {
    ...
    _gridHistoricoIntegracao = new GridView("tblHistoricoIntegracao", "TransportadorIntegracao/ConsultarHistoricoIntegracao", _pesquisaHistoricoIntegracao, menuOpcoes, { column: 1, dir: orderDir.desc });
    _gridHistoricoIntegracao.CarregarGrid();
}

function DownloadArquivosHistoricoIntegracao(historicoConsulta) {
    executarDownload("TransportadorIntegracao/DownloadArquivosHistoricoIntegracao", { Codigo: historicoConsulta.Codigo });
}

function RecarregarGrid(cb) {
    _gridTransportadorIntegracao.CarregarGrid(function (data) {
