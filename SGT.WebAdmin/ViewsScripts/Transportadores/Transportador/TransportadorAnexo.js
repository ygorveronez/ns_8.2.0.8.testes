...
var Anexo = function () {
    ...
    this.Anexos = PropertyEntity({ text: Localization.Resources.Transportadores.Transportador.Anexos, type: types.local, getType: typesKnockout.dynamic, def: new Array(), val: ko.observable(new Array()), idGrid: guid(), visible: ko.observable(true) });
...
function loadAnexoTransportador() {
    _anexo = new Anexo();
    KoBindings(_anexo, "knockoutTransportadorAnexo");
...
function loadGridAnexo() {
    ...
    var header = [
        ...
        { data: "NomeArquivo", title: Localization.Resources.Transportadores.Transportador.Nome, width: "25%", className: "text-align-left" }
    ];
...
function adicionarAnexoClick() {
    ...
    if (arquivo.files.length == 0)
        return exibirMensagem(tipoMensagem.atencao, Localization.Resources.Transportadores.Transportador.Anexos, Localization.Resources.Transportadores.Transportador.NenhumArquivoSelecionado);
    ...
    if (_transportador.Codigo.val() > 0)
        enviarAnexos(_transportador.Codigo.val(), [anexo]);
    else {
...
function downloadAnexoClick(registroSelecionado) {
    executarDownload("TransportadorAnexo/DownloadAnexo", { Codigo: registroSelecionado.Codigo });
}
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("TransportadorAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
                    exibirMensagem(tipoMensagem.ok, Localization.Resources.Gerais.Geral.Sucesso, Localization.Resources.Transportadores.Transportador.AnexoExcluidoComSucesso);
                    removerAnexoLocal(registroSelecionado);
                }
                else
...
function enviarAnexos(codigo, anexos) {
    ...
    if (formData) {
        enviarArquivo("TransportadorAnexo/AnexarArquivos?callback=?", { Codigo: codigo }, formData, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
                    ...
                    exibirMensagem(tipoMensagem.aviso, Localization.Resources.Transportadores.Transportador.NaoFoiPossivelAnexarArquivo, retorno.Msg);
            }
            else
                exibirMensagem(tipoMensagem.falha, Localization.Resources.Gerais.Geral.Falha, retorno.Msg);
        });
