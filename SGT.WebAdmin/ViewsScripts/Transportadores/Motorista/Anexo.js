...
var Anexos = function () {
    ...
    this.TipoAnexoMotorista = PropertyEntity({ text: Localization.Resources.Gerais.Geral.Tipo.getFieldDescription(), val: ko.observable(""), options: EnumTipoAnexoMotorista.obterOpcoes(), def: "", visible: ko.observable(true) });
    ...
    this.ImprimirNaFicha = PropertyEntity({ text: Localization.Resources.Transportadores.Motorista.ImprimirAnexoNaFichaDoMotorista, val: ko.observable(false), getType: typesKnockout.bool, def: false, visible: ko.observable(_CONFIGURACAO_TMS.TipoServicoMultisoftware === EnumTipoServicoMultisoftware.MultiTMS) });

    this.Anexos = PropertyEntity({ text: ko.observable(Localization.Resources.Transportadores.Motorista.Anexos), type: types.local, getType: typesKnockout.dynamic, def: new Array(), val: ko.observable(new Array()), idGrid: guid(), visible: ko.observable(true) });
    ...
    this.config = PropertyEntity({
        msg: {
            situacaonaopermite: Localization.Resources.Transportadores.Motorista.SituacaoNaoPermiteExcluirArquivos
        },
        url: {
...
function PodeGerenciarAnexos() {
    var codigo = _motorista.Codigo.val();
...
function PodeExcluirAnexos() {
    ...
    if (!_CONFIGURACAO_TMS.UsuarioAdministrador && !VerificaSePossuiPermissaoEspecial(EnumPermissaoPersonalizada.Motorista_PermiteRemoverAnexos, _PermissoesPersonalizadas))
...
function loadAnexos() {
    ...
    var header = [
        ...
        { data: "TipoAnexoMotorista", title: Localization.Resources.Gerais.Geral.Tipo ,width: "40%", className: "text-align-left" },
        { data: "NomeArquivo", title: Localization.Resources.Gerais.Geral.Nome, width: "25%", className: "text-align-left" },
        { data: "ImprimirNaFicha", title: Localization.Resources.Transportadores.Motorista.ImprimeFicha, width: "10%", className: "text-align-left", visible: _CONFIGURACAO_TMS.TipoServicoMultisoftware === EnumTipoServicoMultisoftware.MultiTMS }
    ];
...
function adicionarAnexoClick() {
    // Permissao
    if (!PodeGerenciarAnexos())
        return exibirMensagem(tipoMensagem.atencao, Localization.Resources.Gerais.Geral.Anexo, _anexos.config.msg.situacaonaopermite);
    ...
    var anexo = {
        ...
        TipoAnexoMotorista: _anexos.TipoAnexoMotorista.val(),
        ImprimirNaFicha: _anexos.ImprimirNaFicha.val(),
...
function CriaEEnviaFormData(anexos, dados) {
    ...
    anexos.forEach(function (anexo) {
        if (isNaN(anexo.Codigo)) {
            ...
            formData.append("TipoAnexoMotorista", anexo.TipoAnexoMotorista);
            formData.append("ImprimirNaFicha", anexo.ImprimirNaFicha);
        }
    });
