/// <reference path="../../Enumeradores/EnumSituacaoCadastroVeiculo.js" />
/// <reference path="AutorizacaoCadastroVeiculo.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarCadastroVeiculoClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function loadGridRegras() {
    ...
    var opcaoRejeitar = {
        ...
        metodo: rejeitarCadastroVeiculo
    };
    ...
    var knoutCadastroVeiculo = {
        Codigo: _veiculo.Codigo,
        Usuario: _veiculo.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoCadastroVeiculo/RegrasAprovacao", knoutCadastroVeiculo, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoCadastroVeiculo/AprovarMultiplasRegras", { Codigo: _veiculo.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarCadastroVeiculoClick(dataRow) {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para reprovar o cadastro veículo é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Você realmente deseja reprovar o cadastro veículo?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoCadastroVeiculo/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarCadastroVeiculo(registroSelecionado) {
    executarReST("AutorizacaoCadastroVeiculo/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarCadastroVeiculo() {
    BuscarPorCodigo(_veiculo, "AutorizacaoCadastroVeiculo/BuscarPorCodigo", function (retorno) {
        if (retorno.Success)
            controlarExibicaoAbaDelegar(retorno.Data.SituacaoCadastro === EnumSituacaoCadastroVeiculo.Aprovado);
        else
            exibirMensagem(tipoMensagem.falha, "falha", retorno.Msg);
    }, null);
}

function rejeitarCadastroVeiculo(registroSelecionado) {
