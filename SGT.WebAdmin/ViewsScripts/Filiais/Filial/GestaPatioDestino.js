...
var GestaoPatioDestino = function () {
    ...
    this.GuaritaSaidaExigirAnexo = PropertyEntity({ text: Localization.Resources.Filiais.Filial.ExigirAnexo, getType: typesKnockout.bool, val: ko.observable(false), def: false });
...
function loadGestaoPatioDestino() {
    ...
    new BuscarFilialBalanca(_gestaoPatioDestino.BalancaGuaritaEntrada, null, null, _filial.Codigo);
    new BuscarFilialBalanca(_gestaoPatioDestino.BalancaGuaritaSaida, null, null, _filial.Codigo);

    $('a[href="#tabGestaoPatioConfiguracoes"]').on('shown.bs.tab', ControlarAlturarBoxesDestino);
...
function baixarQRCodeDestino(etapaFluxoGestaoPatio) {
    if (isCadastroEmEdicao())
        executarDownload("Filial/BaixarQrCodeEtapa", { Filial: _filial.Codigo.val(), Etapa: etapaFluxoGestaoPatio, Tipo: EnumTipoFluxoGestaoPatio.Destino });
}
