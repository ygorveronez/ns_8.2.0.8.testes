...
var GestaoPatio = function () {
    ...
    this.CheckListExigirAnexo = PropertyEntity({ getType: typesKnockout.bool, text: Localization.Resources.Filiais.Filial.ExigirAnexo, val: ko.observable(false), def: false, visible: ko.observable(true) });
...
function loadGestaoPatio() {
    ...
    PreencherTipoIntegracao();

    new BuscarFilialBalanca(_gestaoPatio.BalancaGuaritaEntrada, null, null, null, _sequenciaGestaoPatio.Filial);
    new BuscarFilialBalanca(_gestaoPatio.BalancaGuaritaSaida, null, null, null, _sequenciaGestaoPatio.Filial);
...
function baixarQRCode(etapaFluxoGestaoPatio) {
    if (isCadastroEmEdicao())
        executarDownload("Filial/BaixarQrCodeEtapa", { Filial: _sequenciaGestaoPatio.Filial.codEntity(), Etapa: etapaFluxoGestaoPatio, Tipo: EnumTipoFluxoGestaoPatio.Origem });
}
...
function verificaVisibilidadeIntegracaoP44(val) {
...
function PreencherTipoIntegracao() {
        ...
        if (arg.Success) {
            if (arg.Data) {
                ...
                if (integracoes.length > 0) {
                    ...
                    _gestaoPatio.InformarDocaCarregamentoTipoIntegracao.options(integracaoOptionBind);
                }
            } else {
