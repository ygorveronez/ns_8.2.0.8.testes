...
function delegarClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar a não conformidade?", function () {
        var dados = {
            Codigo: _naoConformidade.Codigo.val(),
            UsuarioDelegado: _delegar.UsuarioDelegado.codEntity(),
        };

        executarReST("AutorizacaoNaoConformidade/Delegar", dados, function (retorno) {
            if (retorno.Success) {
...
function delegarSelecionadosClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar todas as não conformidades selecionadas?", function () {
        ...
        executarReST("AutorizacaoNaoConformidade/DelegarMultiplas", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    atualizarGridNaoConformidade();
