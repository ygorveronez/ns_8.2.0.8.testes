...
var Anexo = function () {
    ...
    this.Anexos = PropertyEntity({ text: "Anexos", type: types.local, getType: typesKnockout.dynamic, def: new Array(), val: ko.observable(new Array()), idGrid: guid(), visible: ko.observable(true), required: ko.observable(false) });
...
function loadAnexo() {
    _anexo = new Anexo();
    KoBindings(_anexo, "knockoutAutorizacaoNaoConformidadeAnexos");
...
function adicionarAnexoClick() {
    ...
    if (_naoConformidade.Codigo.val() > 0)
        enviarAnexos(_naoConformidade.Codigo.val(), [anexo]);
    else {
...
function downloadAnexoClick(registroSelecionado) {
    executarDownload("AutorizacaoNaoConformidadeAnexo/DownloadAnexo", { Codigo: registroSelecionado.Codigo });
}
...
function removerAnexoClick(registroSelecionado) {
    if (isNaN(registroSelecionado.Codigo))
        ...
        executarReST("AutorizacaoNaoConformidadeAnexo/ExcluirAnexo", { Codigo: registroSelecionado.Codigo }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function enviarAnexos(codigo, anexos) {
    ...
    if (formData) {
        enviarArquivo("AutorizacaoNaoConformidadeAnexo/AnexarArquivos?callback=?", { Codigo: codigo }, formData, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
                    ...
                    exibirMensagem(tipoMensagem.aviso, "Não foi possível anexar.", retorno.Msg);
            }
            else
                exibirMensagem(tipoMensagem.falha, "Falha", retorno.Msg);
        });
