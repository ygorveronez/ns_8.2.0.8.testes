...
/// <reference path="../../Enumeradores/EnumCondicaoAutorizacao.js" />
...
var _configuracaoRegras = {
    ...
    Alcadas: [
        ...
        {
            descricao: "Tipo Não Conformidade",
            prop: "TipoNaoConformidade",
            tipoPropriedadeAlcada: EnumTipoPropriedadeAlcada.Entidade,
            busca: function (ko) {
                new BuscarNaoConformidade(ko);
            }
        },
        {
...
var _nomeControllerRegrasAutorizacao = "RegraAutorizacaoNaoConformidade";
...
var RegrasAutorizacao = function () {
    ...
    this.TipoAprovadorRegra = PropertyEntity({ val: ko.observable(EnumTipoAprovadorRegra.Usuario), options: EnumTipoAprovadorRegra.obterOpcoesPorSetor(), def: EnumTipoAprovadorRegra.Usuario, text: "*Tipo do Aprovador: " });
    ...
    this.Setores = PropertyEntity({ type: types.map, getType: typesKnockout.dynamic, def: "", val: obterListaSetores, idGrid: guid() });
    this.AdicionarSetor = PropertyEntity({ text: "Adicionar", idBtnSearch: guid() });
    ...
    _configuracaoRegras.Alcadas.forEach(function (alcada) {
        ...
        this["Condicao" + alcada.prop] = PropertyEntity({ type: types.local, text: "Condição: ", val: ko.observable(EnumCondicaoAutorizacao.IgualA), options: alcada.tipoPropriedadeAlcada === EnumTipoPropriedadeAlcada.Entidade ? EnumCondicaoAutorizacaoEntidade.obterOpcoes() : EnumCondicaoAutorizacao.obterOpcoes(), def: EnumCondicaoAutorizacao.IgualA });
    ...
    this.Grupo = PropertyEntity({ val: ko.observable(EnumGrupoNC.NaoSelecionado), options: EnumGrupoNC.obterOpcoesNaoSelecionado(), def: EnumGrupoNC.NaoSelecionado, text: "Grupo: " });
    this.SubGrupo = PropertyEntity({ val: ko.observable(EnumSubGrupoNC.NaoSelecionado), options: EnumSubGrupoNC.obterOpcoesNaoSelecionado(), def: EnumSubGrupoNC.NaoSelecionado, text: "Sub Grupo: "});
    this.Area = PropertyEntity({ val: ko.observable(EnumAreaGrupoNC.NaoSelecionado), options: EnumAreaGrupoNC.obterOpcoesNaoSelecionado(), def: EnumAreaGrupoNC.NaoSelecionado, text: "Área: " });

    this.CRUDAdicionar = PropertyEntity({ eventClick: adicionarClick, type: types.event, text: "Adicionar", visible: ko.observable(true) });
    this.CRUDCancelar = PropertyEntity({ eventClick: cancelarClick, type: types.event, text: "Limpar / Nova", visible: ko.observable(true) });
...
function loadGridSetores() {
    ...
    _gridSetores = new BasicDataTable(_regrasAutorizacao.Setores.idGrid, header, menuOpcoes, null, null, _configuracaoRegras.NumeroAprovadores);
    _gridSetores.CarregarGrid([]);

    new BuscarSetorFuncionario(_regrasAutorizacao.AdicionarSetor, AdicionarSetorRetorno, null, _gridSetores);
}
...
function editarRegraAprovacao(data) {
    ...
    BuscarPorCodigo(_regrasAutorizacao, _nomeControllerRegrasAutorizacao + "/BuscarPorCodigo", function (arg) {
        ...
        _regrasAutorizacao.Setores.val(arg.Data.Setores);
...
function removerSetorClick(data) {
    var dataGrid = _gridSetores.BuscarRegistros().slice();

    dataGrid = dataGrid.filter(function (a) {
        return a.Codigo != data.Codigo;
    });

    _gridSetores.CarregarGrid(dataGrid);
}
...
function AdicionarSetorRetorno(data) {
    if (data != null) {
        if (!$.isArray(data))
            data = [data];

        var dataGrid = _gridSetores.BuscarRegistros();
...
function obterListaSetores() {
    if (arguments.length > 0 && arguments[0] != "")
        _gridSetores.CarregarGrid(arguments[0]);
    else
        return JSON.stringify(_gridSetores.BuscarRegistros());
}
...
function obterObjetoRegraSalvar() {
    var dados = {
        ...
        Setores: obterListaSetores(),
        Grupo: _regrasAutorizacao.Grupo.val(),
        SubGrupo: _regrasAutorizacao.SubGrupo.val(),
        Area: _regrasAutorizacao.Area.val()
    };
...
function limparCamposAlcada(prop) {
    ...
    if (config.tipoPropriedadeAlcada === EnumTipoPropriedadeAlcada.Entidade)
        LimparCampoEntity(_regrasAutorizacao[prop]);
    else
        _regrasAutorizacao[prop].val(_regrasAutorizacao[prop].def);
