...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarOrdemServicoClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function loadGridRegras() {
    ...
    var knoutOrdemServico = {
        Codigo: _ordemServico.Codigo,
        Usuario: _ordemServico.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoOrdemServico/RegrasAprovacao", knoutOrdemServico, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoOrdemServico/AprovarMultiplasRegras", { Codigo: _ordemServico.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function rejeitarOrdemServicoClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar a Ordem de Serviço é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar a Ordem de Serviço?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoOrdemServico/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarOrdemServico(registroSelecionado) {
    executarReST("AutorizacaoOrdemServico/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function rejeitarOrdemServico(registroSelecionado) {
