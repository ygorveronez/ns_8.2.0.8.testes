...
function loadAprovacaoTermoQuitacao() {
    _aprovacao = new Aprovacao();
    KoBindings(_aprovacao, "knockoutAprovacaoTermoQuitacao");
...
function loadGridAutorizacoes() {
    ...
    _gridAutorizacoes = new GridView(_aprovacao.Regras.idGrid, "TermoQuitacao/PesquisaAutorizacoes", _aprovacao, menuOpcoes);
...
function detalharAutorizacaoClick(registroSelecionado) {
    ...
    BuscarPorCodigo(_detalheAutorizacao, "TermoQuitacao/DetalhesAutorizacao", function (retorno) {
        if (retorno.Success) {
...
function preencherAprovacao(dadosAprovacao, situacaoTermoQuitacao) {
    if (situacaoTermoQuitacao === EnumSituacaoTermoQuitacao.SemRegraAprovacao)
