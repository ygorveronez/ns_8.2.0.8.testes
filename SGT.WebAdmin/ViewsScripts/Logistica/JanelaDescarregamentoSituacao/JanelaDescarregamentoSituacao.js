...
var JanelaDescarregamentoSituacao = function () {
    ...
    this.Situacao = PropertyEntity({ text: "*Situação: ", val: ko.observable(""), options: EnumSituacaoCargaJanelaDescarregamentoCadastrada.obterOpcoes(), def: "" });
}

var PesquisaJanelaDescarregamentoSituacao = function () {
    this.Descricao = PropertyEntity({ text: "Descrição:", required: true, getType: typesKnockout.string, val: ko.observable("") });
    this.Situacao = PropertyEntity({ text: "Situação: ", val: ko.observable(EnumSituacaoCargaJanelaDescarregamentoCadastrada.Todas), options: EnumSituacaoCargaJanelaDescarregamentoCadastrada.obterOpcoesPesquisa(), def: EnumSituacaoCargaJanelaDescarregamentoCadastrada.Todas });
...
function loadGridJanelaDescarregamentoSituacao() {
    ...
    var configuracoesExportacao = { url: "JanelaDescarregamentoSituacao/ExportarPesquisa", titulo: "Situações da janela de Descarregamento" };

    _gridJanelaDescarregamentoSituacao = new GridViewExportacao(_pesquisaJanelaDescarregamentoSituacao.Pesquisar.idGrid, "JanelaDescarregamentoSituacao/Pesquisa", _pesquisaJanelaDescarregamentoSituacao, menuOpcoes, configuracoesExportacao);
...
function adicionarClick(e, sender) {
    Salvar(_janelaDescarregamentoSituacao, "JanelaDescarregamentoSituacao/Adicionar", function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
...
function atualizarClick(e, sender) {
    Salvar(_janelaDescarregamentoSituacao, "JanelaDescarregamentoSituacao/Atualizar", function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
...
function editarClick(registroSelecionado) {
    ...
    BuscarPorCodigo(_janelaDescarregamentoSituacao, "JanelaDescarregamentoSituacao/BuscarPorCodigo", function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
...
function excluirClick() {
    exibirConfirmacao("Confirmação", "Realmente deseja excluir esse cadastro?", function () {
        ExcluirPorCodigo(_janelaDescarregamentoSituacao, "JanelaDescarregamentoSituacao/ExcluirPorCodigo", function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
