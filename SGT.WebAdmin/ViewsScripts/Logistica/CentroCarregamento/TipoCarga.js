...
var TipoCarga = function () {
    ...
    this.Tipo = PropertyEntity({ type: types.event, text: Localization.Resources.Logistica.CentroCarregamento.AdicionarTipoDeCarga, idBtnSearch: guid(), issue: 53 });
};
...
function LoadTipoCarga() {
    ...
    var header = [
        ...
        { data: "CodigoIntegracao", visible: false },
    ...
    new BuscarTiposdeCarga(_tipoCarga.Tipo, function (r) {
        if (r != null) {
            for (var i = 0; i < r.length; i++)
                _tiposCarga.push({ Codigo: r[i].Codigo, CodigoIntegracao: r[i].CodigoTipoCargaEmbarcador, Descricao: r[i].Descricao });
...
function RecarregarGridTipoCarga() {
    _gridTipoCarga.CarregarGrid(_centroCarregamento.TiposCarga.val());
    _tiposCarga = _centroCarregamento.TiposCarga.val();
}

function ExcluirTipoCargaClick(knoutTipoCarga, data) {
    if (isTipoCargaPossuiTempoCarregamentoVinculado(data.Codigo)) {
        exibirMensagem(tipoMensagem.atencao, Localization.Resources.Gerais.Geral.Atencao, Localization.Resources.Logistica.CentroCarregamento.NaoPossivelExcluirEsteTipoDeCargaPoisMesmoEstaVinculadoUmTempoDeCarregamento);
        return;
    }

    var tiposCarga = knoutTipoCarga.basicTable.BuscarRegistros();
...
function RecarregarOpcoesTipoCarga() {
    var tipoCargaGrid = _tipoCarga.Tipo.basicTable.BuscarRegistros();
    ...
    _tempoCarregamentoCadastro.TipoCarga.options(_opcoesTipoCarga.sort(function (a, b) {
        if (a.text < b.text)
            return -1;
...
function ObterCodigoIntegracaoTipoCargaPorCodigo(codigo) {
    return _tipoCarga.Tipo.basicTable.BuscarRegistros().filter(o => o.Codigo == codigo)[0].CodigoIntegracao;
}