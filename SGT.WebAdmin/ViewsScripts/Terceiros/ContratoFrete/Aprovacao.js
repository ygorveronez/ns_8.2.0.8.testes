...
function detalharAutorizacaoContratoFreteClick(dataRow) {
    _detalheAutorizacao.Codigo.val(dataRow.Codigo);

    BuscarPorCodigo(_detalheAutorizacao, "ContratoFrete/DetalhesAutorizacao", function (arg) {
        if (arg.Success) {
            if (arg.Data != null) {
...
function GridAprovadores() {
    var detalhes = { descricao: "Detalhes", id: "clasEditar", evento: "onclick", metodo: detalharAutorizacaoContratoFreteClick, tamanho: "10", icone: "" };
    var menuOpcoes = {
        tipo: TypeOptionMenu.link,
        opcoes: [detalhes]
    };

    _gridAutorizacoes = new GridView(_aprovacao.Regras.idGrid, "ContratoFrete/PesquisaAutorizacoes", _aprovacao, menuOpcoes);
    _gridAutorizacoes.CarregarGrid();
}

function ListarAprovacoes(data) {
    _aprovacao.Codigo.val(data.Codigo);
    if (_contratoFrete.SituacaoContratoFrete.val() == EnumSituacaoContratoFrete.SemRegra) {
