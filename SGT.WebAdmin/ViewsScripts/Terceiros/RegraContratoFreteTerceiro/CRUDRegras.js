...
function atualizarClick(e, sender) {
    SalvarRegras("RegraContratoFreteTerceiro/Atualizar", "Atualizado com sucesso.", e, sender);
}

function excluirClick(e, sender) {
    exibirConfirmacao("Confirmação", "Realmente deseja excluir as regras?", function () {
        ExcluirPorCodigo(_regraContratoFreteTerceiro, "RegraContratoFreteTerceiro/ExcluirPorCodigo", function (arg) {
            if (arg.Success) {
                if (arg.Data) {
...
function adicionarClick(e, sender) {
    SalvarRegras("RegraContratoFreteTerceiro/Adicionar", "Cadastrado com sucesso.", e, sender);
}
...
function ValidacaoGeral() {
    ...
    //-- Valida aprovadores
    if (_gridAprovadores.BuscarRegistros().length < _regraContratoFreteTerceiro.NumeroAprovadores.val())
        return ["O número de aprovadores selecionados deve ser maior ou igual a " + _regraContratoFreteTerceiro.NumeroAprovadores.val()];


    //-- Valida as regras 
    if (_valorContrato.Alcadas.required() && _valorContrato.Alcadas.val().length == 0)
        msg_regras.push(msg_padrao.replace("#nomeregra#", "Valor do Contrato"));
    
    if (_terceiro.Alcadas.required() && _terceiro.Alcadas.val().length == 0)
        msg_regras.push(msg_padrao.replace("#nomeregra#", "Terceiro"));
...
function SalvarRegras(url, mensagemSucesso, e, sender) {
    ...
    if (msg_regras.length > 0)
        ...
        Salvar(_regraContratoFreteTerceiro, url, function (arg) {
            if (arg.Success) {
                if (arg.Data) {
