/// <reference path="AutorizacaoContratoFreteAcrescimoDesconto.js" />
...
var Autorizacao = function () {
    ...
    this.Rejeitar = PropertyEntity({ eventClick: rejeitarContratoFreteAcrescimoDescontoClick, type: types.event, text: "Rejeitar", visible: ko.observable(false) });
...
function loadGridRegras() {
    var opcaoAprovar = {
        ...
        metodo: AprovarContratoFreteAcrescimoDesconto
    };
    ...
    var opcaoRejeitar = {
        ...
        metodo: rejeitarContratoFreteAcrescimoDesconto
    };
    ...
    var knoutContratoFreteAcrescimoDesconto = {
        Codigo: _contratoFreteAcrescimoDesconto.Codigo,
        Usuario: _contratoFreteAcrescimoDesconto.Usuario,
    };

    _gridRegras = new GridView(_autorizacao.Regras.idGrid, "AutorizacaoContratoFreteAcrescimoDesconto/RegrasAprovacao", knoutContratoFreteAcrescimoDesconto, menuOpcoes);
}
...
function aprovarMultiplasRegrasClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja aprovar todas as regras?", function () {
        executarReST("AutorizacaoContratoFreteAcrescimoDesconto/AprovarMultiplasRegras", { Codigo: _contratoFreteAcrescimoDesconto.Codigo.val() }, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
                    ...
                    atualizarContratoFreteAcrescimoDesconto()
                    atualizarGridRegras();
                } else {
...
function rejeitarContratoFreteAcrescimoDescontoClick() {
    if (_autorizacao.Motivo.val().length < 20)
        return exibirMensagem(tipoMensagem.aviso, "Caracteres Mínimos", "Para rejeitar o Acréscimo/Desconto do Contrato de Frete é necessário informar ao mínimo 20 caracteres no motivo.");

    exibirConfirmacao("Confirmação", "Realmente deseja rejeitar o Acréscimo/Desconto do Contrato de Frete?", function () {
        var dados = {
            Codigo: _regra.Codigo.val(),
            Motivo: _autorizacao.Motivo.val(),
        };

        executarReST("AutorizacaoContratoFreteAcrescimoDesconto/Reprovar", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data) {
...
function AprovarContratoFreteAcrescimoDesconto(registroSelecionado) {
    executarReST("AutorizacaoContratoFreteAcrescimoDesconto/Aprovar", registroSelecionado, function (retorno) {
        if (retorno.Success) {
            if (retorno.Data) {
                exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
...
function atualizarContratoFreteAcrescimoDesconto() {
...
function rejeitarContratoFreteAcrescimoDesconto(registroSelecionado) {
