...
function delegarClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar o Acréscimo/Desconto do Contrato de Frete?", function () {
        var dados = {
            Codigo: _contratoFreteAcrescimoDesconto.Codigo.val(),
            UsuarioDelegado: _delegar.UsuarioDelegado.codEntity(),
        };

        executarReST("AutorizacaoContratoFreteAcrescimoDesconto/Delegar", dados, function (retorno) {
            if (retorno.Success) {
...
function delegarSelecionadosClick() {
    exibirConfirmacao("Confirmação", "Você realmente deseja delegar todos os Acréscimo/Desconto dos Contratos de Frete selecionados?", function () {
        ...
        executarReST("AutorizacaoContratoFreteAcrescimoDesconto/DelegarMultiplas", dados, function (retorno) {
            if (retorno.Success) {
                if (retorno.Data !== false) {
                    exibirMensagem(tipoMensagem.ok, "Sucesso", "Enviado com sucesso.");
                    atualizarGridContratosFreteAcrescimoDesconto();
