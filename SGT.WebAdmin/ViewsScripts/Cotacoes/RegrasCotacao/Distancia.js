...
var Distancia = function () {
    ...
    this.Distancia = PropertyEntity({ text: "Distancia:", type: types.map, getType: typesKnockout.int, required: ko.observable(true), def: 0 });
    ...
    this.Regras.val.subscribe(function () {
        SincronzarRegras();
        RenderizarGridRegras(_distancia, _gridRegrasDistancia, "editarRegraDistanciaClick", typesKnockout.int);
    });
     
    // Controle de uso
    this.UsarRegraPorDistancia = PropertyEntity({ getType: typesKnockout.bool, text: "Ativar Regra de cotação por Distância:", val: ko.observable(false), def: false, visible: ko.observable(true) });
    this.UsarRegraPorDistancia.val.subscribe(function (novoValor) {
        SincronzarRegras();
        UsarRegraPorDistancia(novoValor);
    });
...
//*******EVENTOS*******
function UsarRegraPorDistancia(usarRegra) {
...
function loadDistancia() {
    ...
    //-- Grid Regras
    _gridRegrasDistancia = new GridReordering(_configRegras.infoTable, _distancia.Regras.idGrid, GeraHeadTable("Distância"));
    _gridRegrasDistancia.CarregarGrid();
    $("#" + _distancia.Regras.idGrid).on('sortstop', function () {
        LinhasReordenadasCotacao(_distancia);
    });
}
...
function ObjetoRegraDistancia() {
    ...
    var regra = {
        ...
        Ordem: ordem != 0 ? ordem : _gridRegrasDistancia.ObterOrdencao().length + 1,
