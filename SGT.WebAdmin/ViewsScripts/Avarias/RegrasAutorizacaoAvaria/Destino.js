...
var Destino = function () {
    ...
    // Controle de uso
    this.UsarRegraPorDestino = PropertyEntity({ getType: typesKnockout.bool, text: "Ativar Regra de autorização por destino:", val: ko.observable(false), def: false, visible: ko.observable(true) });
    this.UsarRegraPorDestino.val.subscribe(function (novoValor) {
        SincronzarRegras();
        UsarRegraPorDestino(novoValor);
    });

    this.Adicionar = PropertyEntity({ eventClick: adicionarRegraDestinoClick, type: types.event, text: "Adicionar", visible: ko.observable(true) });
    this.Atualizar = PropertyEntity({ eventClick: atualizarRegraDestinoClick, type: types.event, text: "Atualizar", visible: ko.observable(false) });
    this.Excluir = PropertyEntity({ eventClick: excluirRegraDestinoClick, type: types.event, text: "Excluir", visible: ko.observable(false) });
    this.Cancelar = PropertyEntity({ eventClick: cancelarRegraDestinoClick, type: types.event, text: "Cancelar", visible: ko.observable(false) });
}
...
function loadDestino() {
    ...
    $("#" + _destino.Regras.idGrid).on('sortstop', function () {
        LinhasReordenadasAvaria(_destino);
    });
}
