...
namespace SGT.WebAdmin.Models.Grid
{
    public class Grid
    {
        ...
        public Grid(Dominio.Entidades.Embarcador.Relatorios.Relatorio relatorio, List<Head> headerPadrao, bool buscarCabecalhoPorCodigoDinamico)
        {
            ...
            foreach (Dominio.Entidades.Embarcador.Relatorios.RelatorioColuna coluna in relatorio.Colunas)
            {
                ...
                if (headPadrao != null)
                {
                    ...
                    head.insertInGroupByWhenInvisible = headPadrao.insertInGroupByWhenInvisible;
                    head.useTextFormat = coluna.UtilizarFormatoTexto;

                    if (!string.IsNullOrWhiteSpace(coluna.TabName))
                        head.tabName = coluna.TabName;
                    else
                        head.tabName = headPadrao.tabName;
            ...
            foreach (string tabName in header.Where(h => !string.IsNullOrWhiteSpace(h.tabName)).Select(h => h.tabName).Distinct())
            {
                Tab newTab = new Tab();
                newTab.name = tabName;
                listTabs.Add(newTab);
        ...
        public Grid(HttpRequest request)
        {
            if (request != null)
            {
                ...
                this.listTabs = (grid.listTabs?.Count > 0) ? grid.listTabs.ToList() : new List<Tab>();
        ...
        private byte[] GerarCSVGrid(List<Head> cabecalhos, List<dynamic> registros)
        {
            ...
            Utilidades.IO.FileStorageService.Storage.WriteLine(arquivoTemporario, conteudoArquivo.ToString(), Encoding.UTF8);
        ...
        {
            string caminho = Utilidades.IO.FileStorageService.Storage.Combine(Servicos.Embarcador.Configuracoes.ConfigurationInstance.GetInstance()?.ObterCaminhoArquivos(), "GridExportacao");
        ...
        public void AdicionaRows(dynamic linhas)
        {
            ...
            foreach (dynamic linha in linhas)
            {
                ...
                if (ObterValorPropriedade(linha, "CodigoCarga", out object codigoCarga))
                    dictionary["CodigoCarga"] = codigoCarga;
        ...
        public List<dynamic> ObterDataRows()
        {
            return this.DataRows;
        }

        public List<Dominio.ObjetosDeValor.Grid.HeaderExportacao> ObterHeadersExportacao(List<Head> headers)
        {
            return headers
                    .Select(h => new Dominio.ObjetosDeValor.Grid.HeaderExportacao
                    {
                        title = h.title,
                        data = h.data,
                        name = h.name,
