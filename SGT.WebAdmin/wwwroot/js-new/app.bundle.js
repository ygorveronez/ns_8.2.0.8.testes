...
var initApp = (function (app) {
    ...
    app.resetSettings = function () {
        ...
        $(myapp_config.mythemeAnchor).attr('href', "/css/customTheme7");
    ...
    app.domReadyMisc = function () {
         ...
         **/
        if (typeof ($.fn.tooltip) !== 'undefined' && $('[data-bs-toggle="tooltip"]').length) {
         ...
         **/
        if (typeof ($.fn.popover) !== 'undefined' && $('[data-bs-toggle="popover"]').length) {
            ...
            $('[data-bs-toggle="popover"]').popover({ sanitize: false }); /*{trigger: "focus"}*/
        ...
        //	Popper.Defaults.modifiers.computeStyle.gpuAcceleration = false;         <--------- ERRO
        //} else {
        //	console.log("OOPS! bs.popover is not loaded");
        //} 
        ...
        myapp_config.root_
            .on('click touchend', '[data-action]', function (e) {
                ...
                switch (true) {
                     ...
                     * Usage 2 (target): <a href="#" data-action="toggle" data-class="add-this-class-to-target" data-bs-target="target">...</a>
                     **/
                    case (actiontype === 'toggle'):

                        var target = $(this).attr('data-bs-target') || myapp_config.root_,
                    ...
                    /**
                     * toggle swap trigger
                     * Usage (target): <a href="#" data-action="toggle-swap" data-class="root-text" data-bs-target="html">...</a>
                     **/
                     ...
                     * Usage (target): <a href="#" data-action="toggle-swap" data-replaceclass="classesToReplace" data-class="replaceWithClass" data-bs-target="body">...</a>
                     **/
...
function setPageBreadcrumbPath() {
    let pageBreadcrumbElement = $(".page-breadcrumb");

    if (pageBreadcrumbElement.length <= 0)
        return;
    ...
    let element = $('#js-nav-menu a[href="#' + url + '"]');

    if (!element || element.length <= 0)
        return;

    let favoritePage = _FormulariosFavoritos.find(o => o.Caminho == url);

    let path = [];

    do {
        path.push(element.attr("title"));
        element = element.parent("li").parent("ul").parent("li").find("a:first");
    } while (element.length > 0);

    let breadcrumbHTML = "";

    let favoritePageHTML = "<a href='javascript:void(0);' onclick='SalvarFormularioFavorito();'><i class='setFavoriteUserPageIcon " + (favoritePage != null ? "fas" : "far") + " fa-star me-2'></i></a>"

    if (url == "Home")
        favoritePageHTML = "";

    for (let i = path.length; i > 0; i--)
        breadcrumbHTML += '<li class="breadcrumb-item' + (i == 1 ? ' active' : '') + '">' + ((i == path.length) ? favoritePageHTML : '') + path[i - 1] + "</li>";

    pageBreadcrumbElement.html(breadcrumbHTML);
}
...
function checkURL() {
    ...
    var title = ($('#js-nav-menu a[href="#' + url + '"]').attr('title'));
...
function loadURL(url, container) {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            // place cog
            //===================================TRECHO MODIFICADO========================================//
            container.html('<h1 class="ajax-loading-animation d-flex"><i class="spinner-grow spinner-sm me-2"></i>' + Localization.Resources.Gerais.Geral.Carregando + '...</h1>');
            //===================================FIM TRECHO MODIFICADO========================================//
        ...
        success: function (data, status, xhr) {
            ...
            try {
                ...
                let somenteLeitura = xhr.getResponseHeader("SomenteLeitura");
            ...
            var intervarWaitMasterLayoutLoad = setInterval(function () {
                if (_masterLayoutCarregado) {
                    container
                        .css({
                            opacity: '0.0'
                        })
                    ...
                    setPageBreadcrumbPath();
                    clearInterval(intervarWaitMasterLayoutLoad);
