...
namespace SGT.Monitoramento.Thread
{
    public class EnviarNotificacoesAlertas : AbstractThreadProcessamento
    {
        ...
        {
            if (enable)
                NotificacoesAlertaThread = base.IniciarThread(stringConexao, ambiente, tipoServicoMultisoftware, clienteMultisoftware, arquivoNivelLog, tempoSleep);
        ...
        override protected void Parar()
        {
            ...
            {
                NotificacoesAlertaThread.Abort();
        ...
        private void EnviarEmailNotificacao(string email, string mensagem, string assunto, string stringConexao, Repositorio.UnitOfWork unidadeTrabalho)
        {
            ...
            if (!string.IsNullOrEmpty(email))
            {
                try
                {
                    Servicos.Email serEmail = new Servicos.Email(stringConexao);
