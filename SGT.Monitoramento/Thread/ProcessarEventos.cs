    ...
    public class ProcessarEventos : AbstractThreadProcessarEventos
    {
        ...
        public override void ProcessarEventosPendentes(Repositorio.UnitOfWork unitOfWork)
        {
            ...
            if (total > 0)
            {
                ...
                List<Servicos.Embarcador.Monitoramento.Eventos.AbstractEvento> eventosAtivos = CarregarEventosAtivos(unitOfWork, locaisPernoite, locaisAreaDeRisco);
                if (eventosAtivos.Count > 0)
                {
                    ...
                    List<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> posicoesVeiculos = CarregarPosicoesVeiculos(unitOfWork, codigosVeiculos, dataInicio, dataFinal);
                            ...
                            if (totalEventosProcessar > 0)
                            {
                                ...
                                for (int j = 0; j < totalEventosProcessar; j++)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        ProcessarEventosAtivos(listaMonitoramentoEventoAtivo, eventosAtivos, monitoramento, processarEventosVeiculo[j], posicoesObjetoValor, alertas, monitoramentoCache.Entregas, codigosClientesAlvo, monitoramentoCache.CargaJanelaCarregamento, monitoramentoCache.CargaJanelasDescarregamento, configuracao, unitOfWork);
                                    }
                                    catch (Exception e)
                                    {
        ...
        private List<Servicos.Embarcador.Monitoramento.Eventos.AbstractEvento> CarregarEventosAtivos(Repositorio.UnitOfWork unitOfWork, Dominio.Entidades.Embarcador.Logistica.Locais[] locaisPernoite, Dominio.Entidades.Embarcador.Logistica.Locais[] locaisAreaDeRisco)
        {
            ...
            Servicos.Embarcador.Monitoramento.Eventos.ParadaNaoProgramada ParadaNaoProgramada = new Servicos.Embarcador.Monitoramento.Eventos.ParadaNaoProgramada(unitOfWork, Program.Cliente);
        ...
        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoProcessarEvento> BuscarMonitoramentoProcessarEvento(Repositorio.UnitOfWork unitOfWork)
        {
            ...
            //List<long> codigoPosicoesPendentesTrocaDeAlvo = repMonitoramentoTrocaAlvo.BuscarCodigosPosicoes();
            //int totalPosicoesTrocaDeAlvo = codigoPosicoesPendentesTrocaDeAlvo.Count;
            //Log(totalPosicoesTrocaDeAlvo + " posicoes com troca de alvo pendente", 1);

            //Repositorio.Embarcador.Logistica.Monitoramento repMonitoramento = new Repositorio.Embarcador.Logistica.Monitoramento(unitOfWork);
            //List<int> codigoMonitoramentosPendentes = repMonitoramento.BuscarCodigosMonitoramentosPendentesProcessamento();
            //int totalMonitoramentos = codigoMonitoramentosPendentes.Count;
            //Log(totalMonitoramentos + " monitoramentos com troca de alvo pendente", 1);
            ...
            //if (totalPosicoesTrocaDeAlvo == 0 && totalMonitoramentos == 0)
            //if (totalMonitoramentos == 0)
            //{
            //    // Não há nenhuma troca de alvo pendente, pode processar todos os eventos pendentes
            //    return processarEventos;
            //}
            //else
            //{
            //    // Permite processar apenas os eventos que não tenham a respectiva posição pendente de processamento na troca de alvo ou do monitoramento
            //    List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoProcessarEvento> processarEventosPermitidos = new List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoProcessarEvento>();
            //    bool existe;
            //    int totalEventos = processarEventos.Count;
            //    for (int i = 0; i < totalEventos; i++)
            //    {
            //        existe = false;

            //        //// Não está pendente de processamento da troca de alo
            //        //for (int j = 0; j < totalPosicoesTrocaDeAlvo; j++)
            //        //{
            //        //    if (processarEventos[i].CodigoPosicao == codigoPosicoesPendentesTrocaDeAlvo[j])
            //        //    {
            //        //        existe = true;
            //        //        break;
            //        //    }
            ...
            //        //if (!existe)
            //        //{
            //        // ... e não está pendente de processamento do monitoramento
            //        for (int j = 0; j < totalMonitoramentos; j++)
            //        {
            //            if (processarEventos[i].CodigoMonitoramento != null && processarEventos[i].CodigoMonitoramento == codigoMonitoramentosPendentes[j])
            //            {
            //                existe = true;
            //                break;
            //            }
            ...
            //        // Se não está pendente em nenhuma das filas, pode ser processado
            //        if (!existe)
            //        {
            //            processarEventosPermitidos.Add(processarEventos[i]);
            //        }
            //    }
            //    return processarEventosPermitidos;
            //}
        }
