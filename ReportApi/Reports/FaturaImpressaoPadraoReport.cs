...
[UseReportType(ReportType.FaturaImpressaoPadrao)]
public class FaturaImpressaoPadraoReport : ReportBase
{
    public FaturaImpressaoPadraoReport(UnitOfWork unitOfWork, RelatorioReportService servicoRelatorioReportService,
    ...
    public override ReportResult InternalProcess(Dictionary<string, string> extraData)
    {
        ...
        string caminhoLogo = Utilidades.IO.FileStorageService.Storage.Combine(new Servicos.Embarcador.Relatorios.Relatorio().ObterConfiguracaoArquivo(_unitOfWork).CaminhoLogoEmbarcador, "crystal.png");
        ...
        bool efetuarImpressaoTxMoedaEstrangeira = fatura.Cliente != null && fatura.Cliente.NaoUsarConfiguracaoFaturaGrupo
            ? fatura.Cliente?.ConfiguracaoFatura?.EfetuarImpressaoDaTaxaDeMoedaEstrangeira.GetValueOrDefault() ?? false
            : fatura.GrupoPessoas?.ConfiguracaoFatura?.EfetuarImpressaoDaTaxaDeMoedaEstrangeira.GetValueOrDefault() ?? false;

        if (efetuarImpressaoTxMoedaEstrangeira)
        {
            for (int i = 0; i < listaRelatorioPadraoFaturaDados.Count; i++)
            {
                listaRelatorioPadraoFaturaDados[i].ObservacaoFatura += Environment.NewLine + "Taxa Dólar: " + listaRelatorioPadraoFaturaDados[0].ValorMoedaCotacao.ToString("N4");
                listaRelatorioPadraoFaturaDados[i].ObservacaoFatura += Environment.NewLine + "Data Base do CRT: " + listaRelatorioPadraoFaturaDados[0].DataBaseCRT.ToString("dd/MM/yyyy");
                listaRelatorioPadraoFaturaDados[i].ImprimirObservacaoFatura = true;
            }
        }
