    ...
    public class ProcessarEventosSinal : AbstractThreadProcessarEventos
    {
        ...
        public override void ProcessarEventosPendentes(Repositorio.UnitOfWork unitOfWork, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente cliente)
        {
            ...
            if (eventos.Count > 0)
            {
                ...
                IList<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoProcessarEvento> posicoesAtuais = repPosicaoAtual.BuscarProcessarEventos();
                    ...
                    try
                    {
                        for (int i = 0; i < total; i++)
                        {
                            if (posicoesAtuais[i].CodigoVeiculo != null)
                            {
                                ...
                                ProcessarEventosAtivos(listaMonitoramentoEventoAtivo, eventos, monitoramento, posicoesAtuais[i], null, alertas, null, null, null, null, configuracao, unitOfWork, null);
         ...
         */
        private List<Dominio.Entidades.Embarcador.Logistica.Monitoramento> BuscarMonitoramentosAbertos(Repositorio.UnitOfWork unitOfWork, List<int> codigosVeiculos, IList<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoProcessarEvento> posicoesAtuais)
        {
