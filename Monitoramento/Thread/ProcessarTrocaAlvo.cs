...
namespace Monitoramento.Thread
{
    ...
    public class ProcessarTrocaAlvo : AbstractThreadProcessamento
    {
        ...
        private List<PermanenciasClientesCarga> permanenciasClientesCargaCache = new();
        private List<PermanenciasLocalCarga> permanenciasLocaisCargaCache = new();
        private List<PermanenciasSubareasCarga> permanenciasSubareasCargaCache = new();
        private List<CargaEntregasCarga> cargaEntregasCargaCache = new();
        private List<ClienteOrigemCarga> clienteOrigemCargaCache = new();
        private List<Dominio.Entidades.Embarcador.Cargas.CargaJanelaDescarregamento> cargaJanelasDescarregamentosCache = new();
        private List<Dominio.Entidades.Embarcador.Logistica.Monitoramento> monitoramentosCache = new();
        private IList<Dominio.ObjetosDeValor.Embarcador.Logistica.PosicaoAlvo> posicoesAlvoCache = new List<Dominio.ObjetosDeValor.Embarcador.Logistica.PosicaoAlvo>();
        private IList<Dominio.ObjetosDeValor.Embarcador.Logistica.PosicaoAlvo> posicoesAlvoSubareaCache = new List<Dominio.ObjetosDeValor.Embarcador.Logistica.PosicaoAlvo>();
        private List<Dominio.Entidades.Embarcador.Logistica.MonitoramentoEvento> monitoramentoEventoCache = new();
        ...
        {
            if (enable)
                ProcessarTrocasAlvoThread = base.IniciarThread(stringConexao, ambiente, tipoServicoMultisoftware, clienteMultisoftware, arquivoNivelLog, tempoSleep, cancellationToken);
        ...
        override protected void Parar()
        {
            ...
            {
                ProcessarTrocasAlvoThread.Interrupt();
        ...
        private void BuscarProcessarTrocaAlvo(Repositorio.UnitOfWork unitOfWork, AdminMultisoftware.Dominio.Enumeradores.TipoServicoMultisoftware tipoServicoMultisoftware, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente clienteMultisoftware)
        {
            ...
            if (total > 0)
            {
                ...
                {
                    inicio1 = DateTime.UtcNow;
                    GuardarCargaEntregasNoCache(monitoramentosCarga, unitOfWork);
                    GuardarPermanenciasClientesCache(monitoramentosCarga, unitOfWork);
                    GuardarPermanenciasSubareasCache(monitoramentosCarga, unitOfWork);
                    GuardarPermanenciasLocaisCache(monitoramentosCarga, unitOfWork);
                    GuardarClientesOrigemCargaCache(monitoramentosCarga, unitOfWork);
                    GuardarCargaJanelasDescarregamentosCache(monitoramentosCarga, unitOfWork);
                    //GuardarPosicaoAlvoClienteVeiculoCache(trocasDeAlvo, unitOfWork);
                    //GuardarPosicaoAlvoClienteSubareaVeiculoCache(trocasDeAlvo, unitOfWork);
                    GuardarMonitoramentosNoCache(monitoramentosCarga, unitOfWork);
                    GuardarMonitoramentoEventoCache(monitoramentosCarga, unitOfWork);
                    Log("~Guardar Dados CACHE~", inicio1, 2);

                    inicio1 = DateTime.UtcNow;

                    for (int i = 0; i < total; i++)
                    {
                        ProcessarTrocasDeAlvoMonitoramento(unitOfWork, monitoramentosCarga[i], trocasDeAlvo, clientesFronteira, configuracao, tipoServicoMultisoftware, clienteMultisoftware, configuracaoMonitoramento, configuracaoControleEntrega);
                    }
                ...
                //Log("ProcessarTrocasDeAlvoMonitoramento", inicio, 1);
                ...
                if (configuracao.DistanciaMinimaPercorridaParaSaidaDoAlvo > 0)
                {
                    ...
                    IList<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoSaidaDeAlvo> saidasDeAlvo = repMonitoramentoSaidaAlvo.BuscarSaidasDeAlvoPendentes((from obj in monitoramentosCarga select obj.CodigoMonitoramento).ToList(), limiteRegistros);
                    ...
                    if (totalSaidasDeAlvo > 0)
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < total; i++)
                            {
                                List<int> codigosSaidasAlvoExcluir = ProcessarSaidasDeAlvoMonitoramento(unitOfWork, monitoramentosCarga[i], saidasDeAlvo, clientesFronteira, configuracao, configuracaoIntegracao, tipoServicoMultisoftware, clienteMultisoftware, configuracaoMonitoramento, configuracaoControleEntrega);
                        ...
                        if (total > 0)
                        {
                            ...
                            DbTransaction? transaction1 = null;
            ...
            if (totalTrocasDeAlvo > 0)
            {
                ...
                if (ExisteAlgumaTrocaDeAlvoElegivel(trocasDeAlvoMonitoramento))
                {
                    for (int i = 0; i < totalTrocasDeAlvo; i++)
                    {
                        try
                        {
                            ProcessaTrocaDeAlvo(unitOfWork, trocasDeAlvoMonitoramento[i], clientesFronteira, configuracao, tipoServicoMultisoftware, clienteMultisoftware, configuracaoMonitoramento, configuracaoControleEntrega);
                        }
                        catch (Exception ex)
                        {
                        ...
                        if (!string.IsNullOrWhiteSpace(codigosClientesAlvosAtuais[i]))
                        {
                            ...
                            if (ClienteEstaNaLista(codigoClienteAlvoAtual, codigosClientesEntregas))
                            {
                                ...
                                if (cargaEntrega != null)
                                {
                                    ...
                                    {
                                        RegistrarEntradaNoAlvo(trocaDeAlvo, permanenciasCliente, cargaEntrega, codigoClienteAlvoAtual, configuracao, tipoServicoMultisoftware, clienteMultisoftware, unitOfWork, configuracaoMonitoramento, configuracaoControleEntrega);
                                    }
                                }
                            }
                            else
                            {
                                ...
                                {
                                    RegistrarEntradaNoAlvo(trocaDeAlvo, null, null, codigoClienteAlvoAtual, configuracao, tipoServicoMultisoftware, clienteMultisoftware, unitOfWork, configuracaoMonitoramento, configuracaoControleEntrega);
                                }
                            }
                        }
                    }
                            ...
                            if (ClienteEstaNaLista(codigoClienteSubareaAlvoAtual, codigosClientesEntregas))
                            {
                                ...
                                if (cargaEntrega != null)
                                {
                                    ...
                                    if (permanenciaSubarea == null)
                                    {
                                        ...
                                        {
                                            EntradaNaSubarea(permanenciasSubarea, cargaEntrega, posicaoNaSubarea.Data, codigoSubareaAlvoAtual, trocaDeAlvo.CodigoMonitoramento, configuracaoMonitoramento, unitOfWork);
                                        }
                                    }
                                }
                            }
                                ...
                                if (registrarSaida)
                                {
                                    ...
                                    if (configuracao.DistanciaMinimaPercorridaParaSaidaDoAlvo > 0)
                                    {
                                        Dominio.Entidades.Embarcador.Logistica.MonitoramentoSaidaAlvo monitoramentoSaidaAlvo = new Dominio.Entidades.Embarcador.Logistica.MonitoramentoSaidaAlvo
                                        {
                                            ...
                                            Posicao = new Dominio.Entidades.Embarcador.Logistica.Posicao { Codigo = trocaDeAlvo.CodigoPosicaoAnterior },
                                            Latitude = trocaDeAlvo.LongitudePosicaoAnterior.HasValue ? trocaDeAlvo.LongitudePosicaoAnterior.Value : 0,
                                            Longitude = trocaDeAlvo.LongitudePosicaoAnterior.HasValue ? trocaDeAlvo.LongitudePosicaoAnterior.Value : 0,
                                            Data = trocaDeAlvo.DataVeiculoPosicaoAnterior ?? DateTime.Now,
                                            CargaEntrega = cargaEntrega != null ? new Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega() { Codigo = cargaEntrega.Codigo } : null,
                        ...
                        if (!string.IsNullOrWhiteSpace(codigosSubareasAlvosAnterior[i]))
                        {
                            ...
                            if (ClienteEstaNaLista(codigoClienteSubareaAlvoAnterior, codigosClientesEntregas))
                            {
                                ...
                                if (saiuDaSubarea)
                                {
                                    ...
                                    {
                                        List<Dominio.Entidades.Embarcador.Logistica.PermanenciaSubarea> permanenciasSubarea = BuscarPermanenciasSubareasCache(unitOfWork, trocaDeAlvo.CodigoCarga);
                                        SaidaDaSubarea(permanenciasSubarea, cargaEntrega, trocaDeAlvo.DataVeiculoPosicaoAtual ?? DateTime.Now, codigoSubareaAlvoAnterior, unitOfWork);
                                    }
                                }
                            }
                        }
                    ...
                    if (monitoramentoCarga.CodigoMonitoramento > 0 && saidasDeAlvoMonitoramento[i].CodigoPosicao > 0 && saidasDeAlvoMonitoramento[i].CodigoCliente > 0 && saidasDeAlvoMonitoramento[i].CodigoMonitoramento > 0)
                    {
                        ...
                        if (saidasDeAlvoMonitoramento[i].CodigoUltimaPosicaoMonitoramento != saidasDeAlvoMonitoramento[i].CodigoPosicao)
                        {
                            ...
                            if (registrarSaida)
                            {
                                ...
                                if (posicoes.Count >= 2 || (configuracaoMonitoramento?.IdentificarEntradaEmAlvoComPosicaoUnicaIgnorandoTemposDePermanencia ?? false))
                                {
                                    inicio1 = DateTime.UtcNow;
                                    ...
                                    Log($"BuscarPermanenciasClientesCache {posicoes.Count}", inicio1, 5);

                                    unitOfWork.Start(); //Não tem problemas com o start aqui.
                                    if (configuracaoMonitoramento?.IdentificarEntradaEmAlvoComPosicaoUnicaIgnorandoTemposDePermanencia ?? false)
                                    {
                                        SaidaDoAlvo(permanenciasCliente, cargaEntrega, saidasDeAlvoMonitoramento[i].CodigoCliente, saidasDeAlvoMonitoramento[i].DataVeiculoPosicao, saidasDeAlvoMonitoramento[i].LatitudePosicao, saidasDeAlvoMonitoramento[i].LongitudePosicao, saidasDeAlvoMonitoramento[i].CodigoMonitoramento, monitoramentoCarga.CodigoCarga, saidasDeAlvoMonitoramento[i].NaoProcessarTrocaAlvoViaMonitoramentoTipoOperacao, configuracao, tipoServicoMultisoftware, clienteMultisoftware, unitOfWork, configuracaoControleEntrega);
                                        codigosSaidasAlvoExcluir.Add(saidasDeAlvoMonitoramento[i].CodigoSaidaAlvo);
                                    }
                                    else
                                    {
        ...
        private void RegistrarEntradaNoAlvo(Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo trocaDeAlvo, List<Dominio.Entidades.Embarcador.Logistica.PermanenciaCliente> permanenciasCliente, Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega cargaEntrega, double codigoCliente, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, AdminMultisoftware.Dominio.Enumeradores.TipoServicoMultisoftware tipoServicoMultisoftware, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente clienteMultisoftware, Repositorio.UnitOfWork unitOfWork, Dominio.Entidades.Embarcador.Configuracoes.ConfiguracaoMonitoramento configuracaoMonitoramento, Dominio.Entidades.Embarcador.Configuracoes.ConfiguracaoControleEntrega configuracaoControleEntrega)
        {
            ...
            TimeSpan permacenciaNoCliente = (trocaDeAlvo.DataVeiculoPosicaoAtual ?? DateTime.Now) - posicaoEmAlvo.Data;
        ...
        private void VerificarFinalizaMonitoramentoRetornoOrigem(DateTime data, int codigoMonitoramento, int codigoCarga, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, AdminMultisoftware.Dominio.Enumeradores.TipoServicoMultisoftware tipoServicoMultisoftware, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente clienteMultisoftware, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            {
                Dominio.Entidades.Embarcador.Logistica.Monitoramento monitoramento = monitoramentosCache.Find(monitoramento => monitoramento.Codigo == codigoMonitoramento);
            ...
            if (data != null && codigoMonitoramento > 0)
            {
                ...
                if (cargaEntrega != null)
                {
                    if (DeveProcessarTrocaDeAlvoEntrega(cargaEntrega, configuracao, naoProcessarTrocaAlvoViaMonitoramentoTipoOperacao, unitOfWork))
                    {
                        ...
                        {
                            inicio = DateTime.UtcNow;
                            Servicos.Embarcador.Monitoramento.Entrega.IniciarEntrega(cargaEntrega, monitoramento, data, latitude, longitude, configuracao, tipoServicoMultisoftware, clienteMultisoftware, configuracaoControleEntrega, unitOfWork);
                    ...
                    if (!cargaEntrega.DataEntradaRaio.HasValue || (cargaEntrega.DataEntradaRaio.HasValue && data < cargaEntrega.DataEntradaRaio) || (cargaEntrega.Carga.TipoOperacao?.ConfiguracaoControleEntrega?.SobrescreverDataEntradaSaidaAlvo ?? false))
                    {
                        inicio = DateTime.UtcNow;
                        Dominio.Entidades.Embarcador.Cargas.CargaJanelaDescarregamento janelaDescarregamento = cargaJanelasDescarregamentosCache.Find(j => j.Carga.Codigo == cargaEntrega.Carga.Codigo);
                        Servicos.Embarcador.Carga.ControleEntrega.ControleEntrega.AtualizarEntradaNoRaio(cargaEntrega, data, unitOfWork, configuracao, janelaDescarregamento);
                    ...
                    if (permanenciaCliente == null)
                    {
                        ...
                        permanenciaCliente.CargaEntrega = new Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega() { Codigo = cargaEntrega.Codigo };
                        permanenciaCliente.Cliente = new Dominio.Entidades.Cliente() { CPF_CNPJ = codigoCliente };
                        ...
                        if (configuracaoMonitoramento.HabilitarVinculoPermanenciasComHistoricoStatusViagem)
                        {
                            ...
                            Dominio.Entidades.Embarcador.Logistica.MonitoramentoHistoricoStatusViagem monitoramentoHistoricoStatusViagemAnterior = repMonitoramentoHistoricoStatusViagem.BuscarUltimoHistoricoStatusViagem(monitoramento.Codigo);
                            if (monitoramentoHistoricoStatusViagemAnterior != null)
                            {
                                repositorioMonitoramentoHistoricoStatusViagemPermanenciaCliente.Inserir(new Dominio.Entidades.Embarcador.Logistica.MonitoramentoHistoricoStatusViagemPermanenciaCliente()
                                {
                                    PermanenciaCliente = permanenciaCliente,
                                    MonitoramentoHistoricoStatusViagem = monitoramentoHistoricoStatusViagemAnterior
                                });
                            }
                        }
                    }
                    ...
                    if (FinalizarEntregaEntradaRaioTipoOperacao)
                        Servicos.Embarcador.Monitoramento.Entrega.Finalizar(cargaEntrega, monitoramento, data, latitude, longitude, codigoCliente, configuracao, tipoServicoMultisoftware, unitOfWork, configuracaoControleEntrega);
                ...
                {
                    inicio = DateTime.UtcNow;
                    Servicos.Embarcador.Monitoramento.Carga.IniciarViagem(monitoramento, data, latitude, longitude, configuracao, tipoServicoMultisoftware, clienteMultisoftware, unitOfWork);
        ...
        private void EntradaNoLocal(List<Dominio.Entidades.Embarcador.Logistica.PermanenciaLocal> permanenciasLocal, int codigoLocal, DateTime data, double latitude, double longitude, int codigoMonitoramento, int codigoCarga, int codigoVeiculo, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, AdminMultisoftware.Dominio.Enumeradores.TipoServicoMultisoftware tipoServicoMultisoftware, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente clienteMultisoftware, Repositorio.UnitOfWork unitOfWork)
        {
            if (data != DateTime.MinValue && codigoMonitoramento > 0)
            {
                ...
                if (permanenciaLocal == null)
                {
                    ...
                    permanenciaLocal.Carga = new() { Codigo = codigoCarga };
                    permanenciaLocal.DataInicio = data;
                    permanenciaLocal.Local = new() { Codigo = codigoLocal };
        ...
        private void SaidaDoAlvo(List<Dominio.Entidades.Embarcador.Logistica.PermanenciaCliente> permanenciasCliente, Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega cargaEntrega, double codigoCliente, DateTime data, double latitude, double longitude, int codigoMonitoramento, int codigoCarga, bool naoProcessarTrocaAlvoViaMonitoramentoTipoOperacao, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, AdminMultisoftware.Dominio.Enumeradores.TipoServicoMultisoftware tipoServicoMultisoftware, AdminMultisoftware.Dominio.Entidades.Pessoas.Cliente clienteMultisoftware, Repositorio.UnitOfWork unitOfWork, Dominio.Entidades.Embarcador.Configuracoes.ConfiguracaoControleEntrega configuracaoControleEntrega)
        {
            if (data != null && codigoMonitoramento > 0 && codigoCarga > 0)
            {
                ...
                if (cargaEntrega != null)
                {
                    ...
                    if (!cargaEntrega.DataSaidaRaio.HasValue || (cargaEntrega.DataSaidaRaio.HasValue && data < cargaEntrega.DataSaidaRaio) || (cargaEntrega.Carga.TipoOperacao?.ConfiguracaoControleEntrega?.SobrescreverDataEntradaSaidaAlvo ?? false))
                    {
                        inicio = DateTime.UtcNow;
                    ...
                    {
                        Dominio.Entidades.Embarcador.Logistica.PermanenciaCliente permanenciaCliente = ObterPermanenciaNoClienteAberta(permanenciasCliente, codigoCliente);
                        if (permanenciaCliente != null && permanenciaCliente.Codigo > 0)
                        {
                            ...
                            {
                                Servicos.Embarcador.Monitoramento.Monitoramento servMonitoramento = new Servicos.Embarcador.Monitoramento.Monitoramento();
                                servMonitoramento.RegistrarPosicaoEventosRelevantesTrizy(codigoCarga, data, latitude, longitude, EventoRelevanteMonitoramento.SaidaRaio, unitOfWork);
                            }
                            Servicos.Embarcador.Monitoramento.Monitoramento.GerarDadosSumarizadosMonitoramento(monitoramento, TipoRegraQualidadeMonitoramento.SaidaDestino, unitOfWork);
                        }
                    ...
                    {
                        Log("repPermanenciaCliente.Atualizar Exception: " + e.Message, 7);
                    }
                    ...
                    try
                    {
                        ...
                        if (cargaEntrega.Fronteira)
                            serOcorrenciaAutomaticaPorPeriodo.GerarOcorrenciaPorTracking(cargaEntrega.Carga, cargaEntrega.Cliente?.CPF_CNPJ ?? 0, Dominio.ObjetosDeValor.Embarcador.Enumeradores.GatilhoFinalTraking.SaidaFronteira, dataEntrada.Value, data, tipoServicoMultisoftware, clienteMultisoftware, tipoAplicacaoGatilhoTracking, cargaEntrega.DataAgendamento);
                        else if (cargaEntrega.Parqueamento)
                            serOcorrenciaAutomaticaPorPeriodo.GerarOcorrenciaPorTracking(cargaEntrega.Carga, cargaEntrega.Cliente?.CPF_CNPJ ?? 0, Dominio.ObjetosDeValor.Embarcador.Enumeradores.GatilhoFinalTraking.SaidaParqueamento, dataEntrada.Value, data, tipoServicoMultisoftware, clienteMultisoftware, tipoAplicacaoGatilhoTracking, cargaEntrega.DataAgendamento);
                        else
        ...
        private void EntradaNaSubarea(List<Dominio.Entidades.Embarcador.Logistica.PermanenciaSubarea> permanenciasSubarea, Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega cargaEntrega, DateTime data, int codigoSubarea, int codigoMonitoramento, Dominio.Entidades.Embarcador.Configuracoes.ConfiguracaoMonitoramento configuracaoMonitoramento, Repositorio.UnitOfWork unitOfWork)
        {
            if (cargaEntrega != null && data != null && codigoSubarea > 0)
            {
                ...
                if (configuracaoMonitoramento.HabilitarVinculoPermanenciasComHistoricoStatusViagem)
                {
                    ...
                    Dominio.Entidades.Embarcador.Logistica.MonitoramentoHistoricoStatusViagem monitoramentoHistoricoStatusViagemAnterior = repMonitoramentoHistoricoStatusViagem.BuscarUltimoHistoricoStatusViagem(codigoMonitoramento);
                    if (monitoramentoHistoricoStatusViagemAnterior != null)
                    {
                        repositorioMonitoramentoHistoricoStatusViagemPermanenciaSubArea.Inserir(new Dominio.Entidades.Embarcador.Logistica.MonitoramentoHistoricoStatusViagemPermanenciaSubArea()
                        {
                            PermanenciaSubarea = permanenciaSubarea,
                            MonitoramentoHistoricoStatusViagem = monitoramentoHistoricoStatusViagemAnterior
                        });
                    }
                }
            ...
            if (configuracao.MonitorarPassagensFronteiras && codigoClienge > 0 && codigoCarga > 0 && dataSaida > DateTime.MinValue)
            {
                ...
                if (minutosNaFronteira > configuracao.TempoMinutosPermanenciaCliente)
                {
                    ...
                    if (carga != null)
                    {
                        DateTime inicio = DateTime.UtcNow;
                        Servicos.Embarcador.CargaOcorrencia.OcorrenciaAutomaticaPorPeriodo serOcorrenciaAutomaticaPorPeriodo = new Servicos.Embarcador.CargaOcorrencia.OcorrenciaAutomaticaPorPeriodo(unitOfWork);
                        serOcorrenciaAutomaticaPorPeriodo.GerarOcorrenciaPorTracking(carga, codigoClienge, Dominio.ObjetosDeValor.Embarcador.Enumeradores.GatilhoFinalTraking.SaidaFronteira, dataEntrada, dataSaida, tipoServicoMultisoftware, clienteMultisoftware, TipoAplicacaoGatilhoTracking.AplicarSempre, null);
        ...
        private void GuardarClientesOrigemCargaCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            List<Tuple<int, Dominio.Entidades.Cliente>> clientesCarga = Servicos.Embarcador.Monitoramento.Monitoramento.BuscarClienteOrigemDasCargasPeloPedido(unitOfWork, monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            foreach (var clienteCarga in clientesCarga)
            {
                clienteOrigemCargaCache.Add(new ClienteOrigemCarga
                {
                    CodigoCarga = clienteCarga.Item1,
                    CodigoCliente = clienteCarga.Item2?.Codigo
                });
            }
            Log($"GuardarClientesOrigemCargaCache: {cargaEntregasCargaCache.Count}", inicio, 5);
        }

        private void GuardarCargaJanelasDescarregamentosCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            cargaJanelasDescarregamentosCache = BuscarJanelasDescarregamentoPorCargas(unitOfWork, monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            Log($"GuardarCargaJanelasDescarregamentosCache: {cargaEntregasCargaCache.Count}", inicio, 5);
        }

        private void GuardarMonitoramentoEventoCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            List<Dominio.Entidades.Embarcador.Logistica.MonitoramentoEvento> monitoramentoEventoCache = new();
            ...
            tiposAlerta.Add(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.ChegadaNoRaioEntrega);
            tiposAlerta.Add(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.PermanenciaNoPontoApoio);
            tiposAlerta.Add(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.ChegadaNoRaio);
            tiposAlerta.Add(Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.AtrasoNaDescarga);
            monitoramentoEventoCache = repMonitoramentoEvento.BuscarAtivosPorTipos(tiposAlerta);
            Log($"GuardarMonitoramentoEventoCache: {monitoramentoEventoCache.Count}", inicio, 5);
        }

        private void GuardarPosicaoAlvoClienteVeiculoCache(List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> trocasDeAlvo, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            List<Tuple<int, string, DateTime, DateTime>> listaTuplas = trocasDeAlvo
                .Where(trocaDeAlvo => !string.IsNullOrWhiteSpace(trocaDeAlvo.CodigosClientesAlvoPosicaoAtual) &&
                    (trocaDeAlvo.EmAlvoPosicaoAtual ?? false))
                .Select(trocaDeAlvo => new Tuple<int, string, DateTime, DateTime>(trocaDeAlvo.CodigoVeiculo,
                    trocaDeAlvo.CodigosClientesAlvoPosicaoAtual,
                    trocaDeAlvo.DataInicioMonitoramento ?? trocaDeAlvo.DataCriacaoMonitoramento ?? DateTime.MinValue,
                    trocaDeAlvo.DataVeiculoPosicaoAtual ?? DateTime.Now))
                .ToList();
            posicoesAlvoCache = BuscarPosicaoAlvosPorClienteVeiculo(unitOfWork, listaTuplas);
            Log($"GuardarPosicaoAlvoClienteVeiculoCache: {posicoesAlvoCache.Count}", inicio, 5);
        }

        private void GuardarPosicaoAlvoClienteSubareaVeiculoCache(List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> trocasDeAlvo, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            List<Tuple<int, string, DateTime, DateTime>> listaTuplas = trocasDeAlvo
                .Where(trocaDeAlvo => !string.IsNullOrWhiteSpace(trocaDeAlvo.CodigosSubareasAlvoPosicaoAtual) &&
                    (trocaDeAlvo.EmAlvoPosicaoAtual ?? false))
                .Select(trocaDeAlvo => new Tuple<int, string, DateTime, DateTime>(trocaDeAlvo.CodigoVeiculo,
                    string.Join(",", trocaDeAlvo.CodigosSubareasAlvoPosicaoAtual.Split(',').Select(subArea => subArea.Split('-')[0]).ToList()),
                    trocaDeAlvo.DataInicioMonitoramento ?? trocaDeAlvo.DataCriacaoMonitoramento ?? DateTime.MinValue,
                    trocaDeAlvo.DataVeiculoPosicaoAtual ?? DateTime.Now))
                .ToList();
            posicoesAlvoSubareaCache = BuscarPosicaoAlvosSubAreaPorClienteVeiculo(unitOfWork, listaTuplas);
            Log($"GuardarPosicaoAlvoClienteSubareaVeiculoCache: {posicoesAlvoSubareaCache.Count}", inicio, 5);
        }
        ...
        private void GuardarCargaEntregasNoCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            Repositorio.Embarcador.Cargas.ControleEntrega.CargaEntrega repCargaEntrega = new Repositorio.Embarcador.Cargas.ControleEntrega.CargaEntrega(unitOfWork);
            DateTime inicio = DateTime.UtcNow;
            List<Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega> cargaEntregas = repCargaEntrega.BuscarPorCargasComFetchs(monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            if (cargaEntregas.Count > 0)
            {
                ...
                {
                    int index = cargaEntregasCargaCache.FindIndex(c => c.CodigoCarga == cargaEntrega.Carga.Codigo);
                    if (index < 0)
                    {
                        cargaEntregasCargaCache.Add(new CargaEntregasCarga
                        {
                            CodigoCarga = cargaEntrega.Carga.Codigo,
                            CargaEntregas = new() { cargaEntrega }
                        });
                    }
                    else
                        cargaEntregasCargaCache[index].CargaEntregas.Add(cargaEntrega);
                }
            }
            Log($"cargaEntregasCargaCache: {cargaEntregasCargaCache.Count}", inicio, 5);
        }

        private void GuardarMonitoramentosNoCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            DateTime inicio = DateTime.UtcNow;
            monitoramentosCache = repMonitoramento.BuscarPorCargas(monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            Log($"GuardarMonitoramentosNoCache: {monitoramentosCache.Count}", inicio, 5);
        }
        ...
        private void GuardarPermanenciasClientesCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            Repositorio.Embarcador.Logistica.PermanenciaCliente repPermanenciaCliente = new Repositorio.Embarcador.Logistica.PermanenciaCliente(unitOfWork);
            List<Dominio.Entidades.Embarcador.Logistica.PermanenciaCliente> permanenciasCliente = repPermanenciaCliente.BuscarPorCargas(monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            if (permanenciasCliente.Count > 0)
            {
                foreach (Dominio.Entidades.Embarcador.Logistica.PermanenciaCliente permanenciaCliente in permanenciasCliente)
                {
                    int index = permanenciasClientesCargaCache.FindIndex(c => c.CodigoCarga == permanenciaCliente.CargaEntrega.Carga.Codigo);
                    if (index < 0)
                    {
                        permanenciasClientesCargaCache.Add(new PermanenciasClientesCarga
                        {
                            CodigoCarga = permanenciaCliente.CargaEntrega.Carga.Codigo,
                            PermanenciaClientes = new() { permanenciaCliente }
                        });
                    }
                    else
                        permanenciasClientesCargaCache[index].PermanenciaClientes.Add(permanenciaCliente);
                }
            }
            Log($"permanenciasClientesCargaCache: {permanenciasClientesCargaCache.Count}", inicio, 5);
        }
        ...
        private void GuardarPermanenciasSubareasCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            Repositorio.Embarcador.Logistica.PermanenciaSubarea repPermanenciaSubarea = new Repositorio.Embarcador.Logistica.PermanenciaSubarea(unitOfWork);
            List<Dominio.Entidades.Embarcador.Logistica.PermanenciaSubarea> permanenciasSubarea = repPermanenciaSubarea.BuscarPorCargas(monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            if (permanenciasSubarea.Count > 0)
            {
                foreach (Dominio.Entidades.Embarcador.Logistica.PermanenciaSubarea permanenciaSubarea in permanenciasSubarea)
                {
                    int index = permanenciasSubareasCargaCache.FindIndex(c => c.CodigoCarga == permanenciaSubarea.CargaEntrega.Carga.Codigo);
                    if (index < 0)
                    {
                        permanenciasSubareasCargaCache.Add(new PermanenciasSubareasCarga
                        {
                            CodigoCarga = permanenciaSubarea.CargaEntrega.Carga.Codigo,
                            PermanenciaSubareas = new() { permanenciaSubarea }
                        });
                    }
                    else
                        permanenciasSubareasCargaCache[index].PermanenciaSubareas.Add(permanenciaSubarea);
                }
            }
            Log($"permanenciasSubareasCargaCache: {permanenciasSubareasCargaCache.Count}", inicio, 5);
        }
        ...
        private void GuardarPermanenciasLocaisCache(List<MonitoramentoCarga> monitoramentosCarga, Repositorio.UnitOfWork unitOfWork)
        {
            DateTime inicio = DateTime.UtcNow;
            Repositorio.Embarcador.Logistica.PermanenciaLocal repPermanenciaLocal = new Repositorio.Embarcador.Logistica.PermanenciaLocal(unitOfWork);
            List<Dominio.Entidades.Embarcador.Logistica.PermanenciaLocal> permanenciasLocais = repPermanenciaLocal.BuscarPorCargas(monitoramentosCarga.Select(m => m.CodigoCarga).ToList());
            if (permanenciasLocais.Count > 0)
            {
                foreach (Dominio.Entidades.Embarcador.Logistica.PermanenciaLocal permanenciaLocal in permanenciasLocais)
                {
                    int index = permanenciasLocaisCargaCache.FindIndex(c => c.CodigoCarga == permanenciaLocal.Carga.Codigo);
                    if (index < 0)
                    {
                        permanenciasLocaisCargaCache.Add(new PermanenciasLocalCarga
                        {
                            CodigoCarga = permanenciaLocal.Carga.Codigo,
                            PermanenciaLocais = new() { permanenciaLocal }
                        });
                    }
                    else
                        permanenciasLocaisCargaCache[index].PermanenciaLocais.Add(permanenciaLocal);
                }
            }
            Log($"permanenciasLocaisCargaCache: {permanenciasLocaisCargaCache.Count}", inicio, 5);
        }
        ...
        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> ExtrairTrocasDeAlvoMonitoramento(List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> trocasDeAlvo, int codigoMonitoramento)
        {
            ...
            trocasDeAlvoEncontradas = trocasDeAlvo.Where(trocaDeAlvo => trocaDeAlvo.CodigoMonitoramento > 0 && trocaDeAlvo.CodigoMonitoramento == codigoMonitoramento).ToList();
            return trocasDeAlvoEncontradas;
        }

        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoSaidaDeAlvo> ExtrairSaidasDeAlvoMonitoramento(IList<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoSaidaDeAlvo> saidasDeAlvo, int codigoMonitoramento)
        {
            ...
            saidasDeAlvoEncontradas = saidasDeAlvo.Where(saidaDeAlvo => saidaDeAlvo.CodigoMonitoramento > 0 && saidaDeAlvo.CodigoMonitoramento == codigoMonitoramento).ToList();
        ...
        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> BuscarPosicoesVeiculo(Repositorio.UnitOfWork unitOfWork, int codigoVeiculo, DateTime dataInicial, DateTime dataFim)
        {
            ...
            IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> posicoes = repPosicao.BuscarWaypointsPorVeiculoDataInicialeFinal(codigoVeiculo, dataInicial, dataFim);
            return ValidarPosicoes(posicoes);
        }

        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> ValidarPosicoes(IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> posicoes)
        {
        ...
        private PosicaoEmAlvo IdentificarPrimeiraPosicaoEntradaAlvo(double codigoCliente, Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo trocaDeAlvo, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            {
                posicaoEmAlvo = BuscarPrimeiraPosicaoEntradaAlvo(codigoCliente, 0, trocaDeAlvo.CodigoCarga, 0, trocaDeAlvo.CodigoVeiculo, trocaDeAlvo.DataInicioMonitoramento ?? trocaDeAlvo.DataCriacaoMonitoramento ?? DateTime.MinValue, trocaDeAlvo.DataVeiculoPosicaoAtual ?? DateTime.Now, true, configuracao, unitOfWork);
            }
        ...
        private PosicaoEmAlvo IdentificarPrimeiraPosicaoEntradaLocal(int codigoLocal, Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo trocaDeLocal, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            if (configuracao.TempoMinutosPermanenciaCliente > 0)
            {
                posicaoEmAlvo = BuscarPrimeiraPosicaoEntradaLocal(codigoLocal, trocaDeLocal.CodigoCarga, 0, trocaDeLocal.CodigoVeiculo, trocaDeLocal.DataInicioMonitoramento ?? trocaDeLocal.DataCriacaoMonitoramento ?? DateTime.MinValue, trocaDeLocal.DataVeiculoPosicaoAtual ?? DateTime.MaxValue, true, configuracao, unitOfWork);
            }
            if (posicaoEmAlvo == null)
            {
                posicaoEmAlvo = new PosicaoEmAlvo
        ...
        private PosicaoEmAlvo BuscarPrimeiraPosicaoEntradaAlvo(double codigoCliente, int codigoSubarea, int codigoCarga, int codigoCargaEntrega, int codigoVeiculo, DateTime dataInicial, DateTime dataFinal, bool verificarPermancencia, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            if (verificarPermancencia)
            {
                ...
                if (codigoCliente > 0)
                {
                    List<Dominio.Entidades.Embarcador.Logistica.PermanenciaCliente> permanenciaCliente = permanenciasClientesCargaCache.Where(p => p.CodigoCarga == codigoCarga).SelectMany(p => p.PermanenciaClientes).ToList();
                    if (permanenciaCliente.Count > 0)
                    {
                        DateTime dataSaidaUltimaPermanencia = permanenciaCliente
                            .Where(obj => obj.Cliente.CPF_CNPJ == codigoCliente && obj.DataFim != null)?
                            .OrderByDescending(obj => obj.DataFim)
                            .FirstOrDefault()?.DataFim ?? DateTime.MinValue;

                        if (dataSaidaUltimaPermanencia > dataInicial)
                            dataInicial = dataSaidaUltimaPermanencia.AddSeconds(1);
                    }
                }
                else
                {
                    List<Dominio.Entidades.Embarcador.Logistica.PermanenciaSubarea> permanenciaSubAreas = permanenciasSubareasCargaCache.Where(p => p.CodigoCarga == codigoCarga).SelectMany(p => p.PermanenciaSubareas).ToList();
                    if (permanenciaSubAreas.Count > 0)
                    {
                        DateTime dataSaidaUltimaPermanencia = permanenciaSubAreas
                            .Where(obj => obj.Subarea.Codigo == codigoSubarea && obj.DataFim != null && obj.CargaEntrega.Codigo == codigoCargaEntrega)?
                            .OrderByDescending(obj => obj.DataFim)
                            .FirstOrDefault()?.DataFim ?? DateTime.MinValue;
            ...
            Repositorio.Embarcador.Logistica.Posicao repPosicao = new Repositorio.Embarcador.Logistica.Posicao(unitOfWork);
            //Dominio.ObjetosDeValor.Embarcador.Logistica.PosicaoAlvo posicaoAlvoEntradaCliente;
            Dominio.Entidades.Embarcador.Logistica.Posicao posicaoEntradaCliente;
            if (codigoCliente > 0)
            {
                //posicaoAlvoEntradaCliente = posicoesAlvoCache.Where(posicaoAlvo => posicaoAlvo.DataVeiculo >= dataInicial && posicaoAlvo.DataVeiculo <= dataFinal && posicaoAlvo.CodigoVeiculo == codigoVeiculo && posicaoAlvo.CodigoCliente == codigoCliente).OrderBy(posicaoAlvo => posicaoAlvo.DataVeiculo).First();
            ...
            {
                //posicaoAlvoEntradaCliente = posicoesAlvoSubareaCache.Where(posicaoAlvoSubareaCache => posicaoAlvoSubareaCache.DataVeiculo >= dataInicial && posicaoAlvoSubareaCache.DataVeiculo <= dataFinal && posicaoAlvoSubareaCache.CodigoVeiculo == codigoVeiculo && posicaoAlvoSubareaCache.CodigoClienteSubArea == codigoCliente).OrderBy(posicaoAlvo => posicaoAlvo.DataVeiculo).First();
        ...
        private PosicaoEmAlvo BuscarPrimeiraPosicaoEntradaLocal(int codigoLocal, int codigoCarga, int codigoCargaEntrega, int codigoVeiculo, DateTime dataInicial, DateTime dataFinal, bool verificarPermancencia, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracao, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            if (codigoLocal > 0)
            {
                IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> posicoes = repPosicao.BuscarWaypointsPorVeiculoDataInicialeFinal(codigoVeiculo, dataInicial, dataFinal);
        ...
        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> BuscarPendenciasTrocasDeAlvo(Repositorio.UnitOfWork unitOfWork, int quantidadeRegistros)
        {
...
#if DEBUG
            diretorioFila = "C:\\GerenciadorApp\\Producao\\Filas\\ProcessarTrocaAlvo\\";
#endif
            ...
            if (totalArquivos > 0)
            {
                ...
                for (int i = 0; i < totalArquivos; i++)
                {
                    ...
                    List<long> listaTodasPosicoes = pendencias[i].Pendencias.Select(x => x.PosicaoAtual ?? 0).Distinct().ToList();
                    listaTodasPosicoes.AddRange(pendencias[i].Pendencias.Select(x => x.PosicaoAnterior ?? 0).Distinct().ToList());

                    List<MonitoramentosTrocaAlvoSimplificado> ListMonitoramentos = repMonitoramentoTrocaAlvo.BuscarTrocasDeAlvoPendentesMonitoramentosPorCodigos(pendencias[i].Pendencias.Select(x => x.Monitoramento).Distinct().ToList());
                    List<MonitoramentoTrocaAlvoPosicoes> ListPosicoes = repMonitoramentoTrocaAlvo.BuscarPosicoesTrocaAlvoPorCodigos(listaTodasPosicoes.Distinct().ToList());
                    List<MonitoramentoTrocaDeAlvo> ListPendencias = PopularListaMonitoramentoTrocaAlvo(pendencias[i].Pendencias, ListPosicoes, ListMonitoramentos);

                    for (int j = 0; j < totalPendencias; j++)
                    {
                        Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo? pendencia = ListPendencias.Where(x => x.CodigoMonitoramento == pendencias[i].Pendencias[j].Monitoramento && x.CodigoPosicaoAtual == (pendencias[i].Pendencias[j].PosicaoAtual ?? 0) && x.CodigoPosicaoAnterior == (pendencias[i].Pendencias[j].PosicaoAnterior ?? 0)).FirstOrDefault(); //repMonitoramentoTrocaAlvo.BuscarTrocasDeAlvoPendentes(pendencias[i].Pendencias[j].Monitoramento, pendencias[i].Pendencias[j].PosicaoAtual ?? 0, pendencias[i].Pendencias[j].PosicaoAnterior ?? 0);
        ...
        private List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> PopularListaMonitoramentoTrocaAlvo(List<MonitoramentoPendencia> pendencias, List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaAlvoPosicoes> ListPosicoes, List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentosTrocaAlvoSimplificado> ListMonitoramentos)
        {
            List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo> trocasDeAlvoResult = new List<Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo>();

            for (int j = 0; j < pendencias.Count(); j++)
            {
                Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentosTrocaAlvoSimplificado? monitoramento = ListMonitoramentos.Where(x => x.CodigoMonitoramento == pendencias[j].Monitoramento).FirstOrDefault();
                if (monitoramento != null)
                {
                    Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo trocaAlvo = new Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaDeAlvo();
                    trocaAlvo.CodigoMonitoramento = monitoramento.CodigoMonitoramento;
                    trocaAlvo.CodigoCarga = monitoramento.CodigoCarga;
                    trocaAlvo.DataCriacaoMonitoramento = monitoramento.DataCriacaoMonitoramento;
                    trocaAlvo.DataInicioMonitoramento = monitoramento.DataInicioMonitoramento;
                    trocaAlvo.DataFimMonitoramento = monitoramento.DataFimMonitoramento;
                    trocaAlvo.CodigoVeiculo = monitoramento.CodigoVeiculo;
                    trocaAlvo.CodigoTipoOperacao = monitoramento.CodigoTipoOperacao;
                    trocaAlvo.RealizarBaixaEntradaNoRaio = monitoramento.RealizarBaixaEntradaNoRaio;
                    trocaAlvo.NaoProcessarTrocaAlvoViaMonitoramentoTipoOperacao = monitoramento.NaoProcessarTrocaAlvoViaMonitoramentoTipoOperacao;
                    trocaAlvo.CodigosClientesEntregas = monitoramento.CodigosClientesEntregas;

                    Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaAlvoPosicoes? posicaoAtual = ListPosicoes.Where(x => x.CodigoPosicao == pendencias[j].PosicaoAtual).FirstOrDefault();
                    if (posicaoAtual != null)
                    {
                        trocaAlvo.CodigoPosicaoAtual = posicaoAtual.CodigoPosicao;
                        trocaAlvo.CodigosClientesAlvoPosicaoAtual = posicaoAtual.CodigosClientesAlvoPosicao;
                        trocaAlvo.CodigosSubareasAlvoPosicaoAtual = posicaoAtual.CodigosSubareasAlvoPosicao;
                        trocaAlvo.CodigosLocaisPosicaoAtual = posicaoAtual.CodigosLocaisPosicao;
                        trocaAlvo.DataVeiculoPosicaoAtual = posicaoAtual.DataVeiculoPosicao;
                        trocaAlvo.EmAlvoPosicaoAtual = posicaoAtual.EmAlvoPosicao;
                        trocaAlvo.EmLocalPosicaoAtual = posicaoAtual.EmLocalPosicao;
                        trocaAlvo.LatitudePosicaoAtual = posicaoAtual.LatitudePosicao;
                        trocaAlvo.LongitudePosicaoAtual = posicaoAtual.LongitudePosicao;
                    }

                    Dominio.ObjetosDeValor.Embarcador.Logistica.MonitoramentoTrocaAlvoPosicoes? posicaoAnterior = ListPosicoes.Where(x => x.CodigoPosicao == pendencias[j].PosicaoAnterior).FirstOrDefault();
                    if (posicaoAnterior != null)
                    {
                        trocaAlvo.CodigoPosicaoAnterior = posicaoAnterior.CodigoPosicao;
                        trocaAlvo.CodigosClientesAlvoPosicaoAnterior = posicaoAnterior.CodigosClientesAlvoPosicao;
                        trocaAlvo.CodigosSubareasAlvoPosicaoAnterior = posicaoAnterior.CodigosSubareasAlvoPosicao;
                        trocaAlvo.CodigosLocaisPosicaoAnterior = posicaoAnterior.CodigosLocaisPosicao;
                        trocaAlvo.DataVeiculoPosicaoAnterior = posicaoAnterior.DataVeiculoPosicao;
                        trocaAlvo.EmAlvoPosicaoAnterior = posicaoAnterior.EmAlvoPosicao;
                        trocaAlvo.EmLocalPosicaoAnterior = posicaoAnterior.EmLocalPosicao;
                        trocaAlvo.LatitudePosicaoAnterior = posicaoAnterior.LatitudePosicao;
                        trocaAlvo.LongitudePosicaoAnterior = posicaoAnterior.LongitudePosicao;
                    }

                    trocasDeAlvoResult.Add(trocaAlvo);
                }

            }

            return trocasDeAlvoResult;
        ...
        private void GerarEventoAtrasoDescargaNaEntradaRaioEntrega(Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega cargaEntrega, int codigoCarga, int codigoVeiculo, double latitude, double longitude, DateTime data, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            {
                Dominio.Entidades.Embarcador.Logistica.MonitoramentoEvento evento = monitoramentoEventoCache.Find(e => e.TipoAlerta == Dominio.ObjetosDeValor.Embarcador.Enumeradores.TipoAlerta.AtrasoNaDescarga);
        ...
        {
            Repositorio.Embarcador.Cargas.ControleEntrega.CargaEntrega repCargaEntrega = new Repositorio.Embarcador.Cargas.ControleEntrega.CargaEntrega(unitOfWork);
            Dominio.Entidades.Embarcador.Logistica.MonitoramentoEvento evento = monitoramentoEventoCache.Find(e => e.TipoAlerta == tipoAlerta);

            if (evento != null && evento.Gatilho != null)
            {
                CargaEntregasCarga? cargaEntregas = cargaEntregasCargaCache.Find(cargaEntrega => cargaEntrega.CodigoCarga == codigoCarga);
                Dominio.Entidades.Embarcador.Cargas.ControleEntrega.CargaEntrega? cargaEntrega = cargaEntregas?.CargaEntregas.Find(entrega => entrega.Codigo == codigoEntrega);
        ...
        private void ProcessarAcoesFluxoDePatioSubarea(Dominio.Entidades.Embarcador.Logistica.PermanenciaSubarea permanenciaSubarea, MonitoramentoEventoData eventoMonitoramento, Repositorio.UnitOfWork unitOfWork)
        {
            ...
            try
            {
                ...
                if (subAreaCliente.TipoSubarea.PermiteMovimentacaoDoPatioPorEntradaOuSaidaDaArea ?? false)
                {
                    ...
                    Servicos.Embarcador.GestaoPatio.FluxoGestaoPatio servicoFluxoGestaoPatio = new Servicos.Embarcador.GestaoPatio.FluxoGestaoPatio(unitOfWork, auditado);
