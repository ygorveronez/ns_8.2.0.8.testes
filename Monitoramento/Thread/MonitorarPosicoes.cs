...
namespace Monitoramento.Thread
{
    ...
    public class MonitorarPosicoes : AbstractThreadProcessamento
    {
        ...
        private bool espelharPosicaoReboque = false;
        ...
        {
            if (enable)
                MonitorarPosicoesThread = base.IniciarThread(stringConexao, ambiente, tipoServicoMultisoftware, clienteMultisoftware, arquivoNivelLog, tempoSleep, cancellationToken);
        ...
        override protected void Parar()
        {
            ...
            {
                MonitorarPosicoesThread.Interrupt();
        ...
        private MonitorarPosicoes(string stringConexao)
        {
            ...
            try
            {
                ...
                espelharPosicaoReboque = Servicos.Embarcador.Configuracoes.ConfiguracaoMonioramentoInstance.GetInstance(unitOfWork).ObterConfiguracaoMonitoramentoMonitorarPosicoes().EspelharPosicaoReboque;

#if DEBUG
                processarTrocaDeAlvoDiretorioFila = "C:\\GerenciadorApp\\Producao\\Filas\\ProcessarTrocaAlvo\\";
                processarEventosDiretorioFila = "C:\\GerenciadorApp\\Producao\\Filas\\ProcessarEventos\\";
#endif
        ...
        private void ProcessarPosicoes(Repositorio.UnitOfWork unitOfWork, List<Dominio.Entidades.Embarcador.Logistica.Posicao> posicoes)
        {
            ...
            if (total > 0)
            {
                ...
                IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento> monitoramentosAbertos = BuscarMonitoramentosAbertos(unitOfWork, codigosVeiculos, posicoes);

                this.PosicaoAlvosPosicaoAlvosSubareas = new List<PosicaAlvoPosicaoAlvoSubarea>();
                this.PosicaoLocais = new List<Dominio.Entidades.Embarcador.Logistica.PosicaoLocal>();
                ...
                try
                {
                        ...
                        for (int j = 0; j < contj; j++)
                        {
                            ...
                            if (ConfirmarValidadeDaPosicao(unitOfWork, repPosicao, posicao, posicaoObjetoDeValor, configuracao))
                            {
                                ...
                                if (VerificaSeDeveAtualizarPosicaoAtual(codigoVeiculo, monitoramento?.Codigo ?? 0, listaCodigosVeiculosEmContrato, configuracao))
                                {
                                    ...
                                    posicaoAtual = ProcessaPosicaoAtual(unitOfWork, posicaoAtual, posicao, situacao, monitoramento);
                    ...
                    unitOfWork.CommitChanges();//nao remover
        ...
        private void IndicarMonitoramentosParaAtualizarRota(Repositorio.UnitOfWork unitOfWork, List<MonitoramentoPendenteAtualizar> monitoramentosParaAtualizarRota)
        {
            ...
            if (processarMonitoramentos && monitoramentosParaAtualizarRota.Count > 0)
            {
                ...
                List<Dominio.ObjetosDeValor.Embarcador.FaixaTemperatura.FaixaTemperaturaMonitoramento> ListfaixaTemperatura = repFaixaTemperatura.BuscarPorCodigosMonitoramentos(monitoramentosParaAtualizarRota.Select(X => X.codigoMonitoramento).ToList());
                for (int i = 0; i < total; i++)
                {
                    Dominio.ObjetosDeValor.Embarcador.FaixaTemperatura.FaixaTemperaturaMonitoramento? faixaTemperatura = ListfaixaTemperatura?.FirstOrDefault(x => x.CodigoMonitoramento == monitoramentosParaAtualizarRota[i].codigoMonitoramento) ?? null;
                    ...
                    bool contabilizarTemperaturaFaixa = !repMonitoramento.VerificarSeNaoValidaStatusViagemPorMonitoramento(monitoramentosParaAtualizarRota[i].codigoMonitoramento);
                    ...
                    {
                        // Marca o monitoramento para ser atualizado
                        repMonitoramento.AtualizarProcessarRota(new List<int>() { monitoramentosParaAtualizarRota[i].codigoMonitoramento }, monitoramentosParaAtualizarRota[i].codigoPosicao, monitoramentosParaAtualizarRota[i].temperatura, naFaixa, Dominio.ObjetosDeValor.Embarcador.Enumeradores.ProcessarPosicao.Pendente, connection, transaction, contabilizarTemperaturaFaixa);
                        transaction.Commit();
                    }
                    catch (Exception e)
                    {
        ...
        private void LoadPosicoesCache(Repositorio.UnitOfWork unitOfWork, int codigoVeiculo)
        {
            ...
            if (veiculoPosicao == null)
            {
                ...
                Repositorio.Embarcador.Logistica.Posicao repPosicao = new Repositorio.Embarcador.Logistica.Posicao(unitOfWork);
                // TODO: ToList cast
                List<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> posicoes = repPosicao.BuscarWaypointsPorVeiculoDataInicialeFinal(codigoVeiculo, this.dataAtual.AddHours(horasPosicoesCache), this.dataAtual).ToList();
        ...
        private void LoadPosicoesVeiculosCache(Repositorio.UnitOfWork unitOfWork, List<int> codigosVeiculos)
        {
            ...
            if (total > 0)
            {
                ...
                inicio1 = DateTime.UtcNow;
                Repositorio.Embarcador.Logistica.Posicao repPosicao = new Repositorio.Embarcador.Logistica.Posicao(unitOfWork);
                IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao> posicoes = repPosicao.BuscarWaypointsPorVeiculosDataInicialeFinal(codigosVeiculosSemCache, this.dataAtual.AddHours(horasPosicoesCache), this.dataAtual);
        ...
        private void LoadPosicoesAtuaisCache(Repositorio.UnitOfWork unitOfWork, List<int> codigosVeiculos)
        {
            ...
            if (this.espelharPosicaoReboque)
            {
                Repositorio.Veiculo repVeiculo = new Repositorio.Veiculo(unitOfWork);
                List<int> codigosReboques = repVeiculo.BuscarCodigosTodosReboquesAsync().Result;

                if (codigosReboques != null && codigosReboques.Any())
                {
                    codigosVeiculos.AddRange(codigosReboques);
                }
            }

            Repositorio.Embarcador.Logistica.PosicaoAtual repPosicaoAtual = new Repositorio.Embarcador.Logistica.PosicaoAtual(unitOfWork);
            this.PosicoesAtuaisCache.Clear();
            this.PosicoesAtuaisCache = repPosicaoAtual.BuscarPorVeiculosAsync(codigosVeiculos).Result;

            this.PosicoesAtuaisCache = this.PosicoesAtuaisCache
                .GroupBy(p => p.Codigo)
                .Select(g => g.First())
                .ToList();
        ...
        private Dominio.Entidades.Embarcador.Logistica.PosicaoAtual ProcessaPosicaoAtual(Repositorio.UnitOfWork unitOfWork, Dominio.Entidades.Embarcador.Logistica.PosicaoAtual posicaoAtual, Dominio.Entidades.Embarcador.Logistica.Posicao posicao, Dominio.ObjetosDeValor.Embarcador.Enumeradores.StatusPosicao status, Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento monitoramento)
        {
            ...
            if (espelharPosicaoReboque)
                AtualizaPosicaoReboqueVeiculo(unitOfWork, posicaoAtual, posicao, status, monitoramento);
        ...
        private void AtualizaPosicaoReboqueVeiculo(UnitOfWork unitOfWork, Dominio.Entidades.Embarcador.Logistica.PosicaoAtual posicaoAtual, Dominio.Entidades.Embarcador.Logistica.Posicao posicao, StatusPosicao status, Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento monitoramento)
        {
            DateTime inicio = DateTime.UtcNow;
            ...
            if (monitoramento != null)
            {
                var veiculosDaCarga = repVeiculo.BuscarVeiculosVinculadoACarga(monitoramento.Carga);

                foreach (var veiculo in veiculosDaCarga)
                {
                    if (veiculo != posicaoAtual.Veiculo.Codigo)
                    {
                        Dominio.Entidades.Embarcador.Logistica.PosicaoAtual posicaoAtualReboque = ObtemPosicaoAtualDoCache(veiculo);

                        if (posicaoAtualReboque == null)
                        {
                            posicaoAtualReboque = new Dominio.Entidades.Embarcador.Logistica.PosicaoAtual();
                            NovaPosicaoAtualApartirDaPosicao(posicaoAtualReboque, posicao, veiculo);
                            posicaoAtualReboque.Status = status;
                            repPosicaoAtual.Inserir(posicaoAtualReboque);
                            this.PosicoesAtuaisCache.Add(posicaoAtualReboque);
                        }

                        // ... considera a posição recebida como posição atual apenas se for mais nova que a posição atual já existente (Há casos de posições recebidas fora de ordem)
                        else if (posicao.DataVeiculo > posicaoAtualReboque.DataVeiculo)
                        {
                            NovaPosicaoAtualApartirDaPosicao(posicaoAtualReboque, posicao, veiculo);
                            posicaoAtualReboque.Status = status;
                            repPosicaoAtual.Atualizar(posicaoAtualReboque);
                        }
                    }
            ...
            Log("AtualizaPosicaoReboqueVeiculo", inicio, 6);
        }
        ...
        private IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento> BuscarMonitoramentosAbertos(Repositorio.UnitOfWork unitOfWork, List<int> codigosVeiculos, List<Dominio.Entidades.Embarcador.Logistica.Posicao> posicoes)
        {
            DateTime inicio = DateTime.UtcNow;
            ...
            Repositorio.Embarcador.Logistica.Monitoramento repMonitoramento = new Repositorio.Embarcador.Logistica.Monitoramento(unitOfWork);
            IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento> monitoramentos = repMonitoramento.BuscarMonitoramentosEmAbertoNoPeriodo(codigosVeiculos, dataInicio, dataAtual);
         ...
         */
        private Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento BuscarMonitoramentoEmAberto(IList<Dominio.ObjetosDeValor.Embarcador.Logistica.Monitoramento> monitoramentos, Dominio.ObjetosDeValor.Embarcador.Logistica.Posicao posicao)
        {
            DateTime inicio = DateTime.UtcNow;
                    ...
                    if (monitoramentos[i].Veiculo > 0 && monitoramentos[i].Veiculo == posicao.CodigoVeiculo &&
                        (
                            ...
                            (
                                ...
                                posicao.DataVeiculo >= monitoramentos[i].DataInicioMonitoramento && posicao.DataVeiculo <= monitoramentos[i].DataFimMonitoramento
                            )
                        )
                    )
                    {
        ...
        private void IdentificarAlvos(Dominio.Entidades.Embarcador.Logistica.Posicao posicao, Dominio.Entidades.Embarcador.Configuracao.ConfiguracaoTMS configuracaoEmbarcador)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < total; i++)
                {
                    ...
                    PosicaAlvoPosicaoAlvoSubarea posicaoAlvoPosicaoAlvoSubarea = PosicaoAlvosPosicaoAlvosSubareas.FirstOrDefault(x => x.PosicaoAlvo?.Cliente?.CPF_CNPJ == clientesEmAlvo[i].Codigo && x.PosicaoAlvo?.Posicao?.Codigo == posicao.Codigo);

                    if (posicaoAlvoPosicaoAlvoSubarea == null)
                    {
                        adicionar = true;
                        posicaoAlvoPosicaoAlvoSubarea = new PosicaAlvoPosicaoAlvoSubarea();
                    ...
                    if ((clientesEmAlvo[i].Subareas?.Length ?? 0) > 0)
                    {
                        ...
                        for (int j = 0; j < totalSubarea; j++)
                        {
                            ...
                            if (!posicaoAlvoPosicaoAlvoSubarea.PosicaoAlvoSubareas.Exists(PAS => PAS.SubareaCliente.Codigo == posicaoAlvoSubarea.Codigo))
        ...
        private void SalvarPosicaoAlvos(Repositorio.UnitOfWork unitOfWork)
        {
            ...
            if (total > 0)
            {
                ...
                List<Dominio.Entidades.Embarcador.Logistica.PosicaoAlvo> posicaoAlvos = repPosicaoAlvo.BuscarPorPosicaoEClientes(
                    PosicaoAlvosPosicaoAlvosSubareas.Select(posicaoAlvo => posicaoAlvo.PosicaoAlvo.Posicao.Codigo).ToList(),
                    PosicaoAlvosPosicaoAlvosSubareas.Select(posicaoAlvo => posicaoAlvo.PosicaoAlvo.Cliente.CPF_CNPJ).ToList());

                List<Dominio.Entidades.Embarcador.Logistica.PosicaoAlvoSubarea> posicaoAlvoSubareas = repPosicaoAlvoSubarea.BuscarPorPosicaoAlvosESubAreaClientes(
                    PosicaoAlvosPosicaoAlvosSubareas.Select(posicaoAlvo => posicaoAlvo.PosicaoAlvo.Codigo).ToList(),
                    PosicaoAlvosPosicaoAlvosSubareas.SelectMany(posicaoAlvo => posicaoAlvo.PosicaoAlvoSubareas).Select(subarea => subarea.SubareaCliente.Codigo).ToList());

                for (int i = 0; i < total; i++)
                {
                    try
                    {
                        Dominio.Entidades.Embarcador.Logistica.PosicaoAlvo posicaoAlvoBusca = PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvo;
                        Dominio.Entidades.Embarcador.Logistica.PosicaoAlvo posicaoAlvo = posicaoAlvos.FirstOrDefault(x => x.Equals(posicaoAlvoBusca));
                        if (posicaoAlvo == null)
                            repPosicaoAlvo.Inserir(PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvo);
                        else
                            PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvo = posicaoAlvo;

                        int totalSubareas = PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvoSubareas.Count;
                        for (int j = 0; j < totalSubareas; j++)
                        {
                            Dominio.Entidades.Embarcador.Logistica.PosicaoAlvoSubarea posicaoAlvoSubarea = posicaoAlvoSubareas.FirstOrDefault(posicaoAlvoSubarea => posicaoAlvoSubarea.PosicaoAlvo == PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvo && posicaoAlvoSubarea.SubareaCliente.Cliente.Codigo == PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvoSubareas[j].SubareaCliente.Cliente.Codigo);

                            PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvoSubareas[j].PosicaoAlvo = PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvo;
                            if (posicaoAlvoSubarea == null)
                                repPosicaoAlvoSubarea.Inserir(PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvoSubareas[j]);
                            else
                                PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvoSubareas[j] = posicaoAlvoSubarea;
                        }
                    }
                    catch (Exception ex)
                    {
                        Log($"Erro ao salvar PosicaoAlvo {PosicaoAlvosPosicaoAlvosSubareas[i].PosicaoAlvo?.Cliente?.CPF_CNPJ} - {ex.Message}", 5);
                    }
