namespace SGT.WebAdmin.ProcessarEmissaoDocumentos
{
    public class BackgroundServiceBase : BackgroundService
    {
        ...
        public string WebServiceConsultaCTe
        {
            get
            {
                using (AdminMultisoftware.Repositorio.UnitOfWork unitOfWork = new AdminMultisoftware.Repositorio.UnitOfWork(StringConexaoAdmin))
                {
                    ...
                    return clienteURLAcesso.WebServiceConsultaCTe;
                }
            }
        }
                ...
                if (string.IsNullOrWhiteSpace(_stringConexao))
                {
                    using (AdminMultisoftware.Repositorio.UnitOfWork unitOfWork = new AdminMultisoftware.Repositorio.UnitOfWork(StringConexaoAdmin, AdminMultisoftware.Dominio.Enumeradores.TipoSessaoBancoDados.Nova))
                    {
                        ...
                        if (clienteURLAcesso.URLHomologacao && clienteURLAcesso.Cliente.ClienteConfiguracaoHomologacao != null)
                            _stringConexao = GerarStringConexao(clienteURLAcesso.Cliente.ClienteConfiguracaoHomologacao, this.MaxPoolSize);
                        else
                            _stringConexao = GerarStringConexao(clienteURLAcesso.Cliente.ClienteConfiguracao, this.MaxPoolSize);
        ...
        private static string GerarStringConexao(AdminMultisoftware.Dominio.Entidades.Pessoas.ClienteConfiguracao configuracao, int maxPoolSize = 0)
        {
            string conectionString = $"Application Name=SGT.WebAdmin.ProcessarEmissaoDocumentos;Data Source={configuracao.DBServidor};Initial Catalog={configuracao.DBBase};User Id={configuracao.DBUsuario};Password={configuracao.DBSenha};";
            if (maxPoolSize != 0)
                conectionString += $"Max Pool Size={maxPoolSize};";
